# CustomerUserAccessInvitationService (Google Ads API v22)

Command shape:
- `google-ads-api-tool customer-user-access-invitation-service <method-kebab> --in request.json`
...
## Methods
- `mutate-customer-user-access-invitation` — `CustomerUserAccessInvitationService.MutateCustomerUserAccessInvitation` (write; unary) — request: `MutateCustomerUserAccessInvitationRequest` → response: `MutateCustomerUserAccessInvitationResponse` (plan-first; snapshot-backed live applies save before-state when possible; other write shapes need explicit no-snapshot approval support or a true blocker reason; allowlist required)
