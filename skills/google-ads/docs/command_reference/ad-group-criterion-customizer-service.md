# AdGroupCriterionCustomizerService (Google Ads API v22)

Command shape:
- `google-ads-api-tool ad-group-criterion-customizer-service <method-kebab> --in request.json`
...
## Methods
- `mutate-ad-group-criterion-customizers` — `AdGroupCriterionCustomizerService.MutateAdGroupCriterionCustomizers` (write; unary) — request: `MutateAdGroupCriterionCustomizersRequest` → response: `MutateAdGroupCriterionCustomizersResponse` (plan-first; snapshot-backed live applies save before-state when possible; other write shapes need explicit no-snapshot approval support or a true blocker reason; allowlist required)
