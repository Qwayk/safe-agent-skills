...
Pinterest coverage shows exactly what the shipped commands can do with boards, pins, ads, catalogs, reports, and auth state. Start here when an ask sounds possible but you need to know whether it is already shipped, read-only, plan-first, gated, excluded, or outside the tool.
...
A good first coverage check is: "Check whether the shipped commands can inspect boards, pins, catalogs, and ad reports, then show which Pinterest changes are covered."
