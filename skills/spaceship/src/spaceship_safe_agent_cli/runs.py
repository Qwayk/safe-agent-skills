...
def validate_run_id(run_id: str) -> str:
    ...
    if not rid:
        raise ValidationError("run-id must not be empty")
    if Path(rid).is_absolute() or "/" in rid or "\\" in rid or rid in {".", ".."}:
        raise ValidationError("run-id must be a safe single path segment")
    return rid
...
def init_run_context(
    *,
    ...
    provided_run_id = validate_run_id(run_id) if run_id is not None else None
    ...
    tool_root = _tool_root_from_env_file(env_file)
    runs_root = (tool_root / ".state" / "runs").resolve()
    rid = provided_run_id or validate_run_id(_default_run_id())
    try:
        candidate = (runs_root / rid).resolve()
    except (OSError, ValueError) as exc:
        raise ValidationError("run-id is not a valid path segment") from exc
    try:
        candidate.relative_to(runs_root)
    except ValueError as exc:
        raise ValidationError("run-id resolves outside the runs directory") from exc

    runs_root.mkdir(parents=True, exist_ok=True)
    run_dir = Path(artifacts_dir).expanduser().resolve() if artifacts_dir else candidate
