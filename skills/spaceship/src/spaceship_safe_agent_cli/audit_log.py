...
_OPAQUE_KEYS = {"_raw", "body", "detail", "details", "error", "message", "payload", "raw", "response"}
...
def _sanitize_opaque(obj: Any) -> Any:
    if isinstance(obj, dict):
        return {
            key: ("***REDACTED***" if str(key).lower() in _OPAQUE_KEYS else _sanitize_opaque(value))
            for key, value in obj.items()
        }
    if isinstance(obj, list):
        return [_sanitize_opaque(value) for value in obj]
    return obj
...
class AuditLogger:
    ...
    def write(self, event: str, payload: dict[str, Any]) -> None:
        ...
        if bool(self._context.get("private_data")):
            safe_payload = _sanitize_opaque(safe_payload)
        row = {
