...
class TestReadmePublicContract(unittest.TestCase):
    def test_public_readme_opening_stays_user_facing(self) -> None:
        ...
        self.assertTrue(text.startswith("# Figma\n"))
        self.assertNotIn("## Simplicity lock", text)
        self.assertNotIn("# figma-safe-agent-cli", text)
        self.assertIn("Figma is where product screens", text)
        for stale_phrase in [
            "Use this skill when",
    ...
    def test_helpful_docs_stay_human_facing(self) -> None:
        ...
        self.assertIn("[Browse all Figma docs](docs/README.md)", text)
    ...
    def test_start_here_section_stays_human_facing(self) -> None:
        ...
        self.assertIn("[What you can do with Figma](docs/use_cases.md)", text)
        self.assertIn("[Connect your Figma account](docs/onboarding.md)", text)
    ...
    def test_use_cases_stay_specific_and_human(self) -> None:
        ...
        self.assertIn("Figma work is usually about understanding a file", text)
        self.assertIn("## What the agent should show you", text)
        self.assertNotIn("Why this beats typical no-code automation", text)
        self.assertNotIn("generic request bridge", text)
