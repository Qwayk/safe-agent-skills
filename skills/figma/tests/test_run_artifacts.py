...
class TestRunArtifacts(unittest.TestCase):
    ...
    def test_operations_read_run_creates_run_folder_and_index(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            env_path = root / ".env"
            self._write_env(env_path)
            body_path = root / "comment_body.json"
            body_path.write_text('{"comment":"hello"}', encoding="utf-8")

            run_id = "2026-01-19T120000Z_readop"
            buf = io.StringIO()
    ...
    def test_runs_list_and_show_work(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            run_id = "2026-01-19T120500Z_list"
    ...
    def test_refusal_still_creates_run_history(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            body_path = root / "comment_body.json"
            body_path.write_text('{"text":"delete test"}', encoding="utf-8")
