...
def cmd_live_media_list(args: Any, ctx: dict[str, Any]) -> int:
    ...
    client = _client(ctx)
    result = client.get(f"/{ig_user_id}/live_media", params=params)
    out = {"ok": True, "command": "live-media.list", "result": result}
    ctx["audit"].write("live_media.list", out)
    ctx["out"].emit(out)
    return 0
