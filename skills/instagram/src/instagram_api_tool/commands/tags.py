...
def cmd_tags_list(args: Any, ctx: dict[str, Any]) -> int:
    ...
    client = _client(ctx)
    result = client.get(f"/{ig_user_id}/tags", params=params)
    out = {"ok": True, "command": "tags.list", "result": result}
    ctx["audit"].write("tags.list", out)
    ctx["out"].emit(out)
    return 0
