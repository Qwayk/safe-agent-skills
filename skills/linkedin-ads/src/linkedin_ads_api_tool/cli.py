...
def build_parser() -> argparse.ArgumentParser:
    p = _ToolArgumentParser(prog="linkedin-ads-api-tool")
    ...
    operations_cmd.register_operation_families(sub, parser_class=_ToolArgumentParser)
...
def main(argv: list[str]) -> int:
    ...
    try:
        if bool(args.version):
            payload = {"ok": True, "tool": "linkedin-ads-api-tool", "version": __version__}
            if args.output == "json":
                out.emit(payload)
            else:
                print(f"linkedin-ads-api-tool {__version__}")
            return 0

        if not getattr(args, "cmd", None):
            parser.error("Missing command. Use --help to see available commands.")

        command_str = "linkedin-ads-api-tool " + " ".join(argv)
        audit.bind_context(
            {
                "tool": "linkedin-ads-api-tool",
                "version": __version__,
                "command": command_str,
        ...
        _finalize_run_artifacts(
            ...
            tool="linkedin-ads-api-tool",
            version=__version__,
            command="linkedin-ads-api-tool " + " ".join(argv),
            env_fingerprint=None,
            output_obj=out.last if isinstance(out.last, dict) else None,
            audit_log_path=run_audit_log_path or global_audit_log_path,
            audit_log_global_path=global_audit_log_path,
