...
def cmd_onboarding(args: object, ctx: dict) -> int:
    ...
    missing: list[str] = []
    if not _has_nonempty_env_value(lines, "SHOPIFY_SHOP_DOMAIN"):
        missing.append("SHOPIFY_SHOP_DOMAIN")
    if not _has_nonempty_env_value(lines, "SHOPIFY_ADMIN_ACCESS_TOKEN"):
        missing.append("SHOPIFY_ADMIN_ACCESS_TOKEN")
    if not _has_nonempty_env_value(lines, "SHOPIFY_ADMIN_API_VERSION"):
        missing.append("SHOPIFY_ADMIN_API_VERSION")
    ...
    steps = [
        ...
        "Run: shopify-admin-api-tool --output json auth check",
    ]
    ...
    payload = {
        ...
        "onboarding": {
            ...
            "missing": missing,
            "next_command": "shopify-admin-api-tool --output json auth check",
            "steps": steps,
        },
    }
    ...
    if str(getattr(args, "output", "json")) == "json":
        ...
        print("")
        print("Next: shopify-admin-api-tool --output json auth check")
    return 0
