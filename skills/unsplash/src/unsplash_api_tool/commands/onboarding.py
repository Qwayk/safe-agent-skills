...
def cmd_onboarding(args: object, ctx: dict) -> int:
    ...
    missing: list[str] = []
    if not _has_nonempty_env_value(lines, "UNSPLASH_API_BASE_URL"):
        missing.append("UNSPLASH_API_BASE_URL")
    if not _has_nonempty_env_value(lines, "UNSPLASH_ACCESS_KEY"):
    ...
    steps = [
        ...
        "Run: unsplash-api-tool --output json auth check",
    ]
    ...
    payload = {
        ...
        "onboarding": {
            ...
            "missing": missing,
            "next_command": "unsplash-api-tool --output json auth check",
            "steps": steps,
        },
    }
    ...
    if str(getattr(args, "output", "json")) == "json":
        ...
        print("")
        print("Next: unsplash-api-tool --output json auth check")
    return 0
