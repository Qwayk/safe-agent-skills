...
JOBS_WRITE_REFUSAL_REASON = (
    "Refused: Unsplash jobs write.ping is template-only and has no live provider write executor."
)
...
def _demo_write(row: dict[str, str], ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    row_plan = {
        ...
        "before_state": before_state_contract(
            reason=(
                "jobs write.ping is a stubbed write row and does not save real before-state. "
...
def _build_jobs_plan(*, job_file: str | None, rows: list[dict[str, Any]], ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    restore_note = (
        "Jobs run has no automatic rollback in this CLI. "
        "If write actions changed remote state, clean up manually where that API supports it."
    )
    plan = {
...
def cmd_jobs_run(args: Any, ctx: dict[str, Any]) -> int:
    ...
    receipt = {
        ...
        "recovery": {
            ...
            "snapshots": [],
            "rollback_plan": None,
            "restore_note": "Jobs run has no automatic rollback in this CLI. "
            "If write actions changed remote state, clean up manually where that API supports it.",
        },
    }
