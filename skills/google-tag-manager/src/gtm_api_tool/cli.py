...
from .commands.discovery_methods import register_discovery_commands
...
def _finalize_run_artifacts(
    *,
    ...
    before_state_file = run_ctx.artifacts_dir / "before_state.json"
    plan_path = str(plan_file) if plan_file.exists() else None
    receipt_path = str(receipt_file) if receipt_file.exists() else None
    if before_state_path is None and before_state_file.exists():
        before_state_path = str(before_state_file)
...
def _before_state_path_from_output(output_obj: dict | None) -> str | None:
    if not isinstance(output_obj, dict):
        return None
    receipt = output_obj.get("receipt")
    if isinstance(receipt, dict):
        path = str(receipt.get("before_state_path") or "").strip() or None
        if path:
            return path
        before_state = receipt.get("before_state")
        if isinstance(before_state, dict):
            path = str(before_state.get("artifact_path") or "").strip() or None
            if path:
                return path
    plan = output_obj.get("plan")
    if isinstance(plan, dict):
        path = str(plan.get("before_state_path") or "").strip() or None
        if path:
            return path
...
def build_parser() -> argparse.ArgumentParser:
    p = _ToolArgumentParser(prog="gtm-api-tool")
    ...
    # Register explicit method commands from the pinned discovery snapshot.
    register_discovery_commands(root_subparsers=sub, parser_class=_ToolArgumentParser)
...
def main(argv: list[str]) -> int:
    ...
    try:
        if bool(args.version):
            payload = {"ok": True, "tool": "gtm-api-tool", "version": __version__}
            if args.output == "json":
                out.emit(payload)
            else:
                print(f"gtm-api-tool {__version__}")
            return 0

        if not getattr(args, "cmd", None):
            parser.error("Missing command. Use --help to see available commands.")

        command_str = "gtm-api-tool " + " ".join(argv)
        audit.bind_context(
            {
                "tool": "gtm-api-tool",
                "version": __version__,
        ...
        _finalize_run_artifacts(
            ...
            apply=bool(args.apply),
            yes=bool(args.yes),
            before_state_path=_before_state_path_from_output(out.last),
        )
        ...
        _finalize_run_artifacts(
            ...
            tool="gtm-api-tool",
            version=__version__,
            command="gtm-api-tool " + " ".join(argv),
            env_fingerprint=None,
            output_obj=out.last if isinstance(out.last, dict) else None,
            audit_log_path=run_audit_log_path or global_audit_log_path,
            audit_log_global_path=global_audit_log_path,
