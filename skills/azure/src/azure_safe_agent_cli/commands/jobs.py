...
def _demo_write(row: dict[str, str], ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    raise SafetyError("Refused: jobs are dry-run/read-only in the Azure CLI; use generated Azure commands for planned writes")
...
def cmd_jobs_run(args: Any, ctx: dict[str, Any]) -> int:
    ...
    # Jobs are kept as a local planning helper. Real Azure writes stay under generated named commands.
    if bool(ctx.get("apply")) and _has_write_actions(rows):
        out = {
            "ok": True,
            "dry_run": False,
            "refused": True,
            "reasons": ["Refused: jobs are dry-run/read-only in the Azure CLI; use generated Azure commands with --plan-in --apply --yes for writes"],
            "refusal_type": "SafetyError",
            "count": 0,
            "errors": 0,
            "results": [],
