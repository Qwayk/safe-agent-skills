...
def cmd_demo_write(args: Any, ctx: dict[str, Any]) -> int:
    ...
    if not plan_in or not bool(ctx.get("yes")):
        out = {
            "ok": True,
            "dry_run": False,
            "refused": True,
            "reasons": ["Refused: demo write apply is disabled for Azure source safety; use generated Azure commands with --plan-in --apply --yes for writes"],
