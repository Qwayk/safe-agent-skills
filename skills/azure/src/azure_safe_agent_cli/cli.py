...
def _cmd_inventory_summary(args: argparse.Namespace, ctx: dict) -> int:
    _ = args, ctx
    ctx["out"].emit(inventory_summary())
    return 0


def _register_inventory_command(sub: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    inventory = sub.add_parser("inventory", help="Generated Azure REST API inventory")
    inv_sub = inventory.add_subparsers(dest="inventory_cmd", required=True, parser_class=_ToolArgumentParser)
    summary = inv_sub.add_parser("summary", help="Show pinned Azure inventory summary")
...
def _register_generated_services(sub: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    try:
        ...
        return
    reserved = {"auth", "demo", "inventory", "jobs", "onboarding", "runs"}
    services = sorted(registry.data.get("services", []), key=lambda item: str(item.get("service_id") or ""))
    for service in services:
        service_id = str(service.get("service_id") or "").strip()
        if not service_id or service_id in reserved:
            continue
        operations = [op for op in service.get("operations", []) if str(op.get("operation_name") or "").strip()]
        if not operations:
            continue
        parser = sub.add_parser(service_id, help=f"Azure {service.get('service') or service_id} ({len(operations)} operations)")
        parser.add_argument(
            "operation",
            choices=[str(op.get("operation_name") or "").strip() for op in operations],
            metavar="OPERATION",
            help="Pinned generated Azure operation name",
        )
        parser.add_argument("--input-json", required=True, help="Path to operation input JSON")
        parser.add_argument("--ack-no-snapshot", action="store_true", default=argparse.SUPPRESS, help="Acknowledge no Azure snapshot is available")
        parser.add_argument("--output-file", default=argparse.SUPPRESS, help="Write safe command output JSON to a file")
        parser.set_defaults(func=execute_generated_operation, write_capable=True)
...
def build_parser() -> argparse.ArgumentParser:
    p = _ToolArgumentParser(prog="qwayk-azure-safe-agent-cli")
    ...
    jobs = sub.add_parser("jobs", help="Local CSV job helper")
    jobs_sub = jobs.add_subparsers(dest="jobs_cmd", required=True, parser_class=_ToolArgumentParser)
    jobs_run = jobs_sub.add_parser("run", help="Run a CSV job file (read-only/dry-run helper)")
    ...
    demo = sub.add_parser("demo", help="Local demo commands")
    ...
    demo_write = demo_sub.add_parser("write", help="Create a local dry-run demo plan")
...
def main(argv: list[str]) -> int:
    ...
    try:
        if bool(args.version):
            payload = {"ok": True, "tool": "qwayk-azure-safe-agent-cli", "version": __version__}
            if args.output == "json":
                out.emit(payload)
            else:
                print(f"qwayk-azure-safe-agent-cli {__version__}")
            return 0

        if not getattr(args, "cmd", None):
            parser.error("Missing command. Use --help to see available commands.")

        command_str = "qwayk-azure-safe-agent-cli " + " ".join(argv)
        audit.bind_context(
            {
                "tool": "qwayk-azure-safe-agent-cli",
                "version": __version__,
                "command": command_str,
        ...
        env_fingerprint = cfg.management_endpoint
        ...
        _finalize_run_artifacts(
            ...
            tool="qwayk-azure-safe-agent-cli",
            version=__version__,
            command="qwayk-azure-safe-agent-cli " + " ".join(argv),
            env_fingerprint=None,
            output_obj=out.last if isinstance(out.last, dict) else None,
            audit_log_path=run_audit_log_path or global_audit_log_path,
            audit_log_global_path=global_audit_log_path,
