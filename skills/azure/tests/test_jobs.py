...
from azure_safe_agent_cli.commands.jobs import cmd_jobs_run
from azure_safe_agent_cli.output import Output
...
class TestJobs(unittest.TestCase):
    def _ctx(self, **overrides):
        ctx = {
            ...
            "tool_version": "0.0.0",
            "command_str": "qwayk-azure-safe-agent-cli jobs run",
            "apply": False,
            "yes": False,
    ...
    def test_write_action_apply_is_refused_even_with_yes(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            ctx = self._ctx(apply=True, yes=True, command_str="qwayk-azure-safe-agent-cli --apply --yes jobs run")
            buf = io.StringIO()
            ...
            self.assertIn("generated Azure commands", payload["reasons"][0])
    ...
    def test_plan_out_and_plan_in_write_apply_is_refused(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            ctx_apply = self._ctx(
                ...
                command_str="qwayk-azure-safe-agent-cli --apply --yes --plan-in plan.json --file jobs.csv jobs run",
            )
            buf2 = io.StringIO()
            ...
            self.assertIn("generated Azure commands", payload2["reasons"][0])
