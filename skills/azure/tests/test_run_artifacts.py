...
class TestRunArtifacts(unittest.TestCase):
    def test_demo_write_creates_run_folder_and_index(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            env_path.write_text(
                "\n".join(
                    [
                        "AZURE_MANAGEMENT_ENDPOINT=http://example.invalid",
                        "AZURE_TIMEOUT_S=30",
                    ]
                )
                + "\n",
    ...
    def test_runs_list_and_show_work(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            env_path = root / ".env"
            env_path.write_text("AZURE_MANAGEMENT_ENDPOINT=http://example.invalid\nAZURE_TIMEOUT_S=30\n", encoding="utf-8")
    ...
    def test_demo_write_apply_is_refused(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            env_path = root / ".env"
            env_path.write_text("AZURE_MANAGEMENT_ENDPOINT=http://example.invalid\nAZURE_TIMEOUT_S=30\n", encoding="utf-8")
            ...
            with redirect_stdout(io.StringIO()):
                _ = main(["--env-file", str(env_path), "--plan-out", str(plan_path), "demo", "write", "--selector", "x"])
            ...
            self.assertIn("demo write apply is disabled", payload["reasons"][0])
