...
def build_parser() -> argparse.ArgumentParser:
    p = _ToolArgumentParser(prog="dynadot-api-tool")
    ...
    register_all(sub, parser_class=_ToolArgumentParser)
...
def main(argv: list[str]) -> int:
    ...
    try:
        if bool(args.version):
            payload = {"ok": True, "tool": "dynadot-api-tool", "version": __version__}
            if args.output == "json":
                out.emit(payload)
            else:
                print(f"dynadot-api-tool {__version__}")
            return 0

        if not getattr(args, "cmd", None):
            parser.error("Missing command. Use --help to see available commands.")

        command_str = "dynadot-api-tool " + " ".join(argv)
        audit.bind_context(
            {
        ...
        _finalize_run_artifacts(
            ...
            tool="dynadot-api-tool",
            version=__version__,
            command="dynadot-api-tool " + " ".join(argv),
            env_fingerprint=None,
            output_obj=out.last if isinstance(out.last, dict) else None,
            audit_log_path=run_audit_log_path or global_audit_log_path,
            audit_log_global_path=global_audit_log_path,
