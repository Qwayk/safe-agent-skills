...
from dynadot_api_tool.cli import main
...
class TestRunArtifacts(unittest.TestCase):
    def test_demo_write_creates_run_folder_and_index(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            env_path.write_text(
                "\n".join(
                    [
                        "DYNADOT_API_BASE_URL=http://example.invalid",
    ...
    def test_runs_list_only_returns_dynadot_rows(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            env_path = root / ".env"
            env_path.write_text(
            ...
            self.assertEqual(payload["runs"][0]["run_id"], "dy1")
