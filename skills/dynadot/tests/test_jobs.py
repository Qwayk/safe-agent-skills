...
class TestJobs(unittest.TestCase):
    def _ctx(self, **overrides):
        ctx = {
            ...
            "tool_version": "0.0.0",
            "command_str": "dynadot-api-tool jobs run",
            "apply": False,
            "yes": False,
    ...
    def test_write_action_requires_apply_yes(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            ctx = self._ctx(apply=True, yes=False, command_str="dynadot-api-tool --apply jobs run")
            buf = io.StringIO()
    ...
    def test_write_action_apply_requires_plan_in(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            path = Path(d) / "jobs.csv"
            with path.open("w", encoding="utf-8", newline="") as f:
                w = csv.writer(f)
            ...
            args = SimpleNamespace(file=str(path), limit=None)
            ctx = self._ctx(apply=True, yes=True, command_str="dynadot-api-tool --apply --yes jobs run")
            buf = io.StringIO()
            with redirect_stdout(buf):
                rc = cmd_jobs_run(args, ctx)
            self.assertEqual(rc, 0)
    ...
    def test_plan_out_and_plan_in_apply_refuses_write_actions_without_before_state(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            ctx_apply = self._ctx(
                ...
                command_str="dynadot-api-tool --apply --yes --plan-in plan.json --file jobs.csv jobs run",
            )
            buf2 = io.StringIO()
