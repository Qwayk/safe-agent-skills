...
Hacker News coverage shows exactly what the shipped commands can do with public stories, comments, users, and item changes. Start here when an ask sounds possible but you need to know whether it is already shipped, read-only, plan-first, gated, excluded, or outside the tool.
...
A good first coverage check is: "Check whether the shipped commands can fetch top stories, open the first five items, and show why story IDs alone are not enough."
