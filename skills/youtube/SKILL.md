...
description: Run the YouTube Data API v3 Qwayk CLI safely (dry-run first; no-snapshot approval for writes without saved before-state).
---
...
- For non-GET API requests: require `--apply --yes`, then expect the current tool to need required approval before provider writes when no before-state can be saved.
- For delete methods: require `--apply --yes --ack-irreversible`.
- For media uploads: require `--apply --yes`, ensure the plan never embeds file bytes (path-only), and expect refusal before upload endpoint use.
...
2) If OAuth is needed and missing:
   - `auth login` currently plans/needs required approval before writing a token, so report this as blocked unless an existing token file is already present.
3) For any non-GET or upload request:
   - Run the dry-run plan first (no `--apply`).
   - Summarize the plan in plain English.
   - Ask for explicit approval before re-running with `--apply --yes` (and `--ack-irreversible` for deletes), then verify the receipt or exact refusal reason.
...
- `youtube-api-tool --output json --apply --yes api playlists.insert --params-json '{"part":"snippet,status"}' --body-json '{...}'`

Delete (irreversible-gated):
- `youtube-api-tool --output json api playlists.delete --params-json '{"id":"<PLAYLIST_ID>"}'`
- `youtube-api-tool --output json --apply --yes --ack-irreversible api playlists.delete --params-json '{"id":"<PLAYLIST_ID>"}'`
...
- `youtube-api-tool --output json --apply --yes api videos.insert --params-json '{"part":"snippet,status"}' --body-json '{...}' --upload-file "<VIDEO_PATH>"`
