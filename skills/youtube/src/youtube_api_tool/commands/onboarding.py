...
def cmd_onboarding(args: object, ctx: dict) -> int:
    ...
    missing: list[str] = []
    if not _has_nonempty_env_value(lines, "YOUTUBE_OAUTH_CLIENT_SECRETS_FILE"):
        missing.append("YOUTUBE_OAUTH_CLIENT_SECRETS_FILE")
    ...
    steps = [
        ...
        "Optional (read-only public calls):",
        "  - YOUTUBE_API_KEY=<api key>",
        "Run: youtube-api-tool --output json auth check",
    ]
    ...
    payload = {
        ...
        "onboarding": {
            ...
            "missing": missing,
            "next_command": "youtube-api-tool --output json auth check",
            "steps": steps,
        },
    }
    ...
    if str(getattr(args, "output", "json")) == "json":
        ...
        print("")
        print("Next: youtube-api-tool --output json auth check")
    return 0
