...
from ..api_call import build_no_recovery_contract
...
def _demo_write(row: dict[str, str], ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "dry_run": False,
        "refused": True,
        "reasons": ["Refused: jobs write.ping is template-only and has no live YouTube write executor."],
    }
...
def cmd_jobs_run(args: Any, ctx: dict[str, Any]) -> int:
    ...
    receipt = {
        ...
        "verification": {"ok": errors == 0, "details": {"type": "read-only jobs", "notes": "No write-row verification ran."}},
