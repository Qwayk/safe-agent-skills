...
WordPress coverage shows exactly what the shipped commands can do with posts, pages, media, users, comments, settings, and plugin-related checks. Start here when an ask sounds possible but you need to know whether it is already shipped, read-only, plan-first, gated, excluded, or outside the tool.
...
A good first coverage check is: "Check whether the shipped commands can inspect posts, pages, media, and comments, then show which WordPress changes are covered."
