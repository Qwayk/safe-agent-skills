...
class TestReadmePublicContract(unittest.TestCase):
    def test_public_readme_opening_stays_user_facing(self) -> None:
        ...
        self.assertTrue(text.startswith("# WordPress\n"))
        self.assertNotIn("## Simplicity lock", text)
        self.assertNotIn("# wordpress-api-tool", text)
        self.assertIn("**Capability:** Reads + careful changes", text)
        self.assertIn("WordPress sites usually have years", text)
        for stale_phrase in [
            "Use this skill when",
    ...
    def test_start_here_section_stays_human_facing(self) -> None:
        ...
        self.assertIn("[What you can do with WordPress](docs/use_cases.md)", text)
        self.assertIn("[Connect your WordPress site](docs/onboarding.md)", text)
    ...
    def test_helpful_docs_stay_human_facing(self) -> None:
        ...
        self.assertIn("[Browse all WordPress docs](docs/README.md)", text)
    ...
    def test_quickstart_opens_with_human_links(self) -> None:
        ...
        self.assertIn("[What you can do with WordPress](use_cases.md)", text)
        self.assertIn("[Connect your WordPress site](onboarding.md)", text)
    ...
    def test_use_cases_stay_specific_and_human(self) -> None:
        ...
        self.assertIn("WordPress work is usually about cleaning up", text)
