...
from .commands import hubspot as hubspot_cmd
...
def build_parser() -> argparse.ArgumentParser:
    p = _ToolArgumentParser(prog="qwayk-hubspot-safe-agent-cli")
    ...
    hubspot_cmd.register_commands(sub)
...
def main(argv: list[str]) -> int:
    ...
    try:
        if bool(args.version):
            payload = {"ok": True, "tool": "qwayk-hubspot-safe-agent-cli", "version": __version__}
            if args.output == "json":
                out.emit(payload)
            else:
                print(f"qwayk-hubspot-safe-agent-cli {__version__}")
            return 0

        if not getattr(args, "cmd", None):
            parser.error("Missing command. Use --help to see available commands.")

        command_str = "qwayk-hubspot-safe-agent-cli " + " ".join(argv)
        audit.bind_context(
            {
                "tool": "qwayk-hubspot-safe-agent-cli",
                "version": __version__,
                "command": command_str,
        ...
        _finalize_run_artifacts(
            ...
            tool="qwayk-hubspot-safe-agent-cli",
            version=__version__,
            command="qwayk-hubspot-safe-agent-cli " + " ".join(argv),
            env_fingerprint=None,
            output_obj=out.last if isinstance(out.last, dict) else None,
            audit_log_path=run_audit_log_path or global_audit_log_path,
            audit_log_global_path=global_audit_log_path,
