...
class TestReadmePublicContract(unittest.TestCase):
    def test_public_readme_opening_stays_user_facing(self) -> None:
        ...
        self.assertTrue(text.startswith("# PayPal\n"))
        self.assertIn("**Capability:** Reads + careful changes", text)
        self.assertIn("PayPal is where orders, captures, refunds", text)

        stale_phrases = [
            "Use this skill when",
            "For non-technical users",
            "For technical users",
            "with clearer preview before live changes",
            "safety-first CLI",
        ]
    ...
    def test_links_stay_human_facing(self) -> None:
        ...
        self.assertIn("[What you can do with PayPal](docs/use_cases.md)", text)
        self.assertIn("[Connect your PayPal account](docs/onboarding.md)", text)
        ...
        self.assertIn("[Browse all PayPal docs](docs/README.md)", text)
