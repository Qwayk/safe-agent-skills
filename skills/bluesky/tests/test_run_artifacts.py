...
class TestRunArtifacts(unittest.TestCase):
    def _write_sample_env(self, path: Path) -> None:
        path.write_text(
            "\n".join(
                [
                    "BLUESKY_TIMEOUT_S=30",
                ]
            )
            + "\n",
            encoding="utf-8",
    ...
    def test_auth_token_set_creates_run_folder_and_index(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            env_path = root / ".env"
            self._write_sample_env(env_path)
            token_source = root / "token_input.json"
            token_source.write_text('{"accessJwt":"abc","refreshJwt":"def"}\n', encoding="utf-8")
            ...
            with redirect_stdout(buf):
                rc = main(
                    [
                        ...
                        "--apply",
                        "api",
                        "com-atproto-admin-update-account-handle",
                        "--body-json",
                        '{"account":"alice.example","handle":"alice.new"}',
                    ]
                )
            self.assertEqual(rc, 0)
            payload = json.loads(buf.getvalue())
    ...
    def test_invalid_run_id_is_rejected_as_json_error(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            env_path = root / ".env"
            self._write_sample_env(env_path)
