...
class TestOnboardingCommand(unittest.TestCase):
    def test_onboarding_creates_env_from_example(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            with open(example_path, "w", encoding="utf-8") as f:
                f.write("BLUESKY_IDENTIFIER=alice.bsky.social\n")
                f.write("BLUESKY_APP_PASSWORD=paste_app_password_here\n")
    ...
    def test_onboarding_creates_missing_parent_directory(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = os.path.join(td, "nested", ".env")
