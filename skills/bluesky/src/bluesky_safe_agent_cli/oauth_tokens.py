...
def get_token_status(path: Path) -> TokenStatus:
    ...
    has_refresh_token = None
    if "refresh_token" in data or "refreshJwt" in data:
        has_refresh_token = bool(data.get("refresh_token") or data.get("refreshJwt"))
...
def redact_token_dict(data: dict[str, Any]) -> dict[str, Any]:
    ...
    for k, v in data.items():
        lk = str(k).lower()
        if lk in {"access_token", "refresh_token", "accessjwt", "refreshjwt", "id_token", "client_secret", "token"} or lk.endswith("_token"):
            out[k] = "***REDACTED***"
        else:
            out[k] = v
    return out
