...
def _normalize_run_id(run_id: str | None) -> str:
    ...
    if Path(rid).is_absolute():
        raise ValidationError("Invalid --run-id: absolute paths are not allowed")
    if "/" in rid or "\\" in rid:
        raise ValidationError("Invalid --run-id: path separators are not allowed")
    if rid in {".", ".."}:
        raise ValidationError("Invalid --run-id")
    if not _RUN_ID_RE.fullmatch(rid):
        raise ValidationError("Invalid --run-id: use only letters, numbers, dot, dash, or underscore")
    return rid
