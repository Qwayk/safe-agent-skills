...
def cmd_onboarding(args: object, ctx: dict) -> int:
    ...
    missing: list[str] = []
    if not _has_nonempty_env_value(lines, "CALLRAIL_API_BASE_URL"):
        missing.append("CALLRAIL_API_BASE_URL")
    if not _has_nonempty_env_value(lines, "CALLRAIL_API_TOKEN"):
        missing.append("CALLRAIL_API_TOKEN")

    steps = [
        "Copy .env.example → .env (local-only; do not commit it).",
        "Open .env and fill the required values:",
        "  - CALLRAIL_API_BASE_URL=https://api.callrail.com",
        "  - CALLRAIL_API_TOKEN=<your token>",
        "Run: qwayk-callrail-safe-agent-cli --output json auth check",
    ]
    ...
    payload = {
        ...
        "onboarding": {
            ...
            "missing": missing,
            "next_command": "qwayk-callrail-safe-agent-cli --output json auth check",
            "steps": steps,
        },
    }
    ...
    if str(getattr(args, "output", "json")) == "json":
        ...
        print("")
        print("Next: qwayk-callrail-safe-agent-cli --output json auth check")
    return 0
