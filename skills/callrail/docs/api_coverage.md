...
CallRail coverage shows exactly what the shipped commands can do with calls, forms, companies, trackers, messages, and account settings. Start here when an ask sounds possible but you need to know whether it is already shipped, read-only, plan-first, gated, excluded, or outside the tool.
...
A good first coverage check is: "Check whether the shipped commands can list recent calls and forms, inspect one tracker, and show which account changes are available."
