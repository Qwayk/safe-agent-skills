...
Google Search Console coverage shows exactly what the shipped commands can do with sites, search analytics, sitemaps, URL inspection, and ownership data. Start here when an ask sounds possible but you need to know whether it is already shipped, read-only, plan-first, gated, excluded, or outside the tool.
...
A good first coverage check is: "Check whether the shipped commands can run search analytics, inspect this URL, and show which Search Console endpoints are included."
