...
def cmd_onboarding(args: object, ctx: dict) -> int:
    ...
    missing: list[str] = []
    if not _has_nonempty_env_value(lines, "N8N_BASE_URL"):
        missing.append("N8N_BASE_URL")
    if not _has_nonempty_env_value(lines, "N8N_API_KEY"):
    ...
    steps = [
        ...
        "Run: n8n-safe-agent-cli --output json auth check",
    ]
    ...
    payload = {
        ...
        "onboarding": {
            ...
            "missing": missing,
            "next_command": "n8n-safe-agent-cli --output json auth check",
            "steps": steps,
        },
    }
    ...
    if str(getattr(args, "output", "json")) == "json":
        ...
        print("")
        print("Next: n8n-safe-agent-cli --output json auth check")
    return 0
