...
def build_parser() -> argparse.ArgumentParser:
    p = _ToolArgumentParser(prog="n8n-safe-agent-cli")
    ...
    api = sub.add_parser("api", help="Official n8n public API operations")
    api_sub = api.add_subparsers(dest="api_family", required=True, parser_class=_ToolArgumentParser)
    api_list = api_sub.add_parser("list", help="List official n8n operation commands")
...
def main(argv: list[str]) -> int:
    ...
    try:
        if bool(args.version):
            payload = {"ok": True, "tool": "n8n-safe-agent-cli", "version": __version__}
            if args.output == "json":
                out.emit(payload)
            else:
                print(f"n8n-safe-agent-cli {__version__}")
            return 0
        ...
        local_only = str(getattr(args, "cmd", "") or "") in {"runs", "onboarding"} or (
