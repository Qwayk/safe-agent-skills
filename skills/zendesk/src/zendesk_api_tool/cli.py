...
from . import commands_api as api_cmd
...
def build_parser() -> argparse.ArgumentParser:
    p = _ToolArgumentParser(prog="zendesk-api-tool")
    ...
    auth_check.add_argument("--live", action="store_true", help="Actually call Zendesk (default is offline)")
    ...
    api_cmd.register_api_commands(sub)
...
def main(argv: list[str]) -> int:
    ...
    try:
        if bool(args.version):
            payload = {"ok": True, "tool": "zendesk-api-tool", "version": __version__}
            if args.output == "json":
                out.emit(payload)
            else:
                print(f"zendesk-api-tool {__version__}")
            return 0

        if not getattr(args, "cmd", None):
            parser.error("Missing command. Use --help to see available commands.")

        command_str = "zendesk-api-tool " + " ".join(argv)
        audit.bind_context(
            {
        ...
        _finalize_run_artifacts(
            ...
            tool="zendesk-api-tool",
            version=__version__,
            command="zendesk-api-tool " + " ".join(argv),
            env_fingerprint=None,
            output_obj=out.last if isinstance(out.last, dict) else None,
            audit_log_path=run_audit_log_path or global_audit_log_path,
            audit_log_global_path=global_audit_log_path,
