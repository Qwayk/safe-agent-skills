...
def _before_state_contract() -> dict[str, Any]:
    return {
        ...
        "status": "true_blocker",
        "statement": (
            "This jobs command only has a stub write action and cannot execute real Zendesk write rows. "
            "Use the explicit `api` command for live Zendesk writes."
        ),
    }
...
def _write_refusal_message() -> str:
    return (
        "Refused: this jobs command cannot execute real Zendesk write rows. "
        "Use the explicit `api` command for live Zendesk writes."
    )
...
def _build_jobs_plan(*, job_file: str | None, rows: list[dict[str, Any]], ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "before_state": _before_state_contract() if _has_write_actions(rows) else {"required": False, "supported": False},
...
def cmd_jobs_run(args: Any, ctx: dict[str, Any]) -> int:
    ...
    if bool(ctx.get("apply")) and _has_write_actions(rows):
        out = {
            ...
            "dry_run": False,
            "refused": True,
            "reasons": [_write_refusal_message()],
            "refusal_type": "SafetyError",
            "count": 0,
            "errors": 0,
            "results": [],
    ...
    if "audit" in ctx:
        ctx["audit"].write("jobs.apply", {"count": processed, "errors": errors})
    ctx["out"].emit(out)
    return 1 if errors else 0
