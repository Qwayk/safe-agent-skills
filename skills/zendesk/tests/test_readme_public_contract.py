...
class TestReadmePublicContract(unittest.TestCase):
    def test_public_readme_opening_stays_user_facing(self) -> None:
        ...
        self.assertTrue(text.startswith("# Zendesk\n"))
        self.assertNotIn("## Simplicity lock", text)
        self.assertNotIn("# zendesk-api-tool", text)
        self.assertIn("**Capability:** Reads + careful changes", text)
        self.assertIn("Zendesk is where support tickets", text)
    ...
    def test_helpful_docs_stay_human_facing(self) -> None:
        ...
        self.assertIn("[Browse all Zendesk docs](docs/README.md)", text)
    ...
    def test_start_here_section_stays_human_facing(self) -> None:
        ...
        self.assertIn("[What you can do with Zendesk](docs/use_cases.md)", text)
        self.assertIn("[Connect your Zendesk account](docs/onboarding.md)", text)
        self.assertIn("[How this skill stays safe](docs/safety_model.md)", text)
        self.assertIn("[Quickstart](docs/quickstart.md)", text)
        self.assertIn("[Command guide](docs/command_reference.md)", text)

    def test_use_cases_stay_zendesk_specific(self) -> None:
        root = Path(__file__).resolve().parents[1]
        text = (root / "docs" / "use_cases.md").read_text(encoding="utf-8")

        self.assertIn("sensitive customer details", text)
        self.assertIn("Count the tickets", text)
        self.assertIn("pinned command inventory", text)
        self.assertNotIn("Most no-code automation", text)

    def test_quickstart_opens_with_human_links(self) -> None:
        root = Path(__file__).resolve().parents[1]
        text = (root / "docs" / "quickstart.md").read_text(encoding="utf-8")

        self.assertIn("[What you can do with Zendesk](use_cases.md)", text)
        self.assertIn("[Connect your Zendesk account](onboarding.md)", text)
