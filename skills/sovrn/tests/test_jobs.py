...
from sovrn_safe_agent_cli.commands.jobs import cmd_jobs_run
from sovrn_safe_agent_cli.output import Output


class TestJobs(unittest.TestCase):
    def _ctx(self, **overrides):
        ctx = {
            "cfg": SimpleNamespace(env_fingerprint="commerce_secret=0|commerce_site_key=0|advertising_key=0|advertising_publisher=0"),
            "tool": "sovrn-safe-cli",
            "tool_version": "0.0.0",
            "command_str": "sovrn-safe-cli jobs run",
            "apply": False,
            "yes": False,
    ...
    def test_write_action_requires_apply_yes(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            ctx = self._ctx(apply=True, yes=False, command_str="sovrn-safe-cli --apply jobs run")
            buf = io.StringIO()
    ...
    def test_plan_out_and_plan_in_apply(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            ctx_apply = self._ctx(
                ...
                command_str="sovrn-safe-cli --apply --yes --plan-in plan.json --file jobs.csv jobs run",
            )
            buf2 = io.StringIO()
