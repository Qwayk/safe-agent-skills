...
Sovrn coverage shows exactly what the shipped commands can do with publisher commerce, advertising reports, merchants, links, and performance data. Start here when an ask sounds possible but you need to know whether it is already shipped, read-only, plan-first, gated, excluded, or outside the tool.
...
A good first coverage check is: "Check whether the shipped commands can review merchants, links, and performance data, then show which publisher workflows are included."
