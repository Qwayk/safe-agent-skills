...
from .commands import advertising as advertising_cmd
from .commands import auth as auth_cmd
from .commands import commerce as commerce_cmd
...
def build_parser() -> argparse.ArgumentParser:
    p = _ToolArgumentParser(prog="sovrn-safe-cli")
    ...
    auth_check = auth_sub.add_parser("check", help="Check local auth/config readiness")
    auth_check.set_defaults(func=auth_cmd.cmd_auth_check, write_capable=False)

    commerce_cmd.register_commerce_parser(sub)
    advertising_cmd.register_advertising_parser(sub)
...
def main(argv: list[str]) -> int:
    ...
    try:
        if bool(args.version):
            payload = {"ok": True, "tool": "sovrn-safe-cli", "version": __version__}
            if args.output == "json":
                out.emit(payload)
            else:
                print(f"sovrn-safe-cli {__version__}")
            return 0

        if not getattr(args, "cmd", None):
            parser.error("Missing command. Use --help to see available commands.")

        command_str = "sovrn-safe-cli " + " ".join(argv)
        audit.bind_context(
            {
                "tool": "sovrn-safe-cli",
                "version": __version__,
        ...
        _finalize_run_artifacts(
            ...
            tool="sovrn-safe-cli",
            version=__version__,
            command="sovrn-safe-cli " + " ".join(argv),
            env_fingerprint=None,
            output_obj=out.last if isinstance(out.last, dict) else None,
            audit_log_path=run_audit_log_path or global_audit_log_path,
            audit_log_global_path=global_audit_log_path,
