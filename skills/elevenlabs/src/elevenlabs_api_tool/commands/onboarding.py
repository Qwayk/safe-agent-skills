...
def cmd_onboarding(args: object, ctx: dict) -> int:
    ...
    missing: list[str] = []
    if not _has_nonempty_env_value(lines, "ELEVENLABS_API_BASE_URL"):
        missing.append("ELEVENLABS_API_BASE_URL")
    if not _has_nonempty_env_value(lines, "ELEVENLABS_API_KEY"):
        missing.append("ELEVENLABS_API_KEY")
    if not _has_nonempty_env_value(lines, "ELEVENLABS_TIMEOUT_S"):
        missing.append("ELEVENLABS_TIMEOUT_S")

    steps = [
        "Copy .env.example → .env (local-only; do not commit it).",
        "Open .env and fill the required values:",
        "  - ELEVENLABS_API_BASE_URL=https://api.elevenlabs.io",
        "  - ELEVENLABS_API_KEY=<your key from https://elevenlabs.io/app/settings/api-keys>",
        "  - ELEVENLABS_TIMEOUT_S=30",
        "Run: elevenlabs-api-tool --output json auth check",
    ]
    ...
    payload = {
        ...
        "onboarding": {
            ...
            "missing": missing,
            "next_command": "elevenlabs-api-tool --output json auth check",
            "steps": steps,
        },
    }
    ...
    if str(getattr(args, "output", "json")) == "json":
        ...
        print("")
        print("Next: elevenlabs-api-tool --output json auth check")
    return 0
