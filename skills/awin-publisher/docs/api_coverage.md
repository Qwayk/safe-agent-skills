...
Awin Publisher coverage shows exactly what the shipped commands can do with publisher reporting, link building, feeds, and proof-of-purchase workflows. Start here when an ask sounds possible but you need to know whether it is already shipped, read-only, plan-first, gated, excluded, or outside the tool.
...
A good first coverage check is: "Check whether the shipped commands can report publisher performance, build tracking links, and show which publisher workflows are not shipped."
