...
from jobber_safe_agent_cli.oauth_tokens import (
    get_token_status,
...
class TestOAuthTokens(unittest.TestCase):
    ...
    def test_read_stored_access_token_prefers_token_file(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            token_file = root / ".state" / "token.json"
            token_file.parent.mkdir(parents=True, exist_ok=True)
            token_file.write_text(json.dumps({"access_token": "stored-token", "refresh_token": "stored-refresh"}), encoding="utf-8")

            token = read_stored_access_token(env_file=str(root / ".env"), fallback_token="env-token")
