...
from jobber_safe_agent_cli.schema_registry import mutation_fields


class TestRunArtifacts(unittest.TestCase):
    def _first_mutation(self) -> str:
        return str(mutation_fields()[0].name)

    def test_write_command_creates_run_folder_and_index(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            mutation = self._first_mutation()

            buf = io.StringIO()
            with redirect_stdout(buf):
                rc = main(["--env-file", str(env_path), "--run-id", run_id, "write", mutation])
    ...
    def test_runs_list_and_show_work(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            buf = io.StringIO()
            with redirect_stdout(buf):
                _ = main(["--env-file", str(env_path), "--run-id", run_id, "write", mutation])
    ...
    def test_refusal_still_creates_run_history(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            job_csv.write_text(
                "action,args_json,selection\n"
                'write.clientCreate,"{""input"":{""firstName"":""Sample"",""lastName"":""Client""}}",client { id name }\n',
                encoding="utf-8",
            )
