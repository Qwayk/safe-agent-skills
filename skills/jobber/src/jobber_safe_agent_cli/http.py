...
class GraphQLError(ToolError):
    """Raised when GraphQL API returns a response-level error."""
...
class GraphQLClient:
    def __init__(
        ...
        *,
        ...
        timeout_s: float,
        verbose: bool,
        user_agent: str = "qwayk-jobber-safe-agent-cli",
    ) -> None:
        self._http = HttpClient(timeout_s=timeout_s, verbose=verbose, user_agent=user_agent)
        self._endpoint = endpoint
        self._token = token
        self._graphql_version = graphql_version

    def execute(self, query: str, *, variables: dict[str, Any] | None = None) -> dict[str, Any]:
        headers = {
            "Content-Type": "application/json",
            "X-JOBBER-GRAPHQL-VERSION": self._graphql_version,
        }
        if self._token:
        ...
        response = self._http.request("POST", self._endpoint, headers=headers, json_body=payload)
        try:
            body = response.json()
        except Exception as e:  # noqa: BLE001
            raise GraphQLError(f"GraphQL response was not valid JSON: {type(e).__name__}") from None

        if not isinstance(body, dict):
            raise GraphQLError("GraphQL response was not a JSON object")

        if body.get("errors"):
            raise GraphQLError(f"GraphQL API returned errors: {body.get('errors')}")
