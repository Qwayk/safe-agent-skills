...
from .schema_registry import mutation_fields, query_fields
...
def build_parser() -> argparse.ArgumentParser:
    p = _ToolArgumentParser(prog="qwayk-jobber-safe-agent-cli")
    ...
    p.add_argument(
        "--ack-no-snapshot",
        action="store_true",
        help="Confirm no useful before-state snapshot exists for this mutation",
    )
    ...
    auth_authorize_url = auth_sub.add_parser("authorize-url", help="Build OAuth authorize URL")
    auth_authorize_url.add_argument("--scope", default=None, help="OAuth scope list")
    ...
    token_refresh = token_sub.add_parser("refresh", help="Exchange refresh token for a new access token")
    token_refresh.add_argument("--refresh-token", default=None, dest="refresh_token", help="Refresh token to use")
    token_refresh.set_defaults(func=auth_cmd.cmd_auth_token_refresh, write_capable=True)

    # Backward-compatible alias.
    auth_token_refresh = auth_sub.add_parser(
        "token-refresh",
        help="Exchange refresh token for a new access token",
    )
    auth_token_refresh.add_argument("--refresh-token", default=None, dest="refresh_token", help="Refresh token to use")
    auth_token_refresh.set_defaults(func=auth_cmd.cmd_auth_token_refresh, write_capable=True)

    schema = sub.add_parser("schema", help="Operation registry and topic metadata")
    schema_sub = schema.add_subparsers(dest="schema_cmd", required=True, parser_class=_ToolArgumentParser)
    schema_summary = schema_sub.add_parser("summary", help="Show schema inventory summary")
    schema_summary.set_defaults(func=schema_cmd.cmd_schema_summary, write_capable=False)
    schema_queries = schema_sub.add_parser("queries", help="List query fields in registry")
    schema_queries.set_defaults(func=schema_cmd.cmd_schema_queries, write_capable=False)
    schema_mutations = schema_sub.add_parser("mutations", help="List mutation fields in registry")
    schema_mutations.set_defaults(func=schema_cmd.cmd_schema_mutations, write_capable=False)
    schema_topics = schema_sub.add_parser("webhook-topics", help="List webhook topics in registry")
    schema_topics.set_defaults(func=schema_cmd.cmd_schema_webhook_topics, write_capable=False)

    read = sub.add_parser("read", help="Named read operations from Jobber Query registry")
    read_sub = read.add_subparsers(dest="read_cmd", required=True, parser_class=_ToolArgumentParser)

    def _add_read_write_shared_args(p2: argparse.ArgumentParser) -> None:
        p2.add_argument("--args-json", default=None, help="JSON args object")
        p2.add_argument("--args-file", default=None, help="Path to JSON args object")
        p2.add_argument("--selection", default=None, help="GraphQL selection set (optional)")
        p2.add_argument("--selection-file", default=None, help="File with GraphQL selection set")
        p2.add_argument("--limit", type=int, default=None, help="Limit arg if operation supports paging")

    for field in query_fields():
        cmd = read_sub.add_parser(field.name, help=field.description or f"Query {field.name}")
        _add_read_write_shared_args(cmd)
        cmd.set_defaults(func=read_cmd.cmd_read_operation, write_capable=False, schema_operation=field)

    write = sub.add_parser("write", help="Named write operations from Jobber Mutation registry")
    write_sub = write.add_subparsers(dest="write_cmd", required=True, parser_class=_ToolArgumentParser)
    for field in mutation_fields():
        cmd = write_sub.add_parser(field.name, help=field.description or f"Mutation {field.name}")
        _add_read_write_shared_args(cmd)
        cmd.set_defaults(func=write_cmd.cmd_write_operation, write_capable=True, schema_operation=field)

    webhooks = sub.add_parser("webhooks", help="Webhook helpers")
    webhooks_sub = webhooks.add_subparsers(dest="webhooks_cmd", required=True, parser_class=_ToolArgumentParser)
    webhook_topics = webhooks_sub.add_parser("topics", help="List supported webhook topics")
    webhook_topics.set_defaults(func=webhooks_cmd.cmd_webhooks_topics, write_capable=False)
    webhook_verify = webhooks_sub.add_parser("verify-signature", help="Verify X-Jobber-Hmac-SHA256 signature")
    webhook_verify.add_argument("--body", default=None, help="Raw webhook body")
    webhook_verify.add_argument("--body-file", default=None, help="File containing raw webhook body")
    webhook_verify.add_argument("--header", default=None, help="X-Jobber-Hmac-SHA256 header value")
    webhook_verify.add_argument("--secret", default=None, help="Override JOBBER_CLIENT_SECRET for this verification")
    webhook_verify.set_defaults(func=webhooks_cmd.cmd_webhooks_verify_signature, write_capable=False)
    ...
    jobs_run = jobs_sub.add_parser("run", help="Run a CSV job file")
...
def main(argv: list[str]) -> int:
    ...
    try:
        if bool(args.version):
            payload = {"ok": True, "tool": "qwayk-jobber-safe-agent-cli", "version": __version__}
            if args.output == "json":
                out.emit(payload)
            else:
                print(f"qwayk-jobber-safe-agent-cli {__version__}")
            return 0

        if not getattr(args, "cmd", None):
            parser.error("Missing command. Use --help to see available commands.")

        command_str = "qwayk-jobber-safe-agent-cli " + " ".join(argv)
        audit.bind_context(
            {
        ...
        _finalize_run_artifacts(
            ...
            tool="qwayk-jobber-safe-agent-cli",
            version=__version__,
            command="qwayk-jobber-safe-agent-cli " + " ".join(argv),
            env_fingerprint=None,
            output_obj=out.last if isinstance(out.last, dict) else None,
            audit_log_path=run_audit_log_path or global_audit_log_path,
            audit_log_global_path=global_audit_log_path,
