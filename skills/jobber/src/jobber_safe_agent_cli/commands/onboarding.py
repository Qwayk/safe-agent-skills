...
def cmd_onboarding(args: object, ctx: dict) -> int:
    ...
    missing: list[str] = []
    if not _has_nonempty_env_value(lines, "JOBBER_API_BASE_URL"):
        missing.append("JOBBER_API_BASE_URL")
    if not _has_nonempty_env_value(lines, "JOBBER_API_TOKEN"):
        missing.append("JOBBER_API_TOKEN")
    ...
    steps = [
        ...
        "  - JOBBER_API_TOKEN=<your token>",
        "Run: qwayk-jobber-safe-agent-cli --output json auth check",
    ]
    ...
    payload = {
        ...
        "onboarding": {
            ...
            "missing": missing,
            "next_command": "qwayk-jobber-safe-agent-cli --output json auth check",
            "steps": steps,
        },
    }
    ...
    if str(getattr(args, "output", "json")) == "json":
        ...
        print("")
        print("Next: qwayk-jobber-safe-agent-cli --output json auth check")
    return 0
