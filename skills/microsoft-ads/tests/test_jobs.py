...
from msads_api_tool.commands.jobs import cmd_jobs_run
from msads_api_tool.output import Output


class TestJobs(unittest.TestCase):
    def _ctx(self, **overrides):
        ctx = {
            "cfg": SimpleNamespace(env_fingerprint="msads:prod"),
            "tool": "msads-api-tool",
            "tool_version": "0.0.0",
            "command_str": "msads-api-tool jobs run",
            "apply": False,
            "yes": False,
    ...
    def test_write_action_requires_apply_yes(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            ctx = self._ctx(apply=True, yes=False, command_str="msads-api-tool --apply jobs run")
            buf = io.StringIO()
    ...
    def test_plan_out_and_plan_in_apply_refuses_before_write(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            ctx_apply = self._ctx(
                ...
                command_str="msads-api-tool --apply --yes --plan-in plan.json --file jobs.csv jobs run",
            )
            buf2 = io.StringIO()
            ...
            self.assertIn("template-only", " ".join(payload2["reasons"]))
