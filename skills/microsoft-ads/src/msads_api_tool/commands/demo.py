...
def cmd_demo_read(args: Any, ctx: dict[str, Any]) -> int:
    _ = args
    out = {"ok": True, "message": "demo read (safe)", "env": ctx["cfg"].env_fingerprint}
...
def _build_demo_plan(*, selector: str, ctx: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "before_state": {
            ...
            "supported": False,
            "status": "true_blocker",
            "statement": "Demo write has no real Microsoft Ads provider write executor.",
        },
        "proposed_changes": [{"field": "demo_field", "from": "<unknown>", "to": "new_value"}],
        "verification_plan": {
            "type": "true_blocker",
            "notes": "Apply attempts refuse because this demo command has no provider write executor.",
        },
...
def cmd_demo_write(args: Any, ctx: dict[str, Any]) -> int:
    ...
    out = {
        ...
        "refused": True,
        "reasons": [
            "Refused: Microsoft Ads demo write has no real provider write executor. "
