...
def _demo_write(row: dict[str, str], ctx: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "plan": {
                ...
                "before_state": {
                    ...
                    "status": "true_blocker",
                    "statement": (
                        "This jobs template has no real Microsoft Ads provider write executor for write.ping. "
                        "Use the operation command for real provider writes."
                    ),
                },
            },
        }
    return {
        ...
        "refused": True,
        "reasons": [
            "Refused: Microsoft Ads jobs write.ping is only a template action and has no real provider write executor. "
            "Use the operation command for real provider writes."
        ],
    }
...
def _build_jobs_plan(*, job_file: str | None, rows: list[dict[str, Any]], ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "tool": ctx.get("tool") or "msads-api-tool",
        ...
        "before_state": {
            "required": False,
            "supported": False,
            "status": "true_blocker" if risk_level == "high" else "not_required",
            "statement": (
                "Jobs write actions in this template have no real Microsoft Ads provider write executor."
                if risk_level == "high"
                else "Read-only jobs do not require before-state capture."
            ),
        },
        "proposed_changes": [
        ...
        "verification_plan": {
            "type": "true_blocker" if risk_level == "high" else "per-row-read",
            "notes": (
                "Apply attempts with write actions refuse because this template has no provider write executor."
                if risk_level == "high"
                else "Read-only job actions can run without write verification."
            ),
        },
...
def cmd_jobs_run(args: Any, ctx: dict[str, Any]) -> int:
    ...
    if bool(ctx.get("apply")) and _has_write_actions(rows):
        out = {
            ...
            "refused": True,
            "reasons": [
                "Refused: Microsoft Ads jobs write actions are template-only in this command. "
                "Use the operation command for real provider writes."
            ],
            "refusal_type": "SafetyError",
            "count": 0,
