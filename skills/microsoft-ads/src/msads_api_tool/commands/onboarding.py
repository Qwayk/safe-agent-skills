...
def cmd_onboarding(args: object, ctx: dict) -> int:
    ...
    missing: list[str] = []
    # `MSADS_DEVELOPER_TOKEN` is required for live calls; we still allow onboarding to succeed without it.
    if not _has_nonempty_env_value(lines, "MSADS_DEVELOPER_TOKEN"):
        missing.append("MSADS_DEVELOPER_TOKEN")

    steps = [
        "Copy .env.example → .env (local-only; do not commit it).",
        "Open .env and fill the required values (never paste them into chat):",
        "  - MSADS_ENVIRONMENT=prod  (or sandbox)",
        "  - MSADS_DEVELOPER_TOKEN=...  (from Microsoft Ads)",
        "Store your OAuth token JSON locally (refresh token included):",
    ...
    payload = {
        ...
        "onboarding": {
            ...
            "missing": missing,
            "next_command": "msads-api-tool --output json --live auth check",
            "steps": steps,
        },
    }
    ...
    if str(getattr(args, "output", "json")) == "json":
        ...
        print("")
        print("Next: msads-api-tool --output json --live auth check")
    return 0
