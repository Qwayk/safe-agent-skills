...
from .commands import operations as ops_cmd
from .config import load_config
from .project_config import load_project_config
from .errors import SafetyError, ToolError, ValidationError
from .output import Output
from .operations_v13 import OPERATIONS_BY_SERVICE
from .runs import (
    RunContext,
...
def build_parser() -> argparse.ArgumentParser:
    p = _ToolArgumentParser(prog="msads-api-tool")
    ...
    p.add_argument("--live", action="store_true", help="Allow network requests (required for live API calls)")
    ...
    token_show = token_sub.add_parser("show", help="Show stored token (redacted)")
    token_show.set_defaults(func=auth_cmd.cmd_auth_token_show, write_capable=False)
    ...
    token_refresh = token_sub.add_parser("refresh", help="Refresh access token using stored refresh_token")
    token_refresh.set_defaults(func=auth_cmd.cmd_auth_token_refresh, write_capable=True)

    # Microsoft Ads v13 service operations (explicit command surface; generated from audited inventory).
    for service, operations in OPERATIONS_BY_SERVICE.items():
        svc = sub.add_parser(service, help=f"Microsoft Ads v13: {service} service operations")
        svc_sub = svc.add_subparsers(dest=f"{service}_cmd", required=True, parser_class=_ToolArgumentParser)
        for operation in operations:
            op_kebab = ops_cmd.pascal_to_kebab(operation)
            op_parser = svc_sub.add_parser(op_kebab, help=operation)
            op_parser.add_argument(
                "--request-json",
                default=None,
                help="Optional JSON object file with operation request fields",
            )
            risk = ops_cmd.classify_operation_risk(service=service, operation=operation)
            op_parser.set_defaults(
                func=ops_cmd.cmd_msads_operation,
                write_capable=bool(risk.requires_apply),
                service=service,
                operation=operation,
            )
...
def main(argv: list[str]) -> int:
    ...
    try:
        if bool(args.version):
            payload = {"ok": True, "tool": "msads-api-tool", "version": __version__}
            if args.output == "json":
                out.emit(payload)
            else:
                print(f"msads-api-tool {__version__}")
            return 0

        if not getattr(args, "cmd", None):
            parser.error("Missing command. Use --help to see available commands.")

        command_str = "msads-api-tool " + " ".join(argv)
        audit.bind_context(
            {
                "tool": "msads-api-tool",
                "version": __version__,
        ...
        _finalize_run_artifacts(
            ...
            tool="msads-api-tool",
            version=__version__,
            command="msads-api-tool " + " ".join(argv),
            env_fingerprint=None,
            output_obj=out.last if isinstance(out.last, dict) else None,
            audit_log_path=run_audit_log_path or global_audit_log_path,
            audit_log_global_path=global_audit_log_path,
