...
```bash
msads-api-tool jobs run --file jobs.csv
```
...
- Any write action requires `--apply --yes`, then requires explicit no-snapshot approval before any write-like result is emitted when no useful before-state can be saved.
...
2) Review the plan (human/Codex), then apply from the saved plan after approval:
...
- `write.ping` (requires `--apply --yes` and explicit no-snapshot approval when no useful before-state can be saved)
...
If you want to include a write action, run with `--apply --yes` after review, then check the receipt or exact blocker:

```bash
msads-api-tool --apply --yes jobs run --file examples/jobs_with_write.csv
```
