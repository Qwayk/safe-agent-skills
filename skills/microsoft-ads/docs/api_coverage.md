...
Microsoft Ads coverage shows exactly what the shipped commands can do with accounts, campaigns, budgets, ads, keywords, audiences, and reports. Start here when an ask sounds possible but you need to know whether it is already shipped, read-only, plan-first, gated, excluded, or outside the tool.
...
A good first coverage check is: "Check whether the shipped commands can inspect campaigns, budgets, and keywords, then show which Microsoft Ads operations are shipped."
