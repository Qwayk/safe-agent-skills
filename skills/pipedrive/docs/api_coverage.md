...
Pipedrive coverage shows exactly what the shipped commands can do with CRM deals, leads, activities, people, organizations, products, and pipelines. Start here when an ask sounds possible but you need to know whether it is already shipped, read-only, plan-first, gated, excluded, or outside the tool.
...
A good first coverage check is: "Check whether the shipped commands can list open deals, inspect one person or organization, and show which Pipedrive writes are intentionally excluded."
