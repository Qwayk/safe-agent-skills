...
from .commands import discovery as discovery_cmd
...
def build_parser() -> argparse.ArgumentParser:
    p = _ToolArgumentParser(prog="google-merchant-api-tool")
    ...
    p.add_argument("--yes", action="store_true", help="Additional confirmation for higher-risk writes")
    p.add_argument(
        "--ack-no-snapshot",
        action="store_true",
        help="Acknowledge that this approved write may run without a saved before-state snapshot",
    )
    p.add_argument("--plan-in", default=None, help="Apply from a previously generated plan JSON file")
    ...
    discovery_cmd.register_discovery_commands(sub)
...
def main(argv: list[str]) -> int:
    ...
    try:
        if bool(args.version):
            payload = {"ok": True, "tool": "google-merchant-api-tool", "version": __version__}
            if args.output == "json":
                out.emit(payload)
            else:
                print(f"google-merchant-api-tool {__version__}")
            return 0

        if not getattr(args, "cmd", None):
            parser.error("Missing command. Use --help to see available commands.")

        command_str = "google-merchant-api-tool " + " ".join(argv)
        audit.bind_context(
            {
        ...
        _finalize_run_artifacts(
            ...
            tool="google-merchant-api-tool",
            version=__version__,
            command="google-merchant-api-tool " + " ".join(argv),
            env_fingerprint=None,
            output_obj=out.last if isinstance(out.last, dict) else None,
            audit_log_path=run_audit_log_path or global_audit_log_path,
            audit_log_global_path=global_audit_log_path,
