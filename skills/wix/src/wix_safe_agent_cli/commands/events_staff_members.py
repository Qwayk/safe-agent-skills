...
COMMAND_FAMILY = "events-staff-members"
BASE_PATH = "/events/v1/staff-members"
...
def _require_staff_member_revision(payload: dict[str, Any]) -> None:
    staff_member = payload.get("staffMember")
    if not isinstance(staff_member, dict):
        raise ValidationError("--staff-member-json must include a staffMember object")
    revision = staff_member.get("revision")
    if revision is None or str(revision).strip() == "":
        raise ValidationError("--staff-member-json for update must include staffMember.revision")
...
def _build_plan(
    *,
    ...
    return {
        ...
        "state_capture": {"before_state_available": False, "notes": "Events Staff Members plans do not capture a full before-state snapshot in this slice."},
        "proposed_changes": proposed_changes,
        "verification_plan": {"type": "provider-response", "notes": verification_notes},
        "rollback": {"supported": False, "notes": "No automatic rollback. Verify with staff member reads and the Wix dashboard when needed."},
    }
...
def _run_write(
    *,
    ...
    verification_notes: str = "Provider response confirms the staff member request was accepted.",
) -> int:
    auth = _resolve_auth(ctx)
    request: dict[str, Any] = {"method": http_method, "path": path}
    if body is not None:
    ...
    plan = _build_plan(
        ...
        risk_reasons=risk_reasons or ["wix-events-staff-members-write"],
...
def cmd_events_staff_members_create(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.create"
    try:
        body = _read_json_arg(getattr(args, "staff_member_json", None), field="staff-member-json")
        return _run_write(method_name=method, http_method="POST", path=BASE_PATH, body=body, selector={"kind": COMMAND_FAMILY, "operation": "create"}, proposed_changes=[{"operation": "create-event-staff-member"}], ctx=ctx)
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_events_staff_members_get(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.get"
    try:
        staff_member_id = _coerce_text(getattr(args, "staff_member_id", None), field="staff-member-id")
        return _run_read(method_name=method, http_method="GET", path=f"{BASE_PATH}/{staff_member_id}", ctx=ctx)
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_events_staff_members_update(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.update"
    try:
        staff_member_id = _coerce_text(getattr(args, "staff_member_id", None), field="staff-member-id")
        body = _read_json_arg(getattr(args, "staff_member_json", None), field="staff-member-json")
        _require_staff_member_revision(body)
        return _run_write(method_name=method, http_method="PATCH", path=f"{BASE_PATH}/{staff_member_id}", body=body, selector={"kind": COMMAND_FAMILY, "staff_member_id": staff_member_id}, proposed_changes=[{"operation": "update-event-staff-member", "staff_member_id": staff_member_id}], ctx=ctx)
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_events_staff_members_delete(args, ctx) -> int:
    ...
    try:
        ...
        return _run_write(
            ...
            body=None,
            selector={"kind": COMMAND_FAMILY, "staff_member_id": staff_member_id},
            proposed_changes=[{"operation": "delete-event-staff-member", "staff_member_id": staff_member_id}],
            ctx=ctx,
            requires_ack=True,
            risk_reasons=["wix-events-staff-members-delete", "permanent-staff-member-removal"],
            verification_notes="Provider response confirms the permanent staff member deletion request was accepted.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_events_staff_members_query(args, ctx) -> int:
