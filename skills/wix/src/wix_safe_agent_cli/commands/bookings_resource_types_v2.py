...
COMMAND_FAMILY = "bookings-resource-types-v2"
BASE_PATH = "/bookings/v2/resources/resource-types"
...
def _normalize_resource_type_body(raw: Any, *, resource_type_id: str | None = None) -> dict[str, Any]:
    payload = _coerce_json_object(raw, field="resource-type-json")
    body = dict(payload) if "resourceType" in payload else {"resourceType": payload}
    resource_type = body.get("resourceType")
    if not isinstance(resource_type, dict) or not resource_type:
        raise ValidationError("--resource-type-json must include a non-empty resourceType object")
    if resource_type_id is not None:
        payload_id = resource_type.get("id")
        if payload_id is not None and str(payload_id).strip() != resource_type_id:
            raise SafetyError("Refused: resource type id in body does not match --resource-type-id")
        resource_type.setdefault("id", resource_type_id)
        if not str(resource_type.get("revision") or "").strip():
            raise ValidationError("--resource-type-json resourceType.revision is required for update")
    return body
...
def _build_plan(
    *,
    ...
    return {
        ...
        "method": method,
        "risk_level": "high",
        "risk_reasons": ["wix-bookings-resource-type-write"],
        ...
        "state_capture": {
            ...
            "notes": "No useful before-state snapshot is captured for this Resource Types V2 provider-response write.",
        },
        "proposed_changes": proposed_changes,
        "verification_plan": {
            "type": "provider-response",
            "notes": "Provider-response-only in this boundary. Run follow-up resource type reads, queries, or counts for live verification.",
        },
        "rollback": {"supported": False, "notes": "No automatic rollback. Recovery is manual and may require new reviewed resource type plans."},
    }
...
def _build_receipt(
    *,
    ...
    return {
        ...
        "state_capture": {
            "before_state_available": False,
            "notes": "No useful before-state snapshot was available for this Resource Types V2 write.",
        },
...
def cmd_bookings_resource_types_v2_get(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.get"
    try:
        resource_type_id = _coerce_non_empty_text(getattr(args, "resource_type_id", None), field="resource-type-id")
        return _run_read(method_name=method, http_method="GET", path=f"{BASE_PATH}/{resource_type_id}", body=None, ctx=ctx)
    except (ValidationError, RuntimeError) as exc:
        return _write_error(ctx, method=method, exc=exc)


def cmd_bookings_resource_types_v2_query(args, ctx) -> int:
...
def cmd_bookings_resource_types_v2_count(args, ctx) -> int:
...
def cmd_bookings_resource_types_v2_create(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.create"
    try:
        body = _normalize_resource_type_body(getattr(args, "resource_type_json", None))
        return _run_write(
            method_name=method,
            http_method="POST",
            path=BASE_PATH,
            body=body,
            selector={"kind": "bookings-resource-type-v2", "operation": "create"},
...
def cmd_bookings_resource_types_v2_update(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.update"
    try:
        resource_type_id = _coerce_non_empty_text(getattr(args, "resource_type_id", None), field="resource-type-id")
        body = _normalize_resource_type_body(getattr(args, "resource_type_json", None), resource_type_id=resource_type_id)
        return _run_write(
            method_name=method,
            http_method="PATCH",
            path=f"{BASE_PATH}/{resource_type_id}",
            body=body,
            selector={"kind": "bookings-resource-type-v2", "operation": "update", "resource_type_id": resource_type_id},
            proposed_changes=[{"operation": "update", "resource_type_id": resource_type_id, "body": body}],
            ctx=ctx,
        )
    except (SafetyError, ValidationError, RuntimeError) as exc:
        return _write_error(ctx, method=method, exc=exc)
...
def cmd_bookings_resource_types_v2_delete(args, ctx) -> int:
    ...
    try:
        ...
        return _run_write(
            ...
            path=f"{BASE_PATH}/{resource_type_id}",
            body=None,
            selector={"kind": "bookings-resource-type-v2", "operation": "delete", "resource_type_id": resource_type_id},
            proposed_changes=[{"operation": "delete", "resource_type_id": resource_type_id}],
            ctx=ctx,
            requires_ack=True,
        )
    except (SafetyError, ValidationError, RuntimeError) as exc:
