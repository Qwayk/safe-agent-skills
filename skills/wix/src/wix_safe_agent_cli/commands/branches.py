...
def _coerce_branch_id(raw: Any) -> str:
    if raw is None:
        raise ValidationError("Missing --branch-id")
    if not isinstance(raw, str):
        raise ValidationError("--branch-id must be a string")
...
def cmd_branches_get_default(args, ctx) -> int:
    _ = args
    try:
        auth = resolve_auth_mode(
            ...
            command_family="branches",
        )
        request_path = "/branches/v1/branches/default"
...
def cmd_branches_get(args, ctx) -> int:
    try:
        branch_id = _coerce_branch_id(getattr(args, "branch_id", None))
        auth = resolve_auth_mode(
            cfg=ctx["cfg"],
            env_file=str(ctx["env_file"]),
            verbose=bool(ctx.get("verbose")),
        ...
        request_path = f"/branches/v1/branches/{branch_id}"
        payload = _request_json(
            method="GET",
            base_url=ctx["cfg"].base_url,
            path=request_path,
...
def cmd_branches_query(args, ctx) -> int:
    try:
        ...
        request_path = "/branches/v1/branches/query"
        payload = _request_json(
            method="POST",
            base_url=ctx["cfg"].base_url,
            path=request_path,
