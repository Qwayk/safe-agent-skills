...
COMMAND_FAMILY = "member-reports"
BASE_PATH = "/members/v1/member-reports"
...
def _run_read(*, method_name: str, http_method: str, path: str, body: dict[str, Any], ctx: dict[str, Any]) -> int:
    ...
    *,
    ...
    return {
        ...
        "state_capture": {
            ...
            "notes": "Member Reports plans do not capture a full before-state snapshot in this slice.",
        },
        "proposed_changes": proposed_changes,
        "verification_plan": {"type": "provider-response", "notes": verification_notes},
        "rollback": {"supported": False, "notes": "No automatic rollback. Verify with member-reports query when needed."},
    }
...
def cmd_member_reports_query(args, ctx) -> int:
...
def cmd_member_reports_report(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.report"
    try:
        body = _read_json_arg(getattr(args, "report_json", None), field="report-json")
        return _run_write(
            method_name=method,
            http_method="POST",
            path=BASE_PATH,
            body=body,
            selector={"kind": COMMAND_FAMILY, "operation": "report"},
            proposed_changes=[{"operation": "report-member", "body": body}],
            ctx=ctx,
            requires_ack=False,
            risk_reasons=["member-report-create", "site-owner-moderation-notification"],
            verification_notes="Provider response confirms the Report Member request was accepted.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_member_reports_delete(args, ctx) -> int:
    ...
    try:
        ...
        return _run_write(
            method_name=method,
            http_method="DELETE",
            path=f"{BASE_PATH}/members/{member_id}",
            body=None,
            selector={"kind": COMMAND_FAMILY, "member_id": member_id, "operation": "delete"},
            proposed_changes=[{"operation": "delete-member-reports", "member_id": member_id}],
            ctx=ctx,
            requires_ack=True,
            risk_reasons=["member-report-delete", "deletes-all-reports-for-member"],
            verification_notes="Provider response confirms the Delete Member Reports request was accepted.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)
