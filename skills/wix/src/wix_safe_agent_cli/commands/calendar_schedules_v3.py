...
COMMAND_FAMILY = "calendar-schedules-v3"
BASE_PATH = "/calendar/v3/schedules"
...
def _normalize_schedule_body(raw: Any, *, schedule_id: str | None = None, require_revision: bool = False) -> dict[str, Any]:
    payload = _coerce_json_object(raw, field="schedule-json")
    body = dict(payload) if "schedule" in payload else {"schedule": payload}
    schedule = body.get("schedule")
    if not isinstance(schedule, dict) or not schedule:
        raise ValidationError("--schedule-json must include a non-empty schedule object")
    if schedule_id is not None:
        payload_id = schedule.get("id")
        if payload_id is not None and str(payload_id).strip() != schedule_id:
            raise SafetyError("Refused: schedule id in body does not match --schedule-id")
        schedule.setdefault("id", schedule_id)
    if require_revision and not str(schedule.get("revision") or "").strip():
        raise ValidationError("--schedule-json schedule.revision is required for update")
    return body
...
def _normalize_cancel_body(raw: Any) -> dict[str, Any]:
    if raw is None:
        return {}
    return _coerce_json_object(raw, field="request-json", allow_empty=True)
...
def _build_plan(
    *,
    ...
    return {
        ...
        "method": method,
        "risk_level": "high",
        "risk_reasons": ["wix-calendar-schedule-write"],
        ...
        "state_capture": {
            ...
            "notes": "No useful before-state snapshot is captured for this Calendar Schedules V3 provider-response write.",
        },
        "proposed_changes": proposed_changes,
        "verification_plan": {
            "type": "provider-response",
            "notes": "Provider-response-only in this boundary. Run get or query to verify live schedule state after apply.",
        },
        "rollback": {"supported": False, "notes": "No automatic rollback. Recovery is manual and may require new reviewed schedule plans."},
    }
...
def _build_receipt(
    *,
    ...
    return {
        ...
        "state_capture": {
            "before_state_available": False,
            "notes": "No useful before-state snapshot was available for this Calendar Schedules V3 write.",
        },
...
def cmd_calendar_schedules_v3_get(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.get"
    try:
        schedule_id = _coerce_non_empty_text(getattr(args, "schedule_id", None), field="schedule-id")
        return _run_read(method_name=method, http_method="GET", path=f"{BASE_PATH}/{schedule_id}", body=None, ctx=ctx)
    except (ValidationError, RuntimeError) as exc:
        return _write_error(ctx, method=method, exc=exc)


def cmd_calendar_schedules_v3_query(args, ctx) -> int:
...
def cmd_calendar_schedules_v3_create(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.create"
    try:
        body = _normalize_schedule_body(getattr(args, "schedule_json", None))
        return _run_write(
            method_name=method,
            http_method="POST",
            path=BASE_PATH,
            body=body,
            selector={"kind": "calendar-schedule", "operation": "create"},
...
def cmd_calendar_schedules_v3_update(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.update"
    try:
        schedule_id = _coerce_non_empty_text(getattr(args, "schedule_id", None), field="schedule-id")
        body = _normalize_schedule_body(getattr(args, "schedule_json", None), schedule_id=schedule_id, require_revision=True)
        return _run_write(
            method_name=method,
            http_method="PATCH",
            path=f"{BASE_PATH}/{schedule_id}",
            body=body,
            selector={"kind": "calendar-schedule", "scheduleId": schedule_id, "operation": "update"},
            proposed_changes=[{"operation": "update", "scheduleId": schedule_id, "body": body}],
            ctx=ctx,
        )
    except (SafetyError, ValidationError, RuntimeError) as exc:
        return _write_error(ctx, method=method, exc=exc)
...
def cmd_calendar_schedules_v3_cancel(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.cancel"
    try:
        schedule_id = _coerce_non_empty_text(getattr(args, "schedule_id", None), field="schedule-id")
        body = _normalize_cancel_body(getattr(args, "request_json", None))
        return _run_write(
            method_name=method,
            http_method="POST",
            path=f"{BASE_PATH}/{schedule_id}/cancel",
            body=body,
            selector={"kind": "calendar-schedule", "scheduleId": schedule_id, "operation": "cancel"},
            proposed_changes=[{"operation": "cancel", "scheduleId": schedule_id, "body": body}],
            ctx=ctx,
            requires_ack=True,
        )
    except (SafetyError, ValidationError, RuntimeError) as exc:
