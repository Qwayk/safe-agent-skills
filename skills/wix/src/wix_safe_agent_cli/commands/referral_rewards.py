...
COMMAND_FAMILY = "referral-rewards"
BASE_PATH = "/_api/referral-rewards/v1/referral-rewards"
...
def cmd_referral_rewards_get(args, ctx) -> int:
    method = "referralRewards.getReferralReward"
    try:
        reward_id = _coerce_text(getattr(args, "referral_reward_id", None), field="referral-reward-id")
        return _read(method_name=method, http_method="GET", path=f"{BASE_PATH}/{reward_id}", body=None, ctx=ctx)
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_referral_rewards_query(args, ctx) -> int:
    method = "referralRewards.queryReferralRewards"
    try:
