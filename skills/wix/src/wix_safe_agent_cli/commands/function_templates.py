...
COMMAND_FAMILY = "function-templates"
BASE_PATH = "/api/functions/v1/templates"
...
def cmd_function_templates_get(args, ctx) -> int:
    method = "functionTemplates.getFunctionTemplate"
    try:
        app_def_id = _coerce_text(args.app_def_id, field="app-def-id")
        function_template_id = _coerce_text(args.function_template_id, field="function-template-id")
        path = f"{BASE_PATH}/{app_def_id}/{function_template_id}"
        return _run_read(method_name=method, http_method="GET", path=path, body=None, ctx=ctx)
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_function_templates_query(args, ctx) -> int:
    method = "functionTemplates.queryFunctionTemplates"
    try:
