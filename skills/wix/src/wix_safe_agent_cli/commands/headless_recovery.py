...
COMMAND_FAMILY = "headless-recovery"
SEND_RECOVERY_EMAIL_PATH = "/_api/iam/recovery/v1/send-email"
REDACTED = "[REDACTED]"
...
def _build_plan(*, method_name: str, request: dict[str, Any], selector: dict[str, Any], body: dict[str, Any], ctx: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "method": method_name,
        "risk_level": "high",
        "risk_reasons": ["headless-recovery-email", "sends-password-reset-link"],
        "preconditions": [
            ...
            "password-change redirect URL must be an allowed authorization redirect URI when supplied",
        ],
        "selector": selector,
        ...
        "state_capture": {
            ...
            "notes": "Recovery email plans do not capture a before-state snapshot because the action sends an email.",
        },
        "proposed_changes": [{"operation": "send-recovery-email", "body": _redact_sensitive(body)}],
        "verification_plan": {
            "type": "provider-response",
            "notes": "Provider response confirms Wix accepted the recovery email request. It does not prove inbox delivery or password reset completion.",
        },
        "rollback": {"supported": False, "notes": "No automatic rollback. The recovery email cannot be unsent."},
    }
...
def cmd_headless_recovery_send_recovery_email(args, ctx) -> int:
    method = "headlessRecovery.sendRecoveryEmail"
    try:
        body = _read_json_arg(getattr(args, "recovery_json", None), field="recovery-json")
        selector = {"kind": COMMAND_FAMILY, "operation": "send-recovery-email"}
        request = {"method": "POST", "path": SEND_RECOVERY_EMAIL_PATH, "body": body}
        ...
        response = _request_json(path=SEND_RECOVERY_EMAIL_PATH, headers=auth["headers"], json_body=body, ctx=ctx)
        receipt = {
            "tool": ctx.get("tool") or "wix-safe-agent-cli",
            "version": ctx.get("tool_version") or None,
            ...
            "verification": {
                "ok": True,
                "type": "provider-response",
                "notes": "Wix accepted the recovery email request. Inbox delivery and password reset completion are not proven.",
            },
