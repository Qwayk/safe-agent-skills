...
COMMAND_FAMILY = "referring-customers"
BASE_PATH = "/referral-customers/v1/referring-customers"
...
def _build_plan(
    *,
    ...
    return {
        ...
        "state_capture": {"before_state_available": False, "notes": "Referring Customers plans do not capture a before-state snapshot in this slice."},
        ...
        "rollback": {"supported": False, "notes": "No automatic rollback. Recreate or regenerate the referring customer manually if needed."},
    }
...
def cmd_referring_customers_get(args, ctx) -> int:
    method = "referringCustomers.getReferringCustomer"
    try:
        referring_customer_id = _coerce_text(getattr(args, "referring_customer_id", None), field="referring-customer-id")
        return _read(method_name=method, http_method="GET", path=f"{BASE_PATH}/{referring_customer_id}", body=None, ctx=ctx)
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_referring_customers_query(args, ctx) -> int:
    method = "referringCustomers.queryReferringCustomers"
    try:
...
def cmd_referring_customers_get_by_referral_code(args, ctx) -> int:
    method = "referringCustomers.getReferringCustomerByReferralCode"
    try:
        referral_code = _coerce_text(getattr(args, "referral_code", None), field="referral-code")
        return _read(method_name=method, http_method="GET", path=f"{BASE_PATH}/code/{referral_code}", body=None, ctx=ctx)
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_referring_customers_generate_for_contact(args, ctx) -> int:
    ...
    try:
        ...
        return _write(
            method_name=method,
            operation="generate-for-contact",
            http_method="POST",
            path=BASE_PATH,
            params=None,
            body={"contactId": contact_id},
            ctx=ctx,
            risk_reasons=["may-create-referring-customer-record", "can-trigger-referring-customer-created-event"],
            verification_notes="Provider response confirms Wix accepted the generate request. Rerun referring-customers get or get-by-referral-code to verify the record.",
            requires_ack=False,
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)
...
def cmd_referring_customers_delete(args, ctx) -> int:
    ...
    try:
        ...
        return _write(
            ...
            operation="delete",
            http_method="DELETE",
            path=f"{BASE_PATH}/{referring_customer_id}",
            params={"revision": revision},
            body=None,
            ctx=ctx,
            risk_reasons=["permanently-deletes-referring-customer", "requires-current-revision", "can-trigger-referring-customer-deleted-event"],
            verification_notes="Provider response confirms Wix accepted deletion. Rerun referring-customers get and expect the record to be absent.",
            requires_ack=True,
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)
