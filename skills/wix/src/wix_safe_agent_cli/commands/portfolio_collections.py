...
COMMAND_FAMILY = "portfolio-collections"
BASE_PATH = "/portfolio/v1/collections"
...
def _collection_object(payload: dict[str, Any]) -> dict[str, Any]:
...
def _revision_from_payload(payload: dict[str, Any]) -> str | None:
    revision = _collection_object(payload).get("revision")
    if revision is None:
        return None
...
def _body_with_id_and_current_revision(*, body: dict[str, Any], collection_id: str, current_revision: str) -> dict[str, Any]:
    updated = dict(body)
    collection = updated.get("collection")
    if isinstance(collection, dict):
        provided_id = collection.get("id")
        if provided_id is not None and str(provided_id).strip() != collection_id:
            raise SafetyError("Refused: provided collection.id does not match --collection-id")
        provided_revision = collection.get("revision")
        if provided_revision is not None and str(provided_revision).strip() != current_revision:
            raise SafetyError("Refused: provided collection.revision does not match current revision")
        revised_collection = dict(collection)
        revised_collection["id"] = collection_id
        revised_collection["revision"] = current_revision
        updated["collection"] = revised_collection
        return updated
...
def cmd_portfolio_collections_list(args, ctx) -> int:
...
def cmd_portfolio_collections_get(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.get"
    try:
        collection_id = _coerce_text(getattr(args, "collection_id", None), field="collection-id")
        params = _read_json_arg(getattr(args, "params_json", None), field="params-json", allow_empty=True)
        return _emit_read(method_name=method, http_method="GET", path=f"{BASE_PATH}/{collection_id}", params=params, body=None, ctx=ctx)
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_portfolio_collections_query(args, ctx) -> int:
...
def cmd_portfolio_collections_create(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.create"
    try:
        body = _read_json_arg(getattr(args, "collection_json", None), field="collection-json")
        return _emit_write(
            method_name=method,
            http_method="POST",
            path=BASE_PATH,
            ...
            proposed_changes=[{"operation": "create-collection"}],
            before_state=None,
            ctx=ctx,
            requires_ack=False,
            risk_reasons=["portfolio-collection-create"],
            verification_type="provider-response",
            verification_notes="Create is verified by Wix provider response; reread the returned collection id when needed.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_portfolio_collections_update(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.update"
    try:
        collection_id = _coerce_text(getattr(args, "collection_id", None), field="collection-id")
        body = _read_json_arg(getattr(args, "collection_json", None), field="collection-json")
        auth = _resolve_auth(ctx)
        path = f"{BASE_PATH}/{collection_id}"
        ...
        if not current_revision:
            raise SafetyError("Refused: current collection revision was not found")
        body = _body_with_id_and_current_revision(body=body, collection_id=collection_id, current_revision=current_revision)
        return _emit_write(
            method_name=method,
            http_method="PATCH",
            path=path,
            body=body,
            selector={"kind": COMMAND_FAMILY, "collection_id": collection_id},
            proposed_changes=[{"operation": "update-collection", "collection_id": collection_id}],
            before_state=before_state,
            ctx=ctx,
            requires_ack=False,
            risk_reasons=["portfolio-collection-update", "requires-current-revision"],
            verification_type="read-after-write",
            verification_notes="Verify by rereading the collection after update.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_portfolio_collections_delete(args, ctx) -> int:
    ...
    try:
        ...
        return _emit_write(
            ...
            body=None,
            selector={"kind": COMMAND_FAMILY, "collection_id": collection_id},
            proposed_changes=[{"operation": "delete-collection", "collection_id": collection_id}],
            before_state=before_state,
            ctx=ctx,
            requires_ack=True,
            risk_reasons=["portfolio-collection-delete", "irreversible-delete"],
            verification_type="provider-response",
