...
COMMAND_FAMILY = "function-types"
BASE_PATH = "/functions/v1/types"
...
def cmd_function_types_get(args, ctx) -> int:
    method = "functionTypes.getFunctionType"
    try:
        app_def_id = _coerce_text(args.app_def_id, field="app-def-id")
        function_type_id = _coerce_text(args.function_type_id, field="function-type-id")
        path = f"{BASE_PATH}/{app_def_id}/{function_type_id}"
...
def cmd_function_types_query(args, ctx) -> int:
    method = "functionTypes.queryFunctionTypes"
    try:
