...
COMMAND_FAMILY = "contact-extended-fields"
BASE_PATH = "/contacts/v4/extended-fields"
    ...
    *,
    ...
    return {
        ...
        "state_capture": {
            ...
            "notes": "Contact Extended Fields plans do not capture a full before-state snapshot in this slice.",
        },
        "proposed_changes": proposed_changes,
        "verification_plan": {"type": "provider-response", "notes": verification_notes},
        "rollback": {
            "supported": False,
            "notes": "No automatic rollback. Verify with contact-extended-fields get, list, or query when needed.",
        },
    }
...
def cmd_contact_extended_fields_get(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.get"
    try:
        key = _coerce_text(getattr(args, "key", None), field="key")
        return _run_read(method_name=method, http_method="GET", path=f"{BASE_PATH}/{key}", body=None, ctx=ctx)
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_contact_extended_fields_list(args, ctx) -> int:
...
def cmd_contact_extended_fields_query(args, ctx) -> int:
...
def cmd_contact_extended_fields_find_or_create(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.find-or-create"
    try:
        ...
        return _run_write(
            ...
            path=BASE_PATH,
            body=body,
            selector={"kind": COMMAND_FAMILY, "operation": "find-or-create"},
            proposed_changes=[{"operation": "find-or-create-contact-extended-field", "body": body}],
            ctx=ctx,
            requires_ack=False,
            risk_reasons=["contact-extended-field-find-or-create", "may-create-contact-field-definition"],
            verification_notes="Provider response confirms Wix returned an existing or newly created extended field.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_contact_extended_fields_update(args, ctx) -> int:
    ...
    try:
        ...
        return _run_write(
            ...
            body=body,
            selector={"kind": COMMAND_FAMILY, "key": key, "operation": "update"},
            proposed_changes=[{"operation": "update-contact-extended-field", "key": key, "body": body}],
            ctx=ctx,
            requires_ack=False,
            risk_reasons=["contact-extended-field-update", "renames-contact-field-definition"],
            verification_notes="Provider response confirms the Update Extended Field request was accepted.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_contact_extended_fields_delete(args, ctx) -> int:
    ...
    try:
        ...
        return _run_write(
            ...
            path=f"{BASE_PATH}/{key}",
            body=None,
            selector={"kind": COMMAND_FAMILY, "key": key, "operation": "delete"},
            proposed_changes=[{"operation": "delete-contact-extended-field", "key": key}],
            ctx=ctx,
            requires_ack=True,
            risk_reasons=["contact-extended-field-delete", "permanently-removes-contact-field-values"],
            verification_notes="Provider response confirms the Delete Extended Field request was accepted.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)
