...
COMMAND_FAMILY = "faq-category-v2"
BASE_PATH = "/faq/v2/categories"
...
def _resolve_auth(ctx: dict[str, Any]) -> dict[str, Any]:
    return resolve_auth_mode(
        ...
        command_family=str(ctx.get("command_family_override") or COMMAND_FAMILY),
    )
    ...
    *,
    ...
    return {
        ...
        "state_capture": {
            ...
            "notes": "FAQ Category V2 plans do not capture a full before-state snapshot in this slice.",
        },
...
def _category_body(raw: Any, *, field: str) -> tuple[dict[str, Any], dict[str, Any]]:
...
def _category_id_and_revision(category: dict[str, Any], *, field: str) -> str:
    category_id = _coerce_text(category.get("id"), field=f"{field} category.id")
    _coerce_text(category.get("revision"), field=f"{field} category.revision")
    return category_id


def cmd_faq_category_v2_list(args, ctx) -> int:
    method = "faqCategoryV2.listCategories"
    _ = args
    try:
...
def cmd_faq_category_v2_get(args, ctx) -> int:
    method = "faqCategoryV2.getCategory"
    try:
        category_id = _coerce_text(args.category_id, field="category-id")
        return _run_read(method_name=method, http_method="GET", path=f"{BASE_PATH}/{category_id}", body=None, ctx=ctx)
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_faq_category_v2_query(args, ctx) -> int:
    method = "faqCategoryV2.queryCategories"
    try:
...
def cmd_faq_category_v2_create(args, ctx) -> int:
    method = "faqCategoryV2.createCategory"
    try:
        body, category = _category_body(args.category_json, field="category-json")
        return _run_write(
            method_name=method,
            http_method="POST",
            path=BASE_PATH,
            ...
            proposed_changes=[{"operation": "create-category", "category": category}],
            ctx=ctx,
            requires_ack=False,
            risk_reasons=["wix-faq-category-create"],
            verification_notes="Inspect the provider response, then use faq-category-v2 get or query to verify the category.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_faq_category_v2_update(args, ctx) -> int:
    method = "faqCategoryV2.updateCategory"
    try:
        body, category = _category_body(args.category_json, field="category-json")
        category_id = _category_id_and_revision(category, field="category-json")
        return _run_write(
            method_name=method,
            http_method="PATCH",
            ...
            body=body,
            selector={"category": category},
            proposed_changes=[{"operation": "update-category", "category": category}],
            ctx=ctx,
            requires_ack=False,
            risk_reasons=["wix-faq-category-update", "requires-current-revision"],
            verification_notes="Inspect the provider response, then use faq-category-v2 get to verify the category.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_faq_category_v2_delete(args, ctx) -> int:
    method = "faqCategoryV2.deleteCategory"
    try:
        category_id = _coerce_text(args.category_id, field="category-id")
        return _run_write(
            method_name=method,
            ...
            path=f"{BASE_PATH}/{category_id}",
            body=None,
            selector={"categoryId": category_id},
            proposed_changes=[{"operation": "delete-category", "categoryId": category_id}],
            ctx=ctx,
            requires_ack=True,
            risk_reasons=["wix-faq-category-delete", "irreversible", "also-deletes-category-question-entries"],
            verification_notes="Inspect the provider response, then use faq-category-v2 get or query to confirm removal.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_faq_category_v2_update_extended_fields(args, ctx) -> int:
    method = "faqCategoryV2.updateExtendedFields"
    try:
        category_id = _coerce_text(args.category_id, field="category-id")
        ...
        return _run_write(
            method_name=method,
            http_method="POST",
            path=f"{BASE_PATH}/{category_id}/update-extended-fields",
            body=body,
            selector={"categoryId": category_id},
            proposed_changes=[{"operation": "update-category-extended-fields", "categoryId": category_id, "body": body}],
            ctx=ctx,
            requires_ack=False,
            risk_reasons=["wix-faq-category-update-extended-fields"],
            verification_notes="Inspect the provider response, then use faq-category-v2 get to verify extended fields.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)
