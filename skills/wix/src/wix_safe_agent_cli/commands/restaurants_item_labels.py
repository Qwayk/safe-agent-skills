...
COMMAND_FAMILY = "restaurants-item-labels"
BASE_PATH = "/restaurants/item-labels/v1"
...
def _build_plan(
    *,
    ...
    return {
        ...
        "state_capture": {"before_state_available": False, "notes": "Restaurants Item Labels plans do not capture a full before-state snapshot in this slice."},
        "proposed_changes": proposed_changes,
        "verification_plan": {"type": "provider-response", "notes": verification_notes},
        "rollback": {"supported": False, "notes": "No automatic rollback. Verify with restaurants-item-labels get/list and the Wix dashboard when needed."},
    }
...
def _run_write(
    *,
    ...
    verification_notes: str = "Provider response confirms the Restaurants Item Labels request was accepted.",
) -> int:
    auth = _resolve_auth(ctx)
    request: dict[str, Any] = {"method": http_method, "path": path}
    if body is not None:
    ...
    plan = _build_plan(
        ...
        risk_reasons=risk_reasons or ["wix-restaurants-item-label-write"],
...
def _labels_path(label_id: str | None = None) -> str:
    if label_id is None:
        return f"{BASE_PATH}/labels"
    return f"{BASE_PATH}/labels/{label_id}"


def _require_revision(body: dict[str, Any], *, field: str) -> None:
    label = body.get("label")
    if not isinstance(label, dict):
        raise ValidationError(f"--{field} must include a label object")
    if not str(label.get("revision") or "").strip():
        raise ValidationError(f"--{field} label.revision is required by Wix")


def cmd_restaurants_item_labels_list(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.list"
    try:
        params = _read_json_arg(getattr(args, "params_json", "{}"), field="params-json", allow_empty=True)
        return _run_read(method_name=method, http_method="GET", path=_labels_path(), params=params or None, body=None, ctx=ctx)
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_restaurants_item_labels_get(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.get"
    try:
        label_id = _coerce_text(getattr(args, "label_id", None), field="label-id")
        return _run_read(method_name=method, http_method="GET", path=_labels_path(label_id), params=None, body=None, ctx=ctx)
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_restaurants_item_labels_query(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.query"
    try:
        body = _read_json_arg(getattr(args, "query_json", "{}"), field="query-json", allow_empty=True)
        return _run_read(method_name=method, http_method="POST", path=f"{_labels_path()}/query", params=None, body=body, ctx=ctx)
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_restaurants_item_labels_create(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.create"
    try:
        body = _read_json_arg(getattr(args, "label_json", None), field="label-json")
        return _run_write(
            method_name=method,
            http_method="POST",
            path=_labels_path(),
            ...
            proposed_changes=[{"operation": "create-item-label", "body": body}],
            ctx=ctx,
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)
...
def cmd_restaurants_item_labels_update(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.update"
    try:
        label_id = _coerce_text(getattr(args, "label_id", None), field="label-id")
        body = _read_json_arg(getattr(args, "label_json", None), field="label-json")
        _require_revision(body, field="label-json")
        return _run_write(
            method_name=method,
            http_method="PATCH",
            path=_labels_path(label_id),
            body=body,
            selector={"kind": COMMAND_FAMILY, "label_id": label_id, "operation": "update"},
            proposed_changes=[{"operation": "update-item-label", "label_id": label_id, "body": body}],
            ctx=ctx,
            risk_reasons=["wix-restaurants-item-label-update", "requires-current-revision"],
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_restaurants_item_labels_delete(args, ctx) -> int:
    ...
    try:
        ...
        return _run_write(
            ...
            path=_labels_path(label_id),
            body=None,
            selector={"kind": COMMAND_FAMILY, "label_id": label_id, "operation": "delete"},
            proposed_changes=[{"operation": "delete-item-label", "label_id": label_id}],
            ctx=ctx,
            requires_ack=True,
            risk_reasons=["wix-restaurants-item-label-delete", "permanently-removes-item-label"],
            verification_notes="Provider response confirms the Restaurants Item Labels delete request was accepted; verify with restaurants-item-labels get/list.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)
