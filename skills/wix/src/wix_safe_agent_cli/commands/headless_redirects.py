...
COMMAND_FAMILY = "headless-redirects"
CREATE_REDIRECT_SESSION_PATH = "/_api/redirects-api/v1/redirect-session"
REDACTED = "[REDACTED]"
...
def _read_json_arg(raw: Any, *, field: str) -> dict[str, Any]:
    ...
    if not payload:
        raise ValidationError(f"--{field} cannot be empty")
    intent_keys = sorted(key for key in payload if key in _INTENT_KEYS)
    if len(intent_keys) != 1:
        expected = ", ".join(sorted(_INTENT_KEYS))
        raise ValidationError(f"--{field} must include exactly one redirect intent: {expected}")
    return payload
...
def _build_plan(*, method_name: str, request: dict[str, Any], selector: dict[str, Any], body: dict[str, Any], ctx: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "method": method_name,
        "risk_level": "high",
        "risk_reasons": ["headless-redirect-session", "creates-single-use-visitor-flow-url"],
        "preconditions": [
            "env_fingerprint must match",
            "selector must match",
            "apply requires --plan-in, --apply, and --yes",
            "apply also requires --ack-irreversible",
            "connected Wix site must be published",
            "callback domains and authorization redirect URIs must be allowed in Headless Settings",
        ],
        "selector": selector,
        ...
        "state_capture": {
            ...
            "notes": "Redirect session plans do not capture before-state because the action creates a new single-use redirect URL.",
        },
        "proposed_changes": [{"operation": "create-redirect-session", "body": _redact_sensitive(body)}],
        "verification_plan": {
            "type": "provider-response",
            "notes": "Provider response confirms Wix created a redirect session. It does not prove the visitor followed the returned URL or completed the hosted flow.",
        },
        "rollback": {"supported": False, "notes": "No automatic rollback. Do not send the generated URL to a visitor if the flow should be abandoned."},
    }
...
def cmd_headless_redirects_create_redirect_session(args, ctx) -> int:
    method = "headlessRedirects.createRedirectSession"
    try:
        body = _read_json_arg(getattr(args, "redirect_session_json", None), field="redirect-session-json")
        selector = {"kind": COMMAND_FAMILY, "operation": "create-redirect-session"}
        request = {"method": "POST", "path": CREATE_REDIRECT_SESSION_PATH, "body": body}
        auth = _resolve_auth(ctx)
        plan = _build_plan(method_name=method, request=request, selector=selector, body=body, ctx=ctx)
        if not reviewed_plan_apply_requested(ctx, requires_ack=True, command_label=method):
        ...
        response = _request_json(path=CREATE_REDIRECT_SESSION_PATH, headers=auth["headers"], json_body=body, ctx=ctx)
        receipt = {
            "tool": ctx.get("tool") or "wix-safe-agent-cli",
            "version": ctx.get("tool_version") or None,
            ...
            "verification": {
                "ok": True,
                "type": "provider-response",
                "notes": "Wix created a redirect session. Visitor redirection and hosted-flow completion are not proven.",
            },
