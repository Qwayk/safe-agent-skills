...
COMMAND_FAMILY = "function-methods"
BASE_PATH = "/functions/v1/methods"
...
def _method_body(raw: Any, *, field: str) -> tuple[dict[str, Any], dict[str, Any]]:
    payload = _read_json_arg(raw, field=field)
    body = dict(payload) if "functionMethod" in payload else {"functionMethod": payload}
    method = body.get("functionMethod")
    if not isinstance(method, dict) or not method:
        raise ValidationError(f"--{field} must include a non-empty functionMethod object")
    return body, method
    ...
    *,
    ...
    return {
        ...
        "state_capture": {
            ...
            "notes": "Function Methods plans do not capture a full before-state snapshot in this slice.",
        },
...
def cmd_function_methods_create(args, ctx) -> int:
    method = "functionMethods.createFunctionMethod"
    try:
        body, function_method = _method_body(args.function_method_json, field="function-method-json")
        return _run_write(
            method_name=method,
            http_method="POST",
            path=BASE_PATH,
            body=body,
            selector={"functionMethod": function_method},
            proposed_changes=[{"operation": "create-function-method", "functionMethod": function_method}],
            ctx=ctx,
            requires_ack=False,
            risk_reasons=["wix-function-method-create", "links-function-to-automation"],
            verification_notes="Inspect the provider response, then query function-methods by functionId or automationId to verify the link.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_function_methods_delete(args, ctx) -> int:
    method = "functionMethods.deleteFunctionMethod"
    try:
        function_method_id = _coerce_text(args.function_method_id, field="function-method-id")
        return _run_write(
            method_name=method,
            http_method="DELETE",
            path=f"{BASE_PATH}/{function_method_id}",
            body=None,
            selector={"functionMethodId": function_method_id},
            proposed_changes=[{"operation": "delete-function-method", "functionMethodId": function_method_id}],
            ctx=ctx,
            requires_ack=True,
            risk_reasons=["wix-function-method-delete", "removes-function-automation-link", "irreversible"],
            verification_notes="Inspect the provider response, then query function-methods to confirm the link is gone.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_function_methods_query(args, ctx) -> int:
    method = "functionMethods.queryFunctionMethods"
    try:
