...
COMMAND_FAMILY = "contact-notes"
BASE_PATH = "/crm/notes/v2/notes"
    ...
    *,
    ...
    return {
        ...
        "state_capture": {
            ...
            "notes": "Contact Notes plans do not capture a full before-state snapshot in this slice.",
        },
        "proposed_changes": proposed_changes,
        "verification_plan": {"type": "provider-response", "notes": verification_notes},
        "rollback": {
            "supported": False,
            "notes": "No automatic rollback. Verify with contact-notes get or query when needed.",
        },
    }
...
def cmd_contact_notes_get(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.get"
    try:
        note_id = _coerce_text(getattr(args, "note_id", None), field="note-id")
        return _run_read(method_name=method, http_method="GET", path=f"{BASE_PATH}/{note_id}", body=None, ctx=ctx)
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_contact_notes_query(args, ctx) -> int:
...
def cmd_contact_notes_create(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.create"
    try:
        body = _read_json_arg(getattr(args, "note_json", None), field="note-json")
        return _run_write(
            method_name=method,
            http_method="POST",
            path=BASE_PATH,
            ...
            proposed_changes=[{"operation": "create-contact-note", "body": body}],
            ctx=ctx,
            requires_ack=False,
            risk_reasons=["contact-note-create", "adds-note-to-contact"],
            verification_notes="Provider response confirms Wix returned a created note or accepted the create request.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_contact_notes_update(args, ctx) -> int:
    ...
    try:
        ...
        return _run_write(
            method_name=method,
            http_method="PATCH",
            path=f"{BASE_PATH}/{note_id}",
            body=body,
            selector={"kind": COMMAND_FAMILY, "note_id": note_id, "operation": "update"},
            proposed_changes=[{"operation": "update-contact-note", "note_id": note_id, "body": body}],
            ctx=ctx,
            requires_ack=False,
            risk_reasons=["contact-note-update", "changes-contact-note-content"],
            verification_notes="Provider response confirms the Update Note request was accepted.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_contact_notes_delete(args, ctx) -> int:
    ...
    try:
        ...
        return _run_write(
            ...
            path=f"{BASE_PATH}/{note_id}",
            body=None,
            selector={"kind": COMMAND_FAMILY, "note_id": note_id, "operation": "delete"},
            proposed_changes=[{"operation": "delete-contact-note", "note_id": note_id}],
            ctx=ctx,
            requires_ack=True,
            risk_reasons=["contact-note-delete", "removes-contact-history-note"],
            verification_notes="Provider response confirms the Delete Note request was accepted.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)
