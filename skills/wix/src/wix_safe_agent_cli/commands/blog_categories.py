...
COMMAND_FAMILY = "blog-categories"
BASE_PATH = "/blog/v3/categories"
...
def _build_plan(
    *,
    ...
    return {
        ...
        "state_capture": {"before_state_available": False, "notes": "Blog Categories plans do not capture a full before-state snapshot in this slice."},
        "proposed_changes": proposed_changes,
        "verification_plan": {"type": "provider-response", "notes": verification_notes},
        "rollback": {"supported": False, "notes": "No automatic rollback. Verify with blog-categories get/list/query or the Wix dashboard when needed."},
    }
...
def cmd_blog_categories_create(args, ctx) -> int:
    ...
    try:
        ...
        return _run_write(
            ...
            proposed_changes=[{"operation": "create-category", "body": body}],
            ctx=ctx,
            requires_ack=False,
            risk_reasons=["blog-category-create"],
            verification_notes="Provider response confirms the Create Category request was accepted.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_blog_categories_get(args, ctx) -> int:
    ...
    try:
        ...
        return _run_read(method_name=method, http_method="GET", path=f"{BASE_PATH}/{category_id}", params=params, body=None, ctx=ctx)
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_blog_categories_update(args, ctx) -> int:
    ...
    try:
        ...
        return _run_write(
            ...
            body=body,
            selector={"kind": COMMAND_FAMILY, "category_id": category_id, "operation": "update"},
            proposed_changes=[{"operation": "update-category", "category_id": category_id, "body": body}],
            ctx=ctx,
            requires_ack=False,
            risk_reasons=["blog-category-update"],
            verification_notes="Provider response confirms the Update Category request was accepted.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_blog_categories_delete(args, ctx) -> int:
    ...
    try:
        ...
        return _run_write(
            ...
            path=f"{BASE_PATH}/{category_id}",
            body=None,
            selector={"kind": COMMAND_FAMILY, "category_id": category_id, "operation": "delete"},
            proposed_changes=[{"operation": "delete-category", "category_id": category_id}],
            ctx=ctx,
            requires_ack=True,
            risk_reasons=["blog-category-delete", "category-removal-affects-blog-navigation"],
            verification_notes="Provider response confirms the Delete Category request was accepted.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_blog_categories_query(args, ctx) -> int:
...
def cmd_blog_categories_list(args, ctx) -> int:
...
def cmd_blog_categories_get_by_slug(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.get-by-slug"
    try:
        slug = _coerce_text(getattr(args, "slug", None), field="slug")
        params = _read_json_arg(getattr(args, "params_json", "{}"), field="params-json", allow_empty=True)
        return _run_read(method_name=method, http_method="GET", path=f"{BASE_PATH}/slugs/{slug}", params=params, body=None, ctx=ctx)
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)
