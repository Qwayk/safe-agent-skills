...
COMMAND_FAMILY = "bookings-attendance"
BASE_PATH = "/bookings/bookings-attendance"
...
def _normalize_filter_body(raw: Any) -> dict[str, Any]:
    if raw is None:
        return {}
    payload = _coerce_json_object(raw, field="filter-json", allow_empty=True)
    if "filter" in payload:
        if not isinstance(payload.get("filter"), dict):
            raise ValidationError("--filter-json filter must be a JSON object")
        return payload
    return {"filter": payload}


def _normalize_request_body(raw: Any, *, field: str) -> dict[str, Any]:
    return _coerce_json_object(raw, field=field, allow_empty=False)
...
def _build_plan(
    *,
    ...
    return {
        ...
        "method": method,
        "risk_level": "high",
        "risk_reasons": ["wix-bookings-attendance-write"],
        ...
        "state_capture": {
            ...
            "notes": "No useful before-state snapshot is captured for this Bookings Attendance provider-response write.",
        },
        "proposed_changes": proposed_changes,
        "verification_plan": {
            "type": "provider-response",
            "notes": "Provider-response-only in this boundary. Run get, query, or count reads for live verification.",
        },
        "rollback": {"supported": False, "notes": "No automatic rollback. Recovery is manual and may require Set Attendance."},
    }
...
def _build_receipt(
    *,
    ...
    return {
        ...
        "state_capture": {
            "before_state_available": False,
            "notes": "No useful before-state snapshot was available for this Bookings Attendance write.",
        },
...
def cmd_bookings_attendance_get(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.get"
    try:
        attendance_id = _coerce_non_empty_text(getattr(args, "attendance_id", None), field="attendance-id")
        return _run_read(method_name=method, http_method="GET", path=f"{BASE_PATH}/{attendance_id}", body=None, ctx=ctx)
    except (ValidationError, RuntimeError) as exc:
        return _write_error(ctx, method=method, exc=exc)


def cmd_bookings_attendance_query(args, ctx) -> int:
...
def cmd_bookings_attendance_count(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.count"
    try:
        body = _normalize_filter_body(getattr(args, "filter_json", None))
        return _run_read(method_name=method, http_method="POST", path=f"{BASE_PATH}/count", body=body, ctx=ctx)
    except (ValidationError, RuntimeError) as exc:
        return _write_error(ctx, method=method, exc=exc)


def cmd_bookings_attendance_set(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.set"
    try:
        body = _normalize_request_body(getattr(args, "attendance_json", None), field="attendance-json")
        return _run_write(
            method_name=method,
            http_method="POST",
            path=f"{BASE_PATH}/set",
            body=body,
            selector={"kind": "bookings-attendance", "operation": "set"},
            proposed_changes=[{"operation": "set", "body": body}],
            ctx=ctx,
        )
...
def cmd_bookings_attendance_bulk_set(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.bulk-set"
    try:
        body = _normalize_request_body(getattr(args, "attendance_json", None), field="attendance-json")
        return _run_write(
            method_name=method,
            http_method="POST",
            path="/bookings/v2/bulk/attendance/set",
            body=body,
            selector={"kind": "bookings-attendance", "operation": "bulk-set"},
            proposed_changes=[{"operation": "bulk-set", "body": body}],
            ctx=ctx,
        )
    except (SafetyError, ValidationError, RuntimeError) as exc:
        return _write_error(ctx, method=method, exc=exc)
...
def cmd_bookings_attendance_delete(args, ctx) -> int:
    ...
    try:
        ...
        return _run_write(
            method_name=method,
            http_method="DELETE",
            path=f"{BASE_PATH}/{attendance_id}",
            body=None,
            selector={"kind": "bookings-attendance", "operation": "delete", "attendance_id": attendance_id},
            proposed_changes=[{"operation": "delete", "attendance_id": attendance_id}],
            ctx=ctx,
            requires_ack=True,
        )
    except (SafetyError, ValidationError, RuntimeError) as exc:
...
def cmd_bookings_attendance_bulk_delete(args, ctx) -> int:
    ...
    try:
        ...
        return _run_write(
            method_name=method,
            http_method="POST",
            path="/bookings/v2/bulk/attendance/delete",
            body=body,
            selector={"kind": "bookings-attendance", "operation": "bulk-delete"},
