...
COMMAND_FAMILY = "badges-v4"
BASE_PATH = "/badges/v4/badges"
...
def _build_plan(
    *,
    ...
    return {
        ...
        "state_capture": {"before_state_available": False, "notes": "Badges V4 plans do not capture a full before-state snapshot in this slice."},
        "proposed_changes": proposed_changes,
        "verification_plan": {"type": "provider-response", "notes": verification_notes},
        "rollback": {"supported": False, "notes": "No automatic rollback. Verify with badges-v4 get or query when needed."},
    }
...
def cmd_badges_v4_create(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.create"
    try:
        body = _read_json_arg(getattr(args, "badge_json", None), field="badge-json")
        return _run_write(
            method_name=method,
            http_method="POST",
            path=BASE_PATH,
            ...
            proposed_changes=[{"operation": "create-badge", "body": body}],
            ctx=ctx,
            requires_ack=False,
            risk_reasons=["badge-create", "badges-may-grant-special-member-page-permissions"],
            verification_notes="Provider response confirms the Create Badge request was accepted.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_badges_v4_get(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.get"
    try:
        badge_id = _coerce_text(getattr(args, "badge_id", None), field="badge-id")
        return _run_read(method_name=method, http_method="GET", path=f"{BASE_PATH}/{badge_id}", body=None, ctx=ctx)
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_badges_v4_update(args, ctx) -> int:
    ...
    try:
        ...
        return _run_write(
            method_name=method,
            http_method="PATCH",
            path=f"{BASE_PATH}/{badge_id}",
            body=body,
            selector={"kind": COMMAND_FAMILY, "badge_id": badge_id, "operation": "update"},
            proposed_changes=[{"operation": "update-badge", "badge_id": badge_id, "body": body}],
            ctx=ctx,
            requires_ack=False,
            risk_reasons=["badge-update", "badges-may-grant-special-member-page-permissions"],
            verification_notes="Provider response confirms the Update Badge request was accepted.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_badges_v4_delete(args, ctx) -> int:
    ...
    try:
        ...
        return _run_write(
            ...
            path=f"{BASE_PATH}/{badge_id}",
            body=None,
            selector={"kind": COMMAND_FAMILY, "badge_id": badge_id, "operation": "delete"},
            proposed_changes=[{"operation": "delete-badge", "badge_id": badge_id}],
            ctx=ctx,
            requires_ack=True,
            risk_reasons=["badge-delete", "official-docs-say-deleting-a-badge-removes-it-from-all-assigned-members"],
            verification_notes="Provider response confirms the Delete Badge request was accepted.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_badges_v4_query(args, ctx) -> int:
...
def cmd_badges_v4_move(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.move"
    try:
        badge_id = _coerce_text(getattr(args, "badge_id", None), field="badge-id")
        body = _read_json_arg(getattr(args, "move_json", None), field="move-json")
        return _run_write(
            method_name=method,
            http_method="POST",
            path=f"{BASE_PATH}/{badge_id}/move",
            body=body,
            selector={"kind": COMMAND_FAMILY, "badge_id": badge_id, "operation": "move"},
            proposed_changes=[{"operation": "move-badge", "badge_id": badge_id, "body": body}],
            ctx=ctx,
            requires_ack=False,
            risk_reasons=["badge-display-order-change"],
            verification_notes="Provider response confirms the Move Badge request was accepted.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)
