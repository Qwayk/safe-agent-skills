...
COMMAND_FAMILY = "member-abouts"
BASE_PATH = "/members/v2/abouts"
...
def _require_revision(body: dict[str, Any]) -> None:
    member_about = body.get("memberAbout")
    if not isinstance(member_about, dict) or not str(member_about.get("revision") or "").strip():
        raise ValidationError("--about-json must include memberAbout.revision for update")
    ...
    *,
    ...
    return {
        ...
        "state_capture": {
            ...
            "notes": "Member About plans do not capture a full before-state snapshot in this slice.",
        },
        "proposed_changes": proposed_changes,
        "verification_plan": {"type": "provider-response", "notes": verification_notes},
        "rollback": {"supported": False, "notes": "No automatic rollback. Verify with member-abouts get or query when needed."},
    }
...
def cmd_member_abouts_create(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.create"
    try:
        body = _read_json_arg(getattr(args, "about_json", None), field="about-json")
        return _run_write(
            method_name=method,
            http_method="POST",
            path=BASE_PATH,
            ...
            proposed_changes=[{"operation": "create-member-about", "body": body}],
            ctx=ctx,
            requires_ack=False,
            risk_reasons=["member-about-create", "creates-rich-profile-content"],
            verification_notes="Provider response confirms the Create Member About request was accepted.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_member_abouts_get(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.get"
    try:
        about_id = _coerce_text(getattr(args, "about_id", None), field="about-id")
        return _run_read(method_name=method, http_method="GET", path=f"{BASE_PATH}/{about_id}", body=None, ctx=ctx)
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_member_abouts_update(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.update"
    try:
        about_id = _coerce_text(getattr(args, "about_id", None), field="about-id")
        body = _read_json_arg(getattr(args, "about_json", None), field="about-json")
        _require_revision(body)
        return _run_write(
            method_name=method,
            http_method="PATCH",
            path=f"{BASE_PATH}/{about_id}",
            body=body,
            selector={"kind": COMMAND_FAMILY, "about_id": about_id, "operation": "update"},
            proposed_changes=[{"operation": "update-member-about", "about_id": about_id, "body": body}],
            ctx=ctx,
            requires_ack=False,
            risk_reasons=["member-about-update", "requires-current-revision"],
            verification_notes="Provider response confirms the Update Member About request was accepted.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_member_abouts_delete(args, ctx) -> int:
    ...
    try:
        ...
        return _run_write(
            ...
            path=f"{BASE_PATH}/{about_id}",
            body=None,
            selector={"kind": COMMAND_FAMILY, "about_id": about_id, "operation": "delete"},
            proposed_changes=[{"operation": "delete-member-about", "about_id": about_id}],
            ctx=ctx,
            requires_ack=True,
            risk_reasons=["member-about-delete", "removes-profile-about-content"],
            verification_notes="Provider response confirms the Delete Member About request was accepted.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_member_abouts_query(args, ctx) -> int:
...
def cmd_member_abouts_get_my(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.get-my"
    try:
