...
COMMAND_FAMILY = "contact-attachments"
BASE_PATH = "/contacts/v4/attachments"
    ...
    *,
    ...
    return {
        ...
        "state_capture": {
            ...
            "notes": "Contact Attachments plans do not capture a full before-state snapshot in this slice.",
        },
        "proposed_changes": proposed_changes,
        "verification_plan": {"type": "provider-response", "notes": verification_notes},
        "rollback": {
            "supported": False,
            "notes": "No automatic rollback. Verify with contact-attachments get or list when needed.",
        },
    }
...
def cmd_contact_attachments_get(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.get"
    try:
        contact_id = _coerce_text(getattr(args, "contact_id", None), field="contact-id")
        attachment_id = _coerce_text(getattr(args, "attachment_id", None), field="attachment-id")
        return _run_read(method_name=method, http_method="GET", path=f"{BASE_PATH}/{contact_id}/{attachment_id}", body=None, ctx=ctx)
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_contact_attachments_list(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.list"
    try:
        contact_id = _coerce_text(getattr(args, "contact_id", None), field="contact-id")
        return _run_read(method_name=method, http_method="GET", path=f"{BASE_PATH}/{contact_id}", body=None, ctx=ctx)
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_contact_attachments_generate_upload_url(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.generate-upload-url"
    try:
        contact_id = _coerce_text(getattr(args, "contact_id", None), field="contact-id")
        body = _read_json_arg(getattr(args, "attachment_json", None), field="attachment-json")
        return _run_write(
            method_name=method,
            http_method="POST",
            path=f"{BASE_PATH}/{contact_id}/upload-url",
            body=body,
            selector={"kind": COMMAND_FAMILY, "contact_id": contact_id, "operation": "generate-upload-url"},
            proposed_changes=[{"operation": "generate-contact-attachment-upload-url", "contact_id": contact_id, "body": body}],
            ctx=ctx,
            requires_ack=False,
            risk_reasons=["contact-attachment-upload-url", "allows-uploading-file-to-contact"],
            verification_notes="Provider response confirms Wix returned or accepted an attachment upload URL request.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_contact_attachments_delete(args, ctx) -> int:
    ...
    try:
        ...
        return _run_write(
            method_name=method,
            http_method="DELETE",
            path=f"{BASE_PATH}/{contact_id}/{attachment_id}",
            body=None,
            selector={"kind": COMMAND_FAMILY, "contact_id": contact_id, "attachment_id": attachment_id, "operation": "delete"},
            proposed_changes=[{"operation": "delete-contact-attachment", "contact_id": contact_id, "attachment_id": attachment_id}],
            ctx=ctx,
            requires_ack=True,
            risk_reasons=["contact-attachment-delete", "removes-file-attachment-from-contact"],
            verification_notes="Provider response confirms the Delete Attachment request was accepted.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)
