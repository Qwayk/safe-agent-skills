...
COMMAND_FAMILY = "blog-likes"
BASE_PATH = "/blog/v1/likes"
...
def _build_plan(
    *,
    ...
    return {
        ...
        "state_capture": {"before_state_available": False, "notes": "Blog Likes plans do not capture a full before-state snapshot."},
        "proposed_changes": proposed_changes,
        "verification_plan": {"type": "provider-response", "notes": verification_notes},
        "rollback": {"supported": False, "notes": "No automatic rollback. Verify with blog-likes get, query, or the Wix dashboard when needed."},
    }
...
def cmd_blog_likes_create(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.create"
    try:
        body = _read_json_arg(getattr(args, "like_json", None), field="like-json")
        return _run_write(
            method_name=method,
            http_method="POST",
            path=BASE_PATH,
            ...
            proposed_changes=[{"operation": "create-like", "body": body}],
            ctx=ctx,
            requires_ack=False,
            risk_reasons=["blog-like-create"],
            verification_notes="Provider response confirms the Create Like request was accepted.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_blog_likes_get(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.get"
    try:
        like_id = _coerce_text(getattr(args, "like_id", None), field="like-id")
        params = _read_json_arg(getattr(args, "params_json", "{}"), field="params-json", allow_empty=True)
        return _run_read(method_name=method, http_method="GET", path=f"{BASE_PATH}/{like_id}", params=params, body=None, ctx=ctx)
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_blog_likes_delete(args, ctx) -> int:
    ...
    try:
        ...
        return _run_write(
            method_name=method,
            http_method="DELETE",
            path=f"{BASE_PATH}/{like_id}",
            body=None,
            selector={"kind": COMMAND_FAMILY, "like_id": like_id, "operation": "delete"},
            proposed_changes=[{"operation": "delete-like", "like_id": like_id}],
            ctx=ctx,
            requires_ack=True,
            risk_reasons=["blog-like-delete", "like-removal-affects-blog-content"],
            verification_notes="Provider response confirms the Delete Like request was accepted.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_blog_likes_query(args, ctx) -> int:
...
def cmd_blog_likes_delete_by_fqdn_entity_id(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.delete-by-fqdn-entity-id"
    try:
        fqdn = _coerce_text(getattr(args, "fqdn", None), field="fqdn")
        entity_id = _coerce_text(getattr(args, "entity_id", None), field="entity-id")
        return _run_write(
            method_name=method,
            http_method="DELETE",
            path=f"{BASE_PATH}/fqdn/{fqdn}/entity-id/{entity_id}",
            body=None,
            selector={"kind": COMMAND_FAMILY, "fqdn": fqdn, "entity_id": entity_id, "operation": "delete-by-fqdn-entity-id"},
            proposed_changes=[{"operation": "delete-like-by-fqdn-entity-id", "fqdn": fqdn, "entity_id": entity_id}],
            ctx=ctx,
            requires_ack=True,
            risk_reasons=["blog-like-delete-by-content-reference", "like-removal-affects-blog-content"],
            verification_notes="Provider response confirms the Delete Like By FQDN And Entity ID request was accepted.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)
