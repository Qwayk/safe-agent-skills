...
COMMAND_FAMILY = "activity-counters"
BASE_PATH = "/members/v1/activity-counters"
...
def _build_plan(
    *,
    ...
    return {
        ...
        "method": method_name,
        "risk_level": "medium",
        "risk_reasons": ["activity-counter-set", "creates-or-updates-member-activity-counters"],
        ...
        "state_capture": {
            ...
            "notes": "Activity Counters plans do not capture a full before-state snapshot in this slice.",
        },
        "proposed_changes": proposed_changes,
        "verification_plan": {"type": "provider-response", "notes": verification_notes},
        "rollback": {
            "supported": False,
            "notes": "No automatic rollback. Verify with activity-counters get or query when needed.",
        },
    }
...
def _run_write(
    *,
    ...
    apply_allowed = reviewed_plan_apply_requested(ctx, command_label=method_name)
...
def cmd_activity_counters_get(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.get"
    try:
        member_id = _coerce_text(getattr(args, "member_id", None), field="member-id")
        return _run_read(method_name=method, http_method="GET", path=f"{BASE_PATH}/{member_id}", params=None, body=None, ctx=ctx)
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_activity_counters_query(args, ctx) -> int:
...
def cmd_activity_counters_set(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.set"
    try:
        member_id = _coerce_text(getattr(args, "member_id", None), field="member-id")
        body = _read_json_arg(getattr(args, "activity_counters_json", None), field="activity-counters-json")
        return _run_write(
            method_name=method,
            http_method="PUT",
            path=f"{BASE_PATH}/{member_id}",
            body=body,
            selector={"kind": COMMAND_FAMILY, "member_id": member_id, "operation": "set"},
            proposed_changes=[{"operation": "set-activity-counters", "member_id": member_id, "body": body}],
            ctx=ctx,
            verification_notes="Provider response confirms the Set Activity Counters request was accepted.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)
