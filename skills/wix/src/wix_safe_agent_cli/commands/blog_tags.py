...
def _build_plan(
    *,
    ...
    return {
        ...
        "state_capture": {"before_state_available": False, "notes": "Blog Tags plans do not capture a full before-state snapshot."},
        "proposed_changes": proposed_changes,
        "verification_plan": {"type": "provider-response", "notes": verification_notes},
        "rollback": {"supported": False, "notes": "No automatic rollback. Verify with blog-tags get, query, get-by-label, get-by-slug, or the Wix dashboard when needed."},
    }
...
def cmd_blog_tags_create(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.create"
    try:
        body = _read_json_arg(getattr(args, "tag_json", None), field="tag-json")
        return _run_write(
            method_name=method,
            http_method="POST",
            path=BASE_PATH,
            ...
            proposed_changes=[{"operation": "create-tag", "body": body}],
            ctx=ctx,
            requires_ack=False,
            risk_reasons=["blog-tag-create"],
            verification_notes="Provider response confirms the Create Tag request was accepted.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_blog_tags_get(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.get"
    try:
        tag_id = _coerce_text(getattr(args, "tag_id", None), field="tag-id")
        params = _read_json_arg(getattr(args, "params_json", "{}"), field="params-json", allow_empty=True)
        return _run_read(method_name=method, http_method="GET", path=f"{BASE_PATH}/{tag_id}", params=params, body=None, ctx=ctx)
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_blog_tags_delete(args, ctx) -> int:
    ...
    try:
        ...
        return _run_write(
            method_name=method,
            http_method="DELETE",
            path=f"{BASE_PATH}/{tag_id}",
            body=None,
            selector={"kind": COMMAND_FAMILY, "tag_id": tag_id, "operation": "delete"},
            proposed_changes=[{"operation": "delete-tag", "tag_id": tag_id}],
            ctx=ctx,
            requires_ack=True,
            risk_reasons=["blog-tag-delete", "tag-removal-affects-blog-posts"],
            verification_notes="Provider response confirms the Delete Tag request was accepted.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_blog_tags_query(args, ctx) -> int:
...
def cmd_blog_tags_get_by_label(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.get-by-label"
    try:
        label = _coerce_text(getattr(args, "label", None), field="label")
        params = _read_json_arg(getattr(args, "params_json", "{}"), field="params-json", allow_empty=True)
        return _run_read(method_name=method, http_method="GET", path=f"{BASE_PATH}/labels/{label}", params=params, body=None, ctx=ctx)
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_blog_tags_get_by_slug(args, ctx) -> int:
