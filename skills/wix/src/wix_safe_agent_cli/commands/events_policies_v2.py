...
COMMAND_FAMILY = "events-policies-v2"
BASE_PATH = "/events-policies/v2/policies"
...
def _require_policy_revision(payload: dict[str, Any]) -> None:
    policy = payload.get("policy")
    if not isinstance(policy, dict):
        raise ValidationError("--policy-json must include a policy object")
    revision = policy.get("revision")
    if revision is None or str(revision).strip() == "":
        raise ValidationError("--policy-json for update must include policy.revision")
...
def _build_plan(
    *,
    ...
    return {
        ...
        "state_capture": {"before_state_available": False, "notes": "Events Policies V2 plans do not capture a full before-state snapshot in this slice."},
        "proposed_changes": proposed_changes,
        "verification_plan": {"type": "provider-response", "notes": verification_notes},
        "rollback": {"supported": False, "notes": "No automatic rollback. Verify with policy reads and the Wix dashboard when needed."},
    }
...
def _run_write(
    *,
    ...
    verification_notes: str = "Provider response confirms the policy request was accepted.",
) -> int:
    auth = _resolve_auth(ctx)
    request: dict[str, Any] = {"method": http_method, "path": path}
    if body is not None:
    ...
    plan = _build_plan(
        ...
        risk_reasons=risk_reasons or ["wix-events-policies-v2-write"],
...
def cmd_events_policies_v2_create(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.create"
    try:
        body = _read_json_arg(getattr(args, "policy_json", None), field="policy-json")
        return _run_write(method_name=method, http_method="POST", path=BASE_PATH, body=body, selector={"kind": COMMAND_FAMILY, "operation": "create"}, proposed_changes=[{"operation": "create-event-policy"}], ctx=ctx)
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_events_policies_v2_get(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.get"
    try:
        policy_id = _coerce_text(getattr(args, "policy_id", None), field="policy-id")
        return _run_read(method_name=method, http_method="GET", path=f"{BASE_PATH}/{policy_id}", ctx=ctx)
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_events_policies_v2_update(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.update"
    try:
        policy_id = _coerce_text(getattr(args, "policy_id", None), field="policy-id")
        body = _read_json_arg(getattr(args, "policy_json", None), field="policy-json")
        _require_policy_revision(body)
        return _run_write(method_name=method, http_method="PATCH", path=f"{BASE_PATH}/{policy_id}", body=body, selector={"kind": COMMAND_FAMILY, "policy_id": policy_id}, proposed_changes=[{"operation": "update-event-policy", "policy_id": policy_id}], ctx=ctx)
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_events_policies_v2_delete(args, ctx) -> int:
    ...
    try:
        ...
        return _run_write(
            ...
            body=None,
            selector={"kind": COMMAND_FAMILY, "policy_id": policy_id},
            proposed_changes=[{"operation": "delete-event-policy", "policy_id": policy_id}],
            ctx=ctx,
            requires_ack=True,
            risk_reasons=["wix-events-policies-v2-delete", "permanent-policy-removal"],
            verification_notes="Provider response confirms the permanent policy deletion request was accepted.",
        )
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)


def cmd_events_policies_v2_query(args, ctx) -> int:
...
def cmd_events_policies_v2_reorder(args, ctx) -> int:
    method = f"{COMMAND_FAMILY}.reorder"
    try:
        body = _read_json_arg(getattr(args, "request_json", None), field="request-json")
        return _run_write(method_name=method, http_method="POST", path=f"{BASE_PATH}/reorder", body=body, selector={"kind": COMMAND_FAMILY, "operation": "reorder"}, proposed_changes=[{"operation": "reorder-event-policies"}], ctx=ctx)
    except Exception as exc:
        return _emit_error(ctx, method=method, exc=exc)
