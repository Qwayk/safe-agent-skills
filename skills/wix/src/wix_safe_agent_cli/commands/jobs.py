...
_ACTIONS = {
    # Public shipped surface does not include synthetic jobs actions yet.
    # Real command implementations must populate this registry before use.
}


def _supported_action_names() -> str:
...
def _build_jobs_plan(*, job_file: str | None, rows: list[dict[str, Any]], ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "verification_plan": {
            "type": "per-row",
            "notes": "Jobs actions must include read-back or idempotent verification in each row handler.",
        },
        "rollback": {"supported": False, "notes": "Jobs runner has no generic rollback in this command path."},
    }
...
def cmd_jobs_run(args: Any, ctx: dict[str, Any]) -> int:
    ...
    for row_num, row in enumerate(rows, start=1):
        ...
        try:
            ...
            if spec is None:
                raise ValidationError(f"Unknown action: {action_name} (supported: {_supported_action_names()})")
    ...
    receipt = {
        ...
        "verification": {"ok": errors == 0, "details": {"type": "jobs", "notes": "No row actions are configured yet."}},
