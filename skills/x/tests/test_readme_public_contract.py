...
class TestReadmePublicContract(unittest.TestCase):
    def test_public_readme_opening_stays_user_facing(self) -> None:
        ...
        self.assertNotIn("# x-api-tool", text)
        self.assertIn("X is where posts", text)
        for stale_phrase in [
            "Use this skill when",
    ...
    def test_helpful_docs_stay_human_facing(self) -> None:
        ...
        self.assertIn("[Browse all X docs](docs/README.md)", text)
    ...
    def test_start_here_section_stays_human_facing(self) -> None:
        ...
        self.assertIn("[What you can do with X](docs/use_cases.md)", text)
        self.assertIn("[Connect your X account](docs/onboarding.md)", text)
    ...
    def test_quickstart_opens_with_human_links(self) -> None:
        ...
        self.assertIn("[What you can do with X](use_cases.md)", text)
        self.assertIn("[Connect your X account](onboarding.md)", text)
    ...
    def test_use_cases_stay_specific_and_human(self) -> None:
        ...
        self.assertIn("X work can affect", text)
        self.assertIn("## Good jobs to give the agent", text)
        self.assertIn("## What the agent should show you", text)
        self.assertNotIn("Most no-code tools", text)
        self.assertNotIn("Who it helps most", text)
