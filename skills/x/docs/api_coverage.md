...
X coverage shows exactly what the shipped commands can do with users, posts, DMs, lists, spaces, and auth-related work. Start here when an ask sounds possible but you need to know whether it is already shipped, read-only, plan-first, gated, excluded, or outside the tool.
...
A good first coverage check is: "Check whether the shipped commands can inspect users, posts, and lists, then show which X actions are shipped."
