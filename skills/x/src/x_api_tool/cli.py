...
def build_parser() -> argparse.ArgumentParser:
    p = _ToolArgumentParser(prog="x-api-tool")
    ...
    p.add_argument(
        "--live",
        action="store_true",
        help="Allow live API reads for read-only commands (default is plan-only/dry-run)",
    )
    ...
    p.add_argument(
        "--no-provenance",
        action="store_true",
        help="Do not include local proof paths (run_id/audit_log/runs_index) in JSON output",
    )
    ...
    pkce = auth_sub.add_parser("pkce", help="OAuth2 PKCE helpers (no browser automation)")
    pkce_sub = pkce.add_subparsers(dest="pkce_cmd", required=True, parser_class=_ToolArgumentParser)
    pkce_start = pkce_sub.add_parser("start", help="Create authorize URL and store verifier locally")
    pkce_start.set_defaults(func=auth_cmd.cmd_auth_pkce_start, write_capable=True)
    pkce_finish = pkce_sub.add_parser("finish", help="Exchange code for token and store it locally")
    pkce_finish.add_argument("--redirect-url", default=None, help="Full redirect URL containing code and state")
    pkce_finish.add_argument("--code", default=None, help="Authorization code (if not using --redirect-url)")
    pkce_finish.add_argument("--state", default=None, help="State (optional; verified if present)")
    pkce_finish.set_defaults(func=auth_cmd.cmd_auth_pkce_finish, write_capable=True)

    api = sub.add_parser("api", help="Pinned OpenAPI operations (plan-only by default)")
    api_sub = api.add_subparsers(dest="api_cmd", required=True, parser_class=_ToolArgumentParser)
    api_ops = api_sub.add_parser("ops", help="List pinned OpenAPI operations (operationIds)")
    ...
    api_ops_list.add_argument("--tag", default=None, help="Filter by tag (exact match)")
    api_ops_list.add_argument("--security-scheme", default=None, help="Filter by security scheme (exact name)")
    ...
    reserved = {"ops"}
    collisions = sorted({op.operation_id for op in ops if op.operation_id in reserved})
    if collisions:
        raise RuntimeError(f"OpenAPI operationIds collide with reserved api subcommands: {', '.join(collisions)}")

    def _add_api_op_shared_args(p2: argparse.ArgumentParser) -> None:
        p2.add_argument("--auth", default="auto", help="Auth mode: auto|app|user|none (default: auto)")
        p2.add_argument("--path-json", default=None, help="JSON object (or file path) with path params")
        p2.add_argument("--query-json", default=None, help="JSON object (or file path) with query params")
        p2.add_argument("--body-json", default=None, help="JSON object (or file path) with request body")
        p2.add_argument("--path", action="append", default=None, help="Path param as k=v (repeatable)")
        p2.add_argument("--query", action="append", default=None, help="Query param as k=v (repeatable)")
        p2.add_argument("--file", action="append", default=None, help="Multipart file as field=/path/to/file (repeatable)")

    for op in sorted(ops, key=lambda o: (o.operation_id, o.method, o.path)):
        api_op = api_sub.add_parser(op.operation_id, help=f"{op.method} {op.path}")
        _add_api_op_shared_args(api_op)
        api_op.set_defaults(func=api_cmd.cmd_api_call, write_capable=True, op=op.operation_id)

    users = sub.add_parser("users", help="User helpers (safe by default)")
    users_sub = users.add_subparsers(dest="users_cmd", required=True, parser_class=_ToolArgumentParser)
    users_resolve = users_sub.add_parser("resolve", help="Resolve a username to a user id")
    users_resolve.add_argument("--username", required=True, help="Username without @")
    users_resolve.add_argument(
        "--include-receives-your-dm",
        action="store_true",
        help="Request the receives_your_dm field when available (requires user-context auth for many accounts)",
    )
    users_resolve.set_defaults(func=users_cmd.cmd_users_resolve, write_capable=False)

    dm = sub.add_parser("dm", help="DM-first workflows (strict safety + policy gates)")
    dm_sub = dm.add_subparsers(dest="dm_cmd", required=True, parser_class=_ToolArgumentParser)

    dm_can = dm_sub.add_parser("can-send", help="Check if a user receives your DMs (live only)")
    dm_can.add_argument("--to-username", default=None, help="Recipient username (without @)")
    dm_can.add_argument("--to-user-id", default=None, help="Recipient user id")
    dm_can.set_defaults(func=dm_cmd.cmd_dm_can_send, write_capable=False)

    dm_send = dm_sub.add_parser("send", help="Send one DM (plan-only by default)")
    dm_send.add_argument("--to-username", default=None, help="Recipient username (without @)")
    dm_send.add_argument("--to-user-id", default=None, help="Recipient user id")
    dm_send.add_argument("--message", required=True, help="Message text")
    dm_send.set_defaults(func=dm_cmd.cmd_dm_send, write_capable=True)

    dm_bulk = dm_sub.add_parser("bulk-send", help="Send DMs from a CSV (strict policy gates)")
    dm_bulk.add_argument("--csv", required=True, help="CSV file path with recipient/message/intent_evidence columns")
    dm_bulk.add_argument(
        "--opt-out-line",
        default="",
        help="Opt-out line appended to every message (required for bulk DMs)",
    )
    dm_bulk.add_argument(
        "--min-delay-s",
        type=float,
        default=1.0,
        help="Minimum delay between approved bulk DM sends; apply still requires explicit no-snapshot approval when no saved before-state is available",
    )
    dm_bulk.set_defaults(func=dm_cmd.cmd_dm_bulk_send, write_capable=True)

    dm_opt = dm_sub.add_parser("opt-out", help="Local opt-out ledger (stored under .state/ next to env file)")
    dm_opt_sub = dm_opt.add_subparsers(dest="opt_cmd", required=True, parser_class=_ToolArgumentParser)
    dm_opt_add = dm_opt_sub.add_parser("add", help="Add a recipient to the opt-out ledger")
    dm_opt_add.add_argument("--recipient", required=True, help="Username (without @) or user id")
    dm_opt_add.add_argument("--reason", default=None, help="Optional reason/note")
    dm_opt_add.set_defaults(func=dm_cmd.cmd_dm_opt_out_add, write_capable=True)
    dm_opt_list = dm_opt_sub.add_parser("list", help="List opt-out recipients")
    dm_opt_list.set_defaults(func=dm_cmd.cmd_dm_opt_out_list, write_capable=False)
...
def main(argv: list[str]) -> int:
    ...
    if bool(getattr(args, "no_provenance", False)):
        out.set_provenance({})
    else:
        out.set_provenance(
            {
    ...
    try:
        if bool(args.version):
            payload = {"ok": True, "tool": "x-api-tool", "version": __version__}
            if args.output == "json":
                out.emit(payload)
            else:
                print(f"x-api-tool {__version__}")
            return 0

        if not getattr(args, "cmd", None):
            parser.error("Missing command. Use --help to see available commands.")

        command_str = "x-api-tool " + " ".join(argv)
        audit.bind_context(
            {
                "tool": "x-api-tool",
                "version": __version__,
