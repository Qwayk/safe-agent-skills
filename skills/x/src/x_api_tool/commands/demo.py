...
def cmd_demo_write(args: Any, ctx: dict[str, Any]) -> int:
    ...
    if not bool(ctx.get("yes")):
        out = {
            ...
            "dry_run": False,
            "refused": True,
            "reasons": ["Refused: demo write requires --apply --yes"],
            "refusal_type": "SafetyError",
        }
        ctx["audit"].write("demo.write.refused", {"reasons": ["Refused: demo write requires --apply --yes"]})
        ctx["out"].emit(out)
        return 0
    ...
    ctx["audit"].write("demo.write.refused", {"reasons": out["reasons"]})
    ctx["out"].emit(out)
    return 0
