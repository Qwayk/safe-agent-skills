        ...
        *,
        ...
        while True:
            ...
            try:
                ...
                redacted_error = str(sanitize(str(e)))
                if self._verbose:
                    print(
                        f"[http] {method} {display_url} -> EXCEPTION ({ms}ms): {type(e).__name__}: {redacted_error}",
                        file=sys.stderr,
                    )
                raise RuntimeError(
                    f"Request failed for {method} {display_url}: {type(e).__name__}: {redacted_error}"
                ) from e
            ms = int((time.time() - start) * 1000)
            if self._verbose:
