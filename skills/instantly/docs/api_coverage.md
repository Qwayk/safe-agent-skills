...
Instantly coverage shows exactly what the shipped commands can do with campaigns, leads, accounts, inboxes, analytics, and send-related workflows. Start here when an ask sounds possible but you need to know whether it is already shipped, read-only, plan-first, gated, excluded, or outside the tool.
...
A good first coverage check is: "Check whether the shipped commands can inspect campaigns, leads, inboxes, and analytics, then show which send actions are covered."

## Coverage notes

Check this section when you need to confirm whether an Instantly API area already has a CLI command and which safety gates apply.
