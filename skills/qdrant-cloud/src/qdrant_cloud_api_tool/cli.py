...
from .operations_v1 import OPERATIONS_BY_DOMAIN
from .runs import (
    RunContext,
...
def build_parser() -> argparse.ArgumentParser:
    p = _ToolArgumentParser(prog="qdrant-cloud-api-tool")
    ...
    p.add_argument(
        "--ack-spend-money",
        action="store_true",
        help="Extra acknowledgement for money-moving/billing actions",
    )
    ...
    # Explicit Qdrant Cloud API operations (one command per official RPC).
    for domain, ops in sorted(OPERATIONS_BY_DOMAIN.items(), key=lambda kv: kv[0]):
        grp = sub.add_parser(domain, help=f"{domain} operations (official)")
        grp_sub = grp.add_subparsers(dest=f"{domain}_cmd", required=True, parser_class=_ToolArgumentParser)
        for op in ops:
            cmd = grp_sub.add_parser(op.command_name, help=f"{op.http_verb} {op.http_path}")
            cmd.add_argument("--request-json", default=None, help="Request JSON file (object)")
            for template_name, arg_name in op.path_params:
                cmd.add_argument(
                    f"--{arg_name.replace('_', '-')}",
                    dest=arg_name,
                    required=True,
                    help=f"Path parameter for {{{template_name}}}",
                )
            cmd.set_defaults(
                func=ops_cmd.cmd_qdrant_cloud_operation,
                write_capable=(op.http_verb.upper() != "GET"),
                rpc=op.rpc,
            )
...
def main(argv: list[str]) -> int:
    ...
    try:
        if bool(args.version):
            payload = {"ok": True, "tool": "qdrant-cloud-api-tool", "version": __version__}
            if args.output == "json":
                out.emit(payload)
            else:
                print(f"qdrant-cloud-api-tool {__version__}")
            return 0

        if not getattr(args, "cmd", None):
            parser.error("Missing command. Use --help to see available commands.")

        command_str = "qdrant-cloud-api-tool " + " ".join(argv)
        audit.bind_context(
            {
                "tool": "qdrant-cloud-api-tool",
                "version": __version__,
                "command": command_str,
        ...
        _finalize_run_artifacts(
            ...
            tool="qdrant-cloud-api-tool",
            version=__version__,
            command="qdrant-cloud-api-tool " + " ".join(argv),
            env_fingerprint=None,
            output_obj=out.last if isinstance(out.last, dict) else None,
            audit_log_path=run_audit_log_path or global_audit_log_path,
            audit_log_global_path=global_audit_log_path,
