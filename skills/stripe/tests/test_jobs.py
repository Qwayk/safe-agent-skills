...
from stripe_api_tool.commands.jobs import cmd_jobs_run
from stripe_api_tool.output import Output
...
class TestJobs(unittest.TestCase):
    def _ctx(self, **overrides):
        ctx = {
            ...
            "tool_version": "0.0.0",
            "command_str": "stripe-api-tool jobs run",
            "apply": False,
            "yes": False,
    ...
    def test_write_action_requires_apply_yes(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            ctx = self._ctx(apply=True, yes=False, command_str="stripe-api-tool --apply jobs run")
            buf = io.StringIO()
    ...
    def test_write_action_is_planned_in_dry_run(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertTrue(payload["dry_run"])
            self.assertEqual(payload["count"], 1)
            rollback = (payload.get("plan") or {}).get("rollback") or {}
            self.assertEqual(rollback.get("supported"), False)
            self.assertEqual(
                rollback.get("notes"),
                "No snapshot is taken and no automatic rollback is available.",
            )
    ...
    def test_plan_out_and_plan_in_apply(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            ctx_apply = self._ctx(
                ...
                command_str="stripe-api-tool --apply --yes --plan-in plan.json --file jobs.csv jobs run",
            )
            buf2 = io.StringIO()
            ...
            receipt = payload2.get("receipt") or {}
