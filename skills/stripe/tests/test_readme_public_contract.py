...
class TestReadmePublicContract(unittest.TestCase):
    def test_public_readme_opening_stays_user_facing(self) -> None:
        ...
        self.assertTrue(text.startswith("# Stripe\n"))
        self.assertNotIn("## Simplicity lock", text)
        self.assertNotIn("# stripe-api-tool", text)
    ...
    def test_helpful_docs_stay_human_facing(self) -> None:
        ...
        self.assertIn("[Browse all Stripe docs](docs/README.md)", text)
    ...
    def test_quickstart_opens_with_human_links(self) -> None:
        ...
        self.assertIn("[What you can do with Stripe](use_cases.md)", text)
        self.assertIn("[Connect your Stripe account](onboarding.md)", text)
