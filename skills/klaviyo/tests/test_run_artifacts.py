...
class TestRunArtifacts(unittest.TestCase):
    def test_api_command_creates_run_folder_and_index(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            run_id = "2026-01-19T120000Z_apisafe"

            buf = io.StringIO()
            with redirect_stdout(buf):
                rc = main(["--env-file", str(env_path), "--run-id", run_id, "api", "get_accounts"])
    ...
    def test_runs_list_and_show_work(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            env_path = root / ".env"
            env_path.write_text("KLAVIYO_API_BASE_URL=http://example.invalid\nKLAVIYO_TIMEOUT_S=30\n", encoding="utf-8")

            run_id = "2026-01-19T120500Z_runsafe"
            with io.StringIO() as buf:
                with redirect_stdout(buf):
                    _ = main(["--env-file", str(env_path), "--run-id", run_id, "api", "get_accounts"])
    ...
    def test_refusal_still_creates_run_history(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            run_id = "2026-01-19T121000Z_refuse2"
            buf = io.StringIO()
            with redirect_stdout(buf):
                rc = main(
                    [
