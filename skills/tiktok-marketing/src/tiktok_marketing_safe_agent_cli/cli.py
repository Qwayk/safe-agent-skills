...
def build_parser() -> argparse.ArgumentParser:
    p = _ToolArgumentParser(prog="tiktok-marketing-api-tool")
    ...
    p.add_argument(
        "--live",
        action="store_true",
        help="Allow live API calls (default is plan-only / no network)",
    )
    ...
    api = sub.add_parser("api", help="TikTok Marketing API operations")
    api_sub = api.add_subparsers(dest="api_cmd", required=True, parser_class=_ToolArgumentParser)
    api_ops = api_sub.add_parser("ops", help="List or show pinned operations")
    ...
    api_ops_list.add_argument("--family", default=None, help="Filter by operation family")
    api_ops_list.set_defaults(func=api_cmd.cmd_api_ops_list, write_capable=False)
    api_ops_show = api_ops_sub.add_parser("show", help="Show operation details")
    api_ops_show.add_argument("--op", dest="operation_command", required=True, help="operation_command from manifest")
    ...
    def _add_api_op_shared_args(parser: argparse.ArgumentParser) -> None:
        parser.add_argument("--path-json", default=None, help="JSON object (or file path) for path params")
        parser.add_argument("--query-json", default=None, help="JSON object (or file path) for query params")
        parser.add_argument("--body-json", default=None, help="JSON object (or file path) for request body")
        parser.add_argument("--path", action="append", default=None, help="Path param as k=v (repeatable)")
        parser.add_argument("--query", action="append", default=None, help="Query param as k=v (repeatable)")
        parser.add_argument("--body", action="append", default=None, help="Body param as k=v (repeatable)")
        parser.add_argument("--file", action="append", default=None, help="Multipart file as field=/path/to/file (repeatable)")

    for op in sorted(ops, key=lambda op: (op.operation_command, op.method, op.path)):
        op_parser = api_sub.add_parser(op.operation_command, help=f"{op.method} {op.path}")
        _add_api_op_shared_args(op_parser)
        op_parser.set_defaults(func=api_cmd.cmd_api_call, write_capable=True, op=op.operation_command)
...
def main(argv: list[str]) -> int:
    ...
    try:
        if bool(args.version):
            payload = {"ok": True, "tool": "tiktok-marketing-api-tool", "version": __version__}
            if args.output == "json":
                out.emit(payload)
            else:
                print(f"tiktok-marketing-api-tool {__version__}")
            return 0

        if not getattr(args, "cmd", None):
            parser.error("Missing command. Use --help to see available commands.")

        command_str = "tiktok-marketing-api-tool " + " ".join(argv)
        audit.bind_context(
            {
        ...
        _finalize_run_artifacts(
            ...
            tool="tiktok-marketing-api-tool",
            version=__version__,
            command="tiktok-marketing-api-tool " + " ".join(argv),
            env_fingerprint=None,
            output_obj=out.last if isinstance(out.last, dict) else None,
            audit_log_path=run_audit_log_path or global_audit_log_path,
            audit_log_global_path=global_audit_log_path,
