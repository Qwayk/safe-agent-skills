...
TikTok Marketing coverage shows exactly what the shipped commands can do with advertiser accounts, campaigns, ad groups, ads, reports, and generated API operations. Start here when an ask sounds possible but you need to know whether it is already shipped, read-only, plan-first, gated, excluded, or outside the tool.
...
A good first coverage check is: "Check whether the shipped commands can inspect campaigns, ad groups, and reports, then show which TikTok Marketing operations are shipped."
