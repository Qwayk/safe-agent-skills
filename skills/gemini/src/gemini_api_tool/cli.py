...
def build_parser() -> argparse.ArgumentParser:
    p = _ToolArgumentParser(prog="gemini-api-tool")
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        if run_ctx.enabled and run_ctx.artifacts_dir:
            if not bool(args.apply) and not ctx_artifact_paths["plan_out"]:
                ctx_artifact_paths["plan_out"] = str(run_ctx.artifacts_dir / "plan.json")
            if bool(args.apply) and not ctx_artifact_paths["receipt_out"]:
                ctx_artifact_paths["receipt_out"] = str(run_ctx.artifacts_dir / "receipt.json")

        if bool(args.version):
            payload = {"ok": True, "tool": "gemini-api-tool", "version": __version__}
            if args.output == "json":
                out.emit(payload)
            else:
                print(f"gemini-api-tool {__version__}")
            return 0

        if not getattr(args, "cmd", None):
            parser.error("Missing command. Use --help to see available commands.")

        command_str = "gemini-api-tool " + " ".join(argv)
        audit.bind_context(
            {
                "tool": "gemini-api-tool",
                "version": __version__,
                "command": command_str,
        ...
        if str(getattr(args, "cmd", "") or "") in {"runs", "onboarding", "demo", "jobs"}:
            ctx = {
                "cfg": None,
                "out": out,
                "audit": audit,
                ...
                "apply": bool(args.apply),
                "yes": bool(args.yes),
                "plan_out": ctx_artifact_paths["plan_out"],
                "plan_in": args.plan_in,
                "receipt_out": ctx_artifact_paths["receipt_out"],
            ...
            _finalize_run_artifacts(
                ...
                version=__version__,
                command=command_str,
                env_fingerprint=str(args.env_file),
        ...
        if run_ctx.enabled and run_ctx.artifacts_dir:
            ctx["plan_out"] = ctx_artifact_paths["plan_out"]
            ctx["receipt_out"] = ctx_artifact_paths["receipt_out"]
        ...
        _finalize_run_artifacts(
            ...
            tool="gemini-api-tool",
            version=__version__,
            command="gemini-api-tool " + " ".join(argv),
            env_fingerprint=None,
            output_obj=out.last if isinstance(out.last, dict) else None,
            audit_log_path=run_audit_log_path or global_audit_log_path,
            audit_log_global_path=global_audit_log_path,
