...
def cmd_onboarding(args: object, ctx: dict) -> int:
    ...
    missing: list[str] = []
    if not _has_nonempty_env_value(lines, "GEMINI_API_KEY"):
        missing.append("GEMINI_API_KEY")

    steps = [
        "Copy .env.example to .env (local-only; do not commit it).",
        "Open .env and fill the required values:",
        "  - GEMINI_API_KEY=<your Gemini API key>",
        "  - GEMINI_API_BASE_URL=https://generativelanguage.googleapis.com",
        "Run: gemini-api-tool --output json auth check",
        "Run: gemini-api-tool --output json models list",
    ]
    ...
    payload = {
        ...
        "onboarding": {
            ...
            "missing": missing,
            "next_command": "gemini-api-tool --output json auth check",
            "steps": steps,
        },
    }
    ...
    if str(getattr(args, "output", "json")) == "json":
        ...
        print("")
        print("Next: gemini-api-tool --output json auth check")
    return 0
