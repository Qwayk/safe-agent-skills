...
def cmd_demo_read(args: Any, ctx: dict[str, Any]) -> int:
    _ = args
    out = {"ok": True, "message": "demo read (safe)", "env": _env_fingerprint(ctx)}
...
def _build_demo_plan(*, selector: str, ctx: dict[str, Any]) -> dict[str, Any]:
    env_fingerprint = _env_fingerprint(ctx)
    return {
        "tool": ctx.get("tool") or "gemini-api-tool",
...
def _validate_plan_for_apply(plan: dict[str, Any], *, selector: str, ctx: dict[str, Any]) -> None:
    ...
    if str(baseline.get("env_fingerprint") or "") != _env_fingerprint(ctx):
...
def _env_fingerprint(ctx: dict[str, Any]) -> str:
    cfg = ctx.get("cfg")
    if cfg is not None:
        return str(getattr(cfg, "base_url", "") or "")
    return str(ctx.get("env_file") or "local")
