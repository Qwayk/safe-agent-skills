...
from .sanitize import redact_pair


_COMMAND_VALUE_REDACTIONS = {
    "--body-json": "<redacted-json>",
    "--body-file": "<redacted-path>",
}
_COMMAND_PAIR_VALUE_FLAGS = {"--path-param", "--query"}
...
def _safe_command_from_argv(argv: list[str]) -> str:
    safe: list[str] = []
    skip_next = False
    for idx, arg in enumerate(argv):
        ...
        for flag in _COMMAND_PAIR_VALUE_FLAGS:
            if arg == flag:
                value = argv[idx + 1] if idx + 1 < len(argv) else ""
                safe.extend([flag, _redact_command_pair(value)])
                skip_next = True
                matched_inline = True
                break
            if arg.startswith(flag + "="):
                safe.append(f"{flag}={_redact_command_pair(arg.split('=', 1)[1])}")
                matched_inline = True
                break
        if matched_inline:
            continue
        for flag, replacement in _COMMAND_VALUE_REDACTIONS.items():
            if arg == flag:
                safe.extend([flag, replacement])
                skip_next = True
                matched_inline = True
                break
            if arg.startswith(flag + "="):
                safe.append(f"{flag}={replacement}")
                matched_inline = True
                break
        if not matched_inline:
            safe.append(arg)
    return "make-com-safe " + " ".join(safe)


def _redact_command_pair(raw: str) -> str:
    return redact_pair(raw, marker="<redacted>")
...
def build_parser() -> argparse.ArgumentParser:
    p = _ToolArgumentParser(prog="make-com-safe")
    ...
    p.add_argument(
        "--ack-no-snapshot",
        action="store_true",
        help="Extra acknowledgement when Make does not provide a safe before-state snapshot",
    )
    ...
    api = sub.add_parser("api", help="Explicit Make API commands from the pinned official inventory")
    api.add_argument("--inventory", default=None, help="Optional official inventory JSON override")
    api_sub = api.add_subparsers(dest="api_cmd", required=True, parser_class=_ToolArgumentParser)
    api_list = api_sub.add_parser("list", help="List Make API families and explicit commands")
    api_list.set_defaults(func=api_cmd.cmd_api_list, write_capable=False)
    api_schema = api_sub.add_parser("schema", help="Show one pinned Make API operation")
    api_schema.add_argument("family", help="Family slug, such as scenarios")
    api_schema.add_argument("operation", help="Operation command, such as list-scenarios")
    api_schema.set_defaults(func=api_cmd.cmd_api_schema, write_capable=False)
    for family, ops in operations_by_family(load_inventory()).items():
        fam_parser = api_sub.add_parser(family, help=f"Make API family: {family}")
        fam_sub = fam_parser.add_subparsers(dest="operation", required=True, parser_class=_ToolArgumentParser)
        for op in ops:
            op_parser = fam_sub.add_parser(str(op["command"]), help=str(op.get("summary") or op["command"]))
            op_parser.add_argument("--path-param", action="append", default=[], help="Path value as name=value")
            op_parser.add_argument("--query", action="append", default=[], help="Query value as name=value")
            op_parser.add_argument("--body-json", default=None, help="JSON request body for write operations")
            op_parser.add_argument("--body-file", default=None, help="JSON file request body for write operations")
            op_parser.set_defaults(
                func=api_cmd.cmd_api_call,
                family=family,
                operation=str(op["command"]),
...
def main(argv: list[str]) -> int:
    parser = build_parser()
    command_str = _safe_command_from_argv(argv)
    ...
    try:
        if bool(args.version):
            payload = {"ok": True, "tool": "make-com-safe", "version": __version__}
            if args.output == "json":
                out.emit(payload)
            else:
                print(f"make-com-safe {__version__}")
            return 0
        ...
        api_local = str(getattr(args, "cmd", "") or "") == "api" and str(getattr(args, "api_cmd", "") or "") in {
            "list",
            "schema",
        }
        if str(getattr(args, "cmd", "") or "") in {"runs", "onboarding"} or api_local:
            ctx = {
                "cfg": None,
                "out": out,
                "audit": audit,
