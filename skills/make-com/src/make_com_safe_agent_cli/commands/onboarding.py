...
def cmd_onboarding(args: object, ctx: dict) -> int:
    ...
    missing: list[str] = []
    if not _has_nonempty_env_value(lines, "MAKE_BASE_URL"):
        missing.append("MAKE_BASE_URL")
    if not _has_nonempty_env_value(lines, "MAKE_API_TOKEN"):
    ...
    steps = [
        ...
        "Run: make-com-safe --output json auth check",
    ]
    ...
    payload = {
        ...
        "onboarding": {
            ...
            "missing": missing,
            "next_command": "make-com-safe --output json auth check",
            "steps": steps,
        },
    }
    ...
    if str(getattr(args, "output", "json")) == "json":
        ...
        print("")
        print("Next: make-com-safe --output json auth check")
    return 0
