...
NO_SNAPSHOT_NOTE = (
    "OpenAI Ads does not guarantee a safe before-state read for this operation from the pinned "
...
def _validate_required(
    ...
    *,
    ...
    for param in op.get("parameters") or []:
        ...
        if location == "header" and name not in header_params:
...
def _headers(ctx: dict[str, Any], header_params: dict[str, str] | None = None) -> dict[str, str]:
    cfg = ctx["cfg"]
    if not cfg.api_key:
        raise ValidationError("Missing OPENAI_ADS_API_KEY")
    headers = {
    ...
    for key, value in (header_params or {}).items():
        ...
        if lowered in {"authorization", "cookie", "set-cookie"}:
            raise ValidationError(f"Refused unsafe header override: {key}")
        headers[key] = value
    return headers
...
def _request(
    ...
    *,
    ...
    response = _client(ctx).request(
        ...
        headers=_headers(ctx, header_params),
...
def _redact_response(op: dict[str, Any], parsed: Any) -> Any:
    ...
    if family not in {"custom-audiences", "conversions"}:
        return safe
...
def _snapshot(ctx: dict[str, Any], op: dict[str, Any], *, path_params: dict[str, str]) -> dict[str, Any]:
    ...
    try:
        before = _request(ctx, snap_op, path_params=path_params, query={}, header_params={}, body=None)
...
def _body_risk_reasons(body: Any) -> list[str]:
    reasons: set[str] = set()
    ...
    def visit(value: Any, parent_key: str = "") -> None:
        ...
        if isinstance(value, dict):
            for raw_key, inner in value.items():
                lowered_key = str(raw_key).lower()
                if any(marker in lowered_key for marker in ("budget", "bid", "spend", "billing")):
                    reasons.add("spend-risk")
                if any(marker in lowered_key for marker in ("status", "state", "active", "enabled", "serving")):
                    reasons.add("serving-status-change")
                if any(
                    ...
                    reasons.add("targeting-change")
                if any(marker in lowered_key for marker in ("upload", "file", "audience", "email", "external_id", "customer_id", "hashed_")):
                    reasons.add("upload-or-audience-data")
                if any(marker in lowered_key for marker in ("ad_account", "account_id", "account")):
                    reasons.add("account-level-change")
                if any(marker in lowered_key for marker in ("api_key", "token", "secret", "authorization", "credential")):
                    reasons.add("auth-or-secret-risk")
                if any(marker in lowered_key for marker in ("conversion", "pixel", "event", "source_url", "oppref", "measurement")):
                    reasons.add("measurement-change")
                visit(inner, lowered_key)
            return
        if isinstance(value, list):
            for item in value:
                visit(item, key)
        ...
        if isinstance(value, str):
            ...
            if key in {"status", "state"} and lowered_value in {"active", "enabled", "live", "running", "paused", "archived"}:
                reasons.add("serving-status-change")
            if key in {"active", "enabled", "is_active"}:
                reasons.add("serving-status-change")
...
def _risk_reasons(op: dict[str, Any], *, body: Any = None) -> list[str]:
    text = " ".join([str(op.get("family_slug") or ""), str(op.get("command") or ""), str(op.get("path") or ""), str(op.get("summary") or "")]).lower()
    reasons = ["openai-ads-write", f"method:{str(op.get('method')).upper()}"]
    for marker, reason in (
        ("activate", "serving-status-change"),
        ("pause", "serving-status-change"),
        ("archive", "serving-status-change"),
        ("budget", "spend-risk"),
        ("bid", "spend-risk"),
        ("spend", "spend-risk"),
        ("bidding", "spend-risk"),
        ("upload", "upload-or-audience-data"),
        ("custom-audience", "upload-or-audience-data"),
        ("custom_audience", "upload-or-audience-data"),
        ("audience", "upload-or-audience-data"),
        ("conversion", "measurement-change"),
        ("pixel", "measurement-change"),
        ("api-key", "auth-or-secret-risk"),
        ("api_keys", "auth-or-secret-risk"),
        ("ad-account", "account-level-change"),
        ("ad_account", "account-level-change"),
        ("targeting", "targeting-change"),
        ("geo", "targeting-change"),
    ):
        if marker in text:
            reasons.append(reason)
    reasons.extend(_body_risk_reasons(body))
...
def _build_plan(
    ...
    *,
    ...
    reasons = _risk_reasons(op, body=body)
    ...
    return {
        ...
        "target": {
            "path_params": redact_pair_map(path_params),
            "query": redact_pair_map(query),
            "headers": redact_pair_map(header_params),
        },
        "target_sha256": _stable_sha256(
            {"path_params": path_params, "query": query, "headers": header_params}
        ),
        ...
        "verification_plan": {"type": "http-status-and-response", "notes": "After apply, inspect the OpenAI Ads response and follow with the closest read command when the API allows it."},
        "rollback": {"supported": False, "notes": "No generic rollback is promised for OpenAI Ads campaign, ad group, ad, audience, file, account, or conversion changes."},
    }
...
def _validate_plan(
    ...
    *,
    ...
    if str(plan.get("env_fingerprint") or "") != ctx["cfg"].base_url:
        raise SafetyError("Refused: plan base URL does not match current OPENAI_ADS_BASE_URL")
    ...
    if plan.get("target_sha256") != _stable_sha256(
        {"path_params": path_params, "query": query, "headers": header_params}
    ):
        raise SafetyError("Refused: path, query, or header target does not match the reviewed plan")
...
def cmd_api_operation(args: Any, ctx: dict[str, Any]) -> int:
    ...
    header_params = _parse_pairs(getattr(args, "header", None), label="--header")
    body = _load_body(args)
    _validate_required(
        op, path_params=path_params, query=query, header_params=header_params, body=body
    )

    if not operation_is_write(op):
        result = _request(
            ctx, op, path_params=path_params, query=query, header_params=header_params, body=body
        )
        payload = {
            "ok": True,
            "operation": op,
    ...
    if not bool(ctx.get("yes")):
        raise SafetyError("Refused: live OpenAI Ads writes require --apply --yes")
    if not ctx.get("plan_in"):
        raise SafetyError("Refused: live OpenAI Ads writes require --plan-in from the reviewed dry-run plan")
    ...
    if not bool((reviewed_plan.get("snapshot") or {}).get("available")) and not bool(ctx.get("ack_no_snapshot")):
        raise SafetyError("Refused: this OpenAI Ads write has no verified before-state snapshot; add --ack-no-snapshot only after reviewing that risk")
    if any(
        r in set(reviewed_plan.get("risk_reasons") or [])
        for r in {
            "serving-status-change",
            "spend-risk",
            "upload-or-audience-data",
    ...
    ) and not bool(ctx.get("ack_irreversible")):
        raise SafetyError("Refused: this high-risk OpenAI Ads write requires --ack-irreversible")
