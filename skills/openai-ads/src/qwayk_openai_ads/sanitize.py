...
PRIVATE_KEY_PATTERN = (
    r"(?:audience|contents|customer[_-]?id|email|external[_-]?id|first[_-]?name|"
    r"hashed[_-]?(?:email|phone)|ip[_-]?address|landing[_-]?page[_-]?url|last[_-]?name|"
    r"oppref|phone|pixel[_-]?id|source[_-]?url|user[_-]?(?:agent|id))"
)
...
def is_private_ads_name(name: str) -> bool:
    lowered = name.lower()
    return any(word in lowered for word in PRIVATE_FIELD_WORDS)
...
def looks_private_ads_value(value: Any) -> bool:
    if not isinstance(value, str):
        return False
    if re.search(EMAIL_PATTERN, value):
        return True
    lowered = value.lower()
    return any(marker in lowered for marker in ("customer_id=", "external_id=", "source_url=", "oppref="))


def redact_value(value: Any, *, key_name: str = "") -> Any:
    if key_name and (is_secret_name(key_name) or is_private_ads_name(key_name)):
        return REDACTED
    ...
    if looks_secret_value(value) or looks_private_ads_value(value):
        return REDACTED
    return value
...
def redact_text(text: str) -> str:
    ...
    safe = re.sub(
        rf"((?:\"|')\s*{PRIVATE_KEY_PATTERN}\s*(?:\"|')\s*:\s*(?:\"|'))(.*?)(\"|')",
        redact_quoted_assignment,
        safe,
        flags=re.IGNORECASE,
    )
    safe = re.sub(
        rf"((?:\"|')\s*{PRIVATE_KEY_PATTERN}\s*(?:\"|')\s*:\s*)([^,\}}\]\s]+)",
        redact_assignment,
        safe,
        flags=re.IGNORECASE,
    )
    safe = re.sub(
        rf"(\b{PRIVATE_KEY_PATTERN}\b\s*:\s*)([^\s,;\}}\]]+)",
        redact_assignment,
        safe,
        flags=re.IGNORECASE,
    )
    safe = re.sub(
        rf"({PRIVATE_KEY_PATTERN}=)"
        r"[^&\s\"'<>]+",
        redact_assignment,
        safe,
        flags=re.IGNORECASE,
    )
    safe = re.sub(EMAIL_PATTERN, REDACTED, safe)
...
def redact_argv(argv: list[str]) -> list[str]:
    ...
    pair_flags = {"--path-param", "--query", "--header"}
    opaque_flags = {
        "--body-json": "[REDACTED_BODY_JSON]",
        "--body-file": "[REDACTED_BODY_FILE]",
        "--env-file": "[REDACTED_ENV_FILE]",
        "--events-json": "[REDACTED_EVENTS_JSON]",
        "--events-file": "[REDACTED_EVENTS_FILE]",
        "--pixel-id": "[REDACTED_PIXEL_ID]",
    }
...
def redact_command(argv: list[str]) -> str:
    return "openai-ads-safe-agent-cli " + " ".join(redact_argv(argv))
