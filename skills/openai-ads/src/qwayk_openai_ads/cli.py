...
from .commands import measurement as measurement_cmd
from .commands import onboarding as onboarding_cmd
from .config import load_config
from .errors import SafetyError, ToolError, ValidationError
from .inventory import load_inventory, operation_is_write, operations_by_family
...
from .sanitize import redact_command, redact_text
...
def build_parser() -> argparse.ArgumentParser:
    p = _ToolArgumentParser(prog="openai-ads-safe-agent-cli")
    ...
    p.add_argument("--ack-no-snapshot", action="store_true", help="Approve applying a reviewed plan without verified before-state")
    p.add_argument(
        "--ack-irreversible",
        action="store_true",
    ...
    api = sub.add_parser("api", help="Official OpenAI Ads Advertiser API operations")
    api_sub = api.add_subparsers(dest="api_family", required=True, parser_class=_ToolArgumentParser)
    api_list = api_sub.add_parser("list", help="List official OpenAI Ads operation commands")
    api_list.set_defaults(func=api_cmd.cmd_api_list, write_capable=False)
    for family, ops in operations_by_family(load_inventory()).items():
        fam = api_sub.add_parser(family, help=f"{family} operations")
        fam_sub = fam.add_subparsers(dest="api_command", required=True, parser_class=_ToolArgumentParser)
        for op in ops:
            cmd = fam_sub.add_parser(str(op["command"]), help=str(op.get("summary") or op["operation_id"]))
            cmd.add_argument("--path-param", action="append", default=[], metavar="NAME=VALUE", help="Path parameter, repeatable")
            cmd.add_argument("--query", action="append", default=[], metavar="NAME=VALUE", help="Query parameter, repeatable")
            cmd.add_argument("--header", action="append", default=[], metavar="NAME=VALUE", help="Documented non-auth header, repeatable")
            cmd.add_argument("--body-json", default=None, help="JSON request body")
            cmd.add_argument("--body-file", default=None, help="JSON request body file")
            cmd.set_defaults(
                func=api_cmd.cmd_api_operation,
                write_capable=operation_is_write(op),
                api_family=family,
                api_command=str(op["command"]),
            )

    measurement = sub.add_parser("measurement", help="OpenAI Ads measurement helpers")
    measurement_sub = measurement.add_subparsers(
        dest="measurement_cmd", required=True, parser_class=_ToolArgumentParser
    )
    events_list = measurement_sub.add_parser("events-list", help="List supported conversion events")
    events_list.set_defaults(func=measurement_cmd.cmd_events_list, write_capable=False)
    pixel_guide = measurement_sub.add_parser("pixel-guide", help="Show JavaScript Pixel setup guidance")
    pixel_guide.set_defaults(func=measurement_cmd.cmd_pixel_guide, write_capable=False)
    image_tag = measurement_sub.add_parser("image-tag-build", help="Build a redacted image tag URL")
    image_tag.add_argument("--pixel-id", default=None, help="Pixel ID override")
    image_tag.add_argument("--event", default="page_viewed", help="Supported event name")
    image_tag.add_argument("--data-type", default="contents", help="Supported event data type")
    image_tag.add_argument("--event-id", default=None, help="Optional event id for deduplication")
    image_tag.add_argument("--custom-event-name", default=None, help="Required for custom events")
    image_tag.add_argument("--data", action="append", default=[], metavar="NAME=VALUE", help="Event data field")
    image_tag.add_argument("--noscript", action="store_true", help="Wrap the image tag in noscript")
    image_tag.set_defaults(func=measurement_cmd.cmd_image_tag_build, write_capable=False)
    conversions_send = measurement_sub.add_parser(
        "conversions-send", help="Plan or send server-side conversion events"
    )
    conversions_send.add_argument("--pixel-id", default=None, help="Pixel ID override")
    conversions_send.add_argument("--events-json", default=None, help="JSON event list or object with events")
    conversions_send.add_argument("--events-file", default=None, help="JSON file with event list or events object")
    conversions_send.add_argument(
        "--validate-only",
        action="store_true",
        default=True,
        help="Send validation-only payload when applying (default behavior)",
    )
    conversions_send.add_argument(
        "--real-events",
        action="store_true",
        help="Send real conversion events instead of validate_only",
    )
    conversions_send.set_defaults(func=measurement_cmd.cmd_conversions_send, write_capable=True)

    product_feeds = sub.add_parser("product-feeds", help="Product-feed API guidance")
    product_feeds_sub = product_feeds.add_subparsers(
        dest="product_feeds_cmd", required=True, parser_class=_ToolArgumentParser
    )
    product_feeds_guide = product_feeds_sub.add_parser("guide", help="Explain product-feed API workflow")
    product_feeds_guide.set_defaults(func=measurement_cmd.cmd_product_feeds_guide, write_capable=False)

    targeting = sub.add_parser("targeting", help="Campaign targeting guidance")
    targeting_sub = targeting.add_subparsers(
        dest="targeting_cmd", required=True, parser_class=_ToolArgumentParser
    )
    targeting_guide = targeting_sub.add_parser("guide", help="Explain targeting workflow")
    targeting_guide.set_defaults(func=measurement_cmd.cmd_targeting_guide, write_capable=False)
...
def main(argv: list[str]) -> int:
    ...
    command_str = redact_command(argv)
    try:
    ...
    try:
        if bool(args.version):
            payload = {"ok": True, "tool": "openai-ads-safe-agent-cli", "version": __version__}
            if args.output == "json":
                out.emit(payload)
            else:
                print(f"openai-ads-safe-agent-cli {__version__}")
            return 0
        ...
        audit.bind_context(
            {
                "tool": "openai-ads-safe-agent-cli",
                "version": __version__,
                "command": command_str,
        ...
        local_only = str(getattr(args, "cmd", "") or "") in {
            "runs",
            "onboarding",
            "product-feeds",
            "targeting",
        } or (
            str(getattr(args, "cmd", "") or "") == "api"
            and str(getattr(args, "api_family", "") or "") == "list"
        ) or (
            str(getattr(args, "cmd", "") or "") == "measurement"
            and str(getattr(args, "measurement_cmd", "") or "") in {"events-list", "pixel-guide"}
        )
        if local_only:
            ctx = {
                "cfg": None,
        ...
        audit.write("refused", {"reason": safe_error})
        out.emit({"ok": True, "refused": True, "reasons": [safe_error], "refusal_type": "SafetyError"})
        _finalize_run_artifacts(
            run_ctx=run_ctx,
            tool="openai-ads-safe-agent-cli",
            version=__version__,
