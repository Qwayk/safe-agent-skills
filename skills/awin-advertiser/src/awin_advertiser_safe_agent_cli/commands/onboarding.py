...
def cmd_onboarding(args: object, ctx: dict) -> int:
    ...
    missing: list[str] = []
    if not _has_nonempty_env_value(lines, "AWIN_API_BASE_URL"):
        missing.append("AWIN_API_BASE_URL")
    if not _has_nonempty_env_value(lines, "AWIN_API_TOKEN"):
        missing.append("AWIN_API_TOKEN")
    if not _has_nonempty_env_value(lines, "AWIN_ADVERTISER_ID"):
        missing.append("AWIN_ADVERTISER_ID")
    ...
    steps = [
        ...
        "  - AWIN_ADVERTISER_ID=<your advertiser id>",
        "  - AWIN_API_TIMEOUT_S=30 (optional)",
        "Run: awin-advertiser-safe-cli --output json auth check",
    ]
    ...
    payload = {
        ...
        "onboarding": {
            ...
            "missing": missing,
            "next_command": "awin-advertiser-safe-cli --output json auth check",
            "steps": steps,
        },
    }
    ...
    if str(getattr(args, "output", "json")) == "json":
        ...
        print("To connect this tool, run:")
        ...
        print("")
        print("Next: awin-advertiser-safe-cli --output json auth check")
    return 0
