...
class TestDocsFormatting(unittest.TestCase):
    def test_no_double_bullet_lines(self) -> None:
        ...
        patterns = [root / "README.md", root / "docs"]
        bad_lines: list[str] = []
        for p in patterns:
            if p.is_file():
                files = [p]
    ...
    def test_use_cases_stays_human_and_specific(self) -> None:
        ...
        required = [
            "Awin Advertiser is useful when a brand needs to understand",
        ...
        stale_phrases = [
            "Awin Advertiser work usually starts with performance and control questions",
