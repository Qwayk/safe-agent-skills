...
Amazon Product Advertising API is easiest to use when you start with the real job, then check setup, safety, and proof before asking an agent to do more. These docs explain how to look up Amazon products, offers, images, and product metadata, what access is needed, and where the tool stops before risky changes.
...
## Start with the work
- [What you can do with Amazon Product Advertising API](use_cases.md) - real jobs to hand to an agent for Amazon Product Advertising API
- [Quickstart](quickstart.md) - the shortest path to one useful first result
- [Connect your Amazon Product Advertising API account](onboarding.md) - what must be connected or confirmed before the first run
