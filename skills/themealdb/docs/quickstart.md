...
Start with a small TheMealDB read: checking recipes, ingredients, categories, and meal ideas from public data.

Need more ideas? See [What you can do with TheMealDB](use_cases.md). Need setup help? See [Use TheMealDB with no account](onboarding.md).
...
TheMealDB is read-only here and needs no account for the first read. The useful limit is recipe coverage: ask the agent to show a few options before choosing one.
...
- For setup details, read [Use TheMealDB with no account](onboarding.md).
