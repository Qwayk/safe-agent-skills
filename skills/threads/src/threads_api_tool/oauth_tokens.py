...
def write_token_from_file(*, src_file: Path, dest_file: Path) -> TokenStatus:
    ...
    if not isinstance(data, dict):
        raise RuntimeError("Token file must be a JSON object")
    return write_token_payload(payload=data, dest_file=dest_file)


def write_token_payload(*, payload: dict[str, Any], dest_file: Path) -> TokenStatus:
    if not isinstance(payload, dict):
        raise RuntimeError("Token payload must be a JSON object")
    dest_file.parent.mkdir(parents=True, exist_ok=True)
    dest_file.write_text(json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")
...
def read_access_token(path: Path) -> str | None:
    ...
    if not isinstance(data, dict):
        return None
    for key in ("access_token", "token", "oauth_token"):
        value = data.get(key)
