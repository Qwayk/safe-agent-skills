...
def _demo_write(row: dict[str, str], ctx: dict[str, Any]) -> dict[str, Any]:
    # Demo write action: does nothing and is still requires explicit no-snapshot approval before apply receipts.
    apply = bool(ctx.get("apply"))
    if not apply:
        return {"ok": True, "dry_run": True, "plan": {"would_write": True, "row": row}}
    raise SafetyError("Refused: jobs write action requires explicit no-snapshot approval before demo/job writes or receipt output")
    ...
    return {
        ...
        "before_state": blocked_before_state(
            ...
            proposed_changes=[
                {"row": idx, "action": str(r.get("action") or "").strip(), "input": r}
                for idx, r in enumerate(rows, start=1)
                if _ACTIONS.get(str(r.get("action") or "").strip(), ActionSpec("", False, _demo_read)).is_write
            ],
            local_state={"kind": "jobs_output", "writes_receipts": True} if _has_write_actions(rows) else None,
        ),
        "verification_plan": before_state_refusal_verification_plan()
        if _has_write_actions(rows)
        else {"type": "none", "notes": "Read-only job batch."},
