...
def token_path_for_env_file(env_file: str) -> Path:
    """
    Store OAuth tokens next to the env file (per-environment), under `.state/oauth_credentials.json`.
    """
    root = Path(env_file).resolve().parent
    return root / ".state" / "oauth_credentials.json"
...
def write_token_data(*, dest_file: Path, data: dict[str, Any]) -> TokenStatus:
    if not isinstance(data, dict):
        raise RuntimeError("OAuth token data must be a JSON object")
    dest_file.parent.mkdir(parents=True, exist_ok=True)
    dest_file.write_text(
        json.dumps(data, ensure_ascii=False, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
