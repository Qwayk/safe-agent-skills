...
Meta Ads coverage shows exactly what the shipped commands can do with ad accounts, campaigns, ad sets, ads, insights, and Graph API inventory. Start here when an ask sounds possible but you need to know whether it is already shipped, read-only, plan-first, gated, excluded, or outside the tool.
...
A good first coverage check is: "Check whether the shipped commands can inspect campaigns, ad sets, and insights, then show which Meta Ads changes are covered."
