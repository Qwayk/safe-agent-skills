...
Meta Ads proof should answer a simple question: what has actually been checked for ad accounts, campaigns, ad sets, ads, insights, and Graph API inventory, and what still depends on account access, provider permissions, public API limits, or reviewer judgment?

You do not need to run every command before using the skill. Start with the evidence that matters most: the last verified date, the smoke checks, the saved example outputs, and the known failure cases.

If you only check one thing, check campaign, ad set, ad, and insights examples before trusting reporting summaries.
