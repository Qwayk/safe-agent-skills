...
from openai_api_tool.commands.jobs import cmd_jobs_run
...
class TestJobs(unittest.TestCase):
    def _ctx(self, **overrides):
        ctx = {
            ...
            "tool_version": "0.0.0",
            "command_str": "openai-api-tool jobs run",
            "apply": False,
            "yes": False,
    ...
    def test_write_action_requires_apply_yes(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            ctx = self._ctx(apply=True, yes=False, command_str="openai-api-tool --apply jobs run")
            buf = io.StringIO()
    ...
    def test_plan_out_and_plan_in_apply_refuses_without_before_state(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertIn("recovery", plan_obj)
            before_state = plan_obj.get("before_state")
            ...
            self.assertEqual(plan_obj["verification_plan"]["type"], "best_effort_after_apply")
            ...
            ctx_apply = self._ctx(
                ...
                command_str="openai-api-tool --apply --yes --plan-in plan.json --file jobs.csv jobs run",
            )
            buf2 = io.StringIO()
