...
def _build_jobs_plan(*, job_file: str | None, rows: list[dict[str, Any]], ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    if has_writes:
        plan["before_state"] = _build_before_state_contract(operation="jobs.run", target=selector["value"])
        plan["verification_plan"] = _build_before_state_refusal_verification_plan()
        plan["post_apply_verification_plan"] = {
            "type": "per-row",
            "notes": (
                "After approved apply, verify writes via read-back when available "
                "or idempotence checks per row."
            ),
        }
    return plan
