...
def build_parser() -> argparse.ArgumentParser:
    p = _ToolArgumentParser(prog="openai-api-tool")
    ...
    p.add_argument(
        "--ack-spend-money",
        action="store_true",
        help="Extra acknowledgement for spend-money actions (model inference/generation)",
    )
    ...
    api = sub.add_parser("api", help="Pinned OpenAI API operations (plan-only by default)")
    ...
    for op in sorted(ops, key=lambda o: (o.operation_command, o.method, o.path)):
...
def main(argv: list[str]) -> int:
    ...
    try:
        if bool(args.version):
            payload = {"ok": True, "tool": "openai-api-tool", "version": __version__}
            if args.output == "json":
                out.emit(payload)
            else:
                print(f"openai-api-tool {__version__}")
            return 0

        if not getattr(args, "cmd", None):
            parser.error("Missing command. Use --help to see available commands.")

        command_str = "openai-api-tool " + " ".join(argv)
        audit.bind_context(
            {
        ...
        _finalize_run_artifacts(
            ...
            tool="openai-api-tool",
            version=__version__,
            command="openai-api-tool " + " ".join(argv),
            env_fingerprint=None,
            output_obj=out.last if isinstance(out.last, dict) else None,
            audit_log_path=run_audit_log_path or global_audit_log_path,
            audit_log_global_path=global_audit_log_path,
