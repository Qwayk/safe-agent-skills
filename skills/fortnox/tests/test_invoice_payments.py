...
class TestInvoicePayments(unittest.TestCase):
    ...
    def test_invoice_payments_create_dry_run_emits_plan(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            with patch("fortnox_api_tool.commands.invoice_payments.request_json") as request_json:
                rc, payload = self._run(
                    env_path=env_path,
                    args=["invoice-payments", "create", "--json-file", str(payload_path)],
                )
    ...
    def test_invoice_payments_create_apply_rechecks_json_payload_hash(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
    ...
    def test_invoice_payments_create_apply_performs_post_and_get_verify(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
            ...
            with patch("fortnox_api_tool.commands.invoice_payments.request_json") as request_json:
                ...
                request_json.side_effect = [
                    _api_response(
                        ...
                        body={"InvoicePayment": {"Number": "42"}},
                    ),
                    _api_response(
                        status=200,
                        path="/invoicepayments/42",
                        body={"InvoicePayment": {"Number": "42", "Booked": False}},
                    ),
                ]
                rc_apply, payload_apply = self._run(
                    env_path=env_path,
    ...
    def test_invoice_payments_update_apply_performs_put_and_get_verify(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
    ...
    def test_invoice_payments_update_dry_run_emits_plan(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
    ...
    def test_invoice_payments_update_apply_rechecks_json_payload_hash(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
    ...
    def test_invoice_payments_update_rejects_number_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
    ...
    def test_invoice_payments_remove_apply_refuses_without_ack(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
            ...
            with patch("fortnox_api_tool.commands.invoice_payments.request_json") as request_json:
                rc, plan_payload = self._run(
                    env_path=env_path,
                    args=["invoice-payments", "remove", "--number", "42"],
                )
                self.assertEqual(rc, 0)
                plan = self._plan_from_output(plan_payload)
                plan_path = Path(td) / "plan.json"
    ...
    def test_invoice_payments_remove_apply_refuses_without_yes(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
    ...
    def test_invoice_payments_remove_apply_performs_delete_and_404_verify(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
            ...
            with patch("fortnox_api_tool.commands.invoice_payments.request_json") as request_json:
                ...
                request_json.side_effect = [
                    _api_response(status=204, path="/invoicepayments/42", body={}),
                    RuntimeError("HTTP 404 for GET https://api.fortnox.se/3/invoicepayments/42"),
                ]
                rc_apply, payload_apply = self._run(
                    env_path=env_path,
                    args=[
    ...
    def test_invoice_payments_bookkeep_dry_run_emits_plan(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
    ...
    def test_invoice_payments_bookkeep_apply_refuses_without_yes(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
    ...
    def test_invoice_payments_bookkeep_rejects_number_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
    ...
    def test_invoice_payments_bookkeep_apply_rechecks_json_payload_hash(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
    ...
    def test_invoice_payments_bookkeep_apply_performs_put_and_verifies_booked(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
            ...
            with patch("fortnox_api_tool.commands.invoice_payments.request_json") as request_json:
                ...
                request_json.side_effect = [
                    _api_response(
                        status=200,
                        path="/invoicepayments/42/bookkeep",
                        body={"InvoicePayment": {"Number": "42", "Booked": True}},
                    ),
                    _api_response(
                        status=200,
                        path="/invoicepayments/42",
            ...
            self.assertTrue(payload_apply.get("ok", False))
            self.assertEqual(request_json.call_args_list[0].kwargs["method"], "PUT")
            self.assertEqual(request_json.call_args_list[0].kwargs["path"], "/invoicepayments/42/bookkeep")
            self.assertEqual(request_json.call_args_list[1].kwargs["method"], "GET")
            self.assertEqual(request_json.call_args_list[1].kwargs["path"], "/invoicepayments/42")

    def test_invoice_payments_bookkeep_apply_fails_when_verify_is_not_booked(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
