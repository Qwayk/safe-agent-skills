...
class TestPriceLists(unittest.TestCase):
    ...
    def _make_payload_file(self, path: Path, *, code: str = "PL-1000") -> None:
        payload = {
            "PriceList": {
                "Code": code,
    ...
    def test_price_lists_create_dry_run_emits_plan(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            self._make_payload_file(payload_path, code="PL-1000")

            with patch("fortnox_api_tool.commands.price_lists.request_json") as request_json:
                rc, payload = self._run(
                    env_path=env_path,
                    args=["price-lists", "create", "--json-file", str(payload_path)],
                )
    ...
    def test_price_lists_create_apply_rechecks_json_payload_hash(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
                    ...
                    json.dumps(
                        {
                            "PriceList": {
                                "Code": "PL-1000",
                                "Description": "Updated sample price list",
                            }
                        },
                        indent=2,
                    ),
    ...
    def test_price_lists_create_apply_uses_response_or_payload_code_for_verification(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
            ...
            with patch("fortnox_api_tool.commands.price_lists.request_json") as request_json:
                ...
                request_json.side_effect = [
                    _api_response(
                        status=201,
                        path="/pricelists",
                        body={"PriceList": {"Code": "PL-2000"}},
                    ),
                    _api_response(
                        status=200,
                        path="/pricelists/PL-2000",
                        body={"PriceList": {"Code": "PL-2000"}},
                    ),
                ]
                rc_apply, payload_apply = self._run(
            ...
            self.assertEqual(request_json.call_args_list[0].kwargs["path"], "/pricelists")
            self.assertEqual(
            ...
            self.assertEqual(request_json.call_args_list[1].kwargs["path"], "/pricelists/PL-2000")

    def test_price_lists_create_apply_fails_without_code_on_verification(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
    ...
    def test_price_lists_update_dry_run_emits_plan(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
            ...
            self.assertTrue(payload["ok"])
            self.assertTrue(payload["dry_run"])
            self.assertEqual(payload["plan"].get("selector", {}).get("code"), "PL-1000")
            self.assertEqual(request_json.call_count, 0)

    def test_price_lists_create_rejects_missing_top_level_price_list(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
            ...
            self.assertIn("top-level PriceList object", payload["error"])

    def test_price_lists_update_apply_rechecks_json_payload_hash(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
    ...
    def test_price_lists_update_apply_performs_put_get_verify(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
            ...
            with patch("fortnox_api_tool.commands.price_lists.request_json") as request_json:
                ...
                request_json.side_effect = [
                    _api_response(
                        status=200,
                        path="/pricelists/PL-1000",
                        body={"PriceList": {"Code": "PL-1000"}},
                    ),
                    _api_response(
                        status=200,
                        path="/pricelists/PL-1000",
            ...
            self.assertEqual(request_json.call_args_list[0].kwargs["path"], "/pricelists/PL-1000")
            self.assertEqual(
                request_json.call_args_list[0].kwargs["json_body"],
                {
                    "PriceList": {
            ...
            self.assertEqual(request_json.call_args_list[1].kwargs["method"], "GET")
            self.assertEqual(request_json.call_args_list[1].kwargs["path"], "/pricelists/PL-1000")

    def test_price_lists_update_rejects_code_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
            ...
            payload_path = Path(td) / "update.json"
            self._make_payload_file(payload_path, code="PL-2000")
