...
class TestRunArtifacts(unittest.TestCase):
    def _write_env(self, root: Path) -> Path:
        ...
        env_path.write_text(
            "\n".join(
                [
                    "FORTNOX_API_BASE_URL=http://example.invalid",
                    "FORTNOX_TIMEOUT_S=30",
                ]
            )
            + "\n",
            encoding="utf-8",
    ...
    def _write_unit_payload(self, root: Path) -> Path:
        payload_path = root / "unit.json"
        payload_path.write_text(
            json.dumps({"Unit": {"Code": "PCS", "Description": "Pieces"}}, indent=2),
            encoding="utf-8",
        )
        return payload_path
    ...
    def test_write_dry_run_creates_run_folder_and_index(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            payload_path = self._write_unit_payload(root)
