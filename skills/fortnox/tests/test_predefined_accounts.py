...
class TestPredefinedAccounts(unittest.TestCase):
    ...
    def _make_payload_file(self, path: Path, *, name: str = "4000") -> None:
        payload = {
            "PreDefinedAccount": {
                "Name": name,
                "Description": "Test predefined account",
            }
        }
        path.write_text(json.dumps(payload, indent=2), encoding="utf-8")

    def test_predefined_accounts_update_dry_run_emits_plan(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            self._make_payload_file(payload_path, name="Expense")
            expected_hash = _sha256(payload_path)

            rc, payload = self._run(
                env_path=env_path,
                args=["predefined-accounts", "update", "--name", "Expense", "--json-file", str(payload_path)],
            )
    ...
    def test_predefined_accounts_update_rechecks_json_payload_hash(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
            ...
            with patch("fortnox_api_tool.commands.predefined_accounts.request_json") as request_json:
                    ...
                    json.dumps(
                        {
                            "PreDefinedAccount": {
                                "Name": "Expense",
                                "Description": "Updated predefined account",
                            }
                        },
                        indent=2,
                    ),
    ...
    def test_predefined_accounts_update_apply_performs_put_get_verify(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
            ...
            with patch("fortnox_api_tool.commands.predefined_accounts.request_json") as request_json:
                ...
                request_json.side_effect = [
                    _api_response(
                        status=200,
                        path="/predefinedaccounts/Expense",
                        body={"PreDefinedAccount": {"Name": "Expense"}},
                    ),
                    _api_response(
                        status=200,
                        path="/predefinedaccounts/Expense",
        ...
        self.assertEqual(request_json.call_args_list[0].kwargs["path"], "/predefinedaccounts/Expense")
        self.assertEqual(request_json.call_args_list[1].kwargs["method"], "GET")
        self.assertEqual(request_json.call_args_list[1].kwargs["path"], "/predefinedaccounts/Expense")

    def test_predefined_accounts_update_rejects_name_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
