...
class TestContractTemplates(unittest.TestCase):
    ...
    def _make_payload_file(self, path: Path, *, template_number: str = "2001") -> None:
        payload = {
            "ContractTemplate": {
    ...
    def test_contract_templates_list(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            with patch(
                "fortnox_api_tool.commands.contract_templates.get_json",
                ...
                rc, payload = self._run(env_path=env_path, args=["contract-templates", "list"])

        self.assertEqual(rc, 0)
        self.assertTrue(payload["ok"])
        self.assertEqual(payload["path"], "/contracttemplates")

    def test_contract_templates_get(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
            ...
            with patch(
                ...
                return_value=_api_response(
                    status=200,
                    path="/contracttemplates/2001",
                    body={"ContractTemplate": {"TemplateNumber": 2001}},
                ),
            ):
                rc, payload = self._run(
                    env_path=env_path,
                    args=["contract-templates", "get", "--template-number", "2001"],
                )

        self.assertEqual(rc, 0)
        self.assertTrue(payload["ok"])
        self.assertEqual(payload["path"], "/contracttemplates/2001")

    def test_contract_templates_create_dry_run_emits_plan(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
            ...
            with patch("fortnox_api_tool.commands.contract_templates.request_json") as request_json:
                rc, payload = self._run(
                    env_path=env_path,
                    args=["contract-templates", "create", "--json-file", str(payload_path)],
                )
    ...
    def test_contract_templates_create_apply_rechecks_json_payload_hash(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
            ...
            with patch("fortnox_api_tool.commands.contract_templates.request_json") as request_json:
                ...
                changed["ContractTemplate"]["TemplateName"] = "Changed template"
    ...
    def test_contract_templates_create_apply_performs_post_and_get_verify(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
        ...
        self.assertEqual(request_json.call_args_list[0].kwargs["path"], "/contracttemplates")
        self.assertEqual(request_json.call_args_list[1].kwargs["method"], "GET")
        self.assertEqual(request_json.call_args_list[1].kwargs["path"], "/contracttemplates/2001")

    def test_contract_templates_update_selector_mismatch_validation(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
            ...
            self._make_payload_file(payload_path, template_number="2002")
    ...
    def test_contract_templates_update_apply_performs_put_and_get_verify(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            env_path = Path(td) / ".env"
            env_path.write_text(
                "FORTNOX_API_BASE_URL=https://api.fortnox.se/3\nFORTNOX_API_TOKEN=token\n",
        ...
        self.assertEqual(request_json.call_args_list[0].kwargs["path"], "/contracttemplates/2001")
        self.assertEqual(request_json.call_args_list[1].kwargs["method"], "GET")
        self.assertEqual(request_json.call_args_list[1].kwargs["path"], "/contracttemplates/2001")
