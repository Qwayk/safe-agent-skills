...
def _load_payload_file(path_str: str) -> tuple[Path, dict[str, Any], dict[str, Any]]:
    ...
    invoice_accrual = obj.get("InvoiceAccrual")
    if not isinstance(invoice_accrual, dict):
        raise ValidationError("JSON file must contain a top-level InvoiceAccrual object")
    return path, obj, invoice_accrual


def _extract_invoice_number_from_payload(invoice_accrual: dict[str, Any]) -> str | None:
    raw = invoice_accrual.get("InvoiceNumber")
    if raw is None:
        return None
    value = str(raw).strip()
...
def _extract_invoice_number_from_response(body: dict[str, Any] | None) -> str | None:
    if not isinstance(body, dict):
        return None
    invoice_accrual = body.get("InvoiceAccrual")
    if not isinstance(invoice_accrual, dict):
        return None
    return _extract_invoice_number_from_payload(invoice_accrual)
...
def _verify_present(*, ctx: dict[str, Any], invoice_number: str) -> dict[str, Any]:
    path = f"/invoiceaccruals/{invoice_number}"
    try:
...
def _verify_removed(*, ctx: dict[str, Any], invoice_number: str) -> dict[str, Any]:
    path = f"/invoiceaccruals/{invoice_number}"
    try:
        payload = request_json(ctx=ctx, method="GET", path=path, expect_json=True)
    except Exception as e:  # noqa: BLE001
    ...
    return {
        ...
        "http_status": payload["status"],
        "data": payload["body"],
        "error": "Expected invoice accrual to be absent after delete verification",
    }


def cmd_invoice_accruals_list(args: Any, ctx: dict[str, Any]) -> int:
    _ = args
    path = "/invoiceaccruals"
    payload = get_json(ctx=ctx, path=path)
    return _emit_read(ctx, audit_key="invoice_accruals.list", path=path, payload=payload)


def cmd_invoice_accruals_get(args: Any, ctx: dict[str, Any]) -> int:
    invoice_number = str(getattr(args, "invoice_number", "") or "").strip()
    path = f"/invoiceaccruals/{invoice_number}"
    payload = get_json(ctx=ctx, path=path)
    return _emit_read(ctx, audit_key="invoice_accruals.get", path=path, payload=payload)


def cmd_invoice_accruals_create(args: Any, ctx: dict[str, Any]) -> int:
    payload_file, payload_obj, invoice_accrual = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    selector = {"kind": "invoice-accrual", "action": "create", "path": "/invoiceaccruals"}
    plan = _build_plan(
        action="create",
        selector=selector,
        ...
        risk_level="medium",
        risk_reasons=["fortnox-write", "invoice-accrual-create"],
        verification_plan={"type": "read-after-write", "path_template": "/invoiceaccruals/{InvoiceNumber}"},
        rollback_notes="No generic rollback. Recreate or update the invoice accrual explicitly if needed.",
        ctx=ctx,
    )
    plan_path = _write_plan_if_requested(ctx, plan)
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("invoice_accruals.create.plan", {"plan_out": plan_path})
        ctx["out"].emit(out)
        return 0
    ...
    invoice_number = _extract_invoice_number_from_response(payload["body"]) or _extract_invoice_number_from_payload(invoice_accrual)
    ...
    ctx["audit"].write("invoice_accruals.create.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1


def cmd_invoice_accruals_update(args: Any, ctx: dict[str, Any]) -> int:
    invoice_number = str(getattr(args, "invoice_number", "") or "").strip()
    payload_file, payload_obj, invoice_accrual = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    payload_invoice_number = _extract_invoice_number_from_payload(invoice_accrual)
    if payload_invoice_number and payload_invoice_number != invoice_number:
        raise ValidationError("InvoiceAccrual.InvoiceNumber in the JSON file must match --invoice-number")
    selector = {
        "kind": "invoice-accrual",
        "action": "update",
        "path": f"/invoiceaccruals/{invoice_number}",
    ...
    plan = _build_plan(
        ...
        payload_obj=payload_obj,
        risk_level="medium",
        risk_reasons=["fortnox-write", "invoice-accrual-update"],
        verification_plan={"type": "read-after-write", "path": f"/invoiceaccruals/{invoice_number}"},
    ...
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("invoice_accruals.update.plan", {"plan_out": plan_path, "invoice_number": invoice_number})
        ctx["out"].emit(out)
        return 0
    ...
    receipt_path = _write_receipt_if_requested(ctx, receipt)
    out = {"ok": bool(verification.get("ok")), "dry_run": False, "receipt": receipt, "receipt_out": receipt_path}
    ctx["audit"].write("invoice_accruals.update.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1


def cmd_invoice_accruals_remove(args: Any, ctx: dict[str, Any]) -> int:
    invoice_number = str(getattr(args, "invoice_number", "") or "").strip()
    selector = {
        "kind": "invoice-accrual",
        "action": "remove",
    ...
    plan = _build_plan(
        ...
        risk_reasons=["fortnox-write", "invoice-accrual-delete", "irreversible"],
        verification_plan={"type": "absence-check", "path": f"/invoiceaccruals/{invoice_number}", "expect_http_status": 404},
        rollback_notes="No generic rollback. Recreate the invoice accrual explicitly if you need to restore it.",
        ctx=ctx,
    )
    plan_path = _write_plan_if_requested(ctx, plan)
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("invoice_accruals.remove.plan", {"plan_out": plan_path, "invoice_number": invoice_number})
        ctx["out"].emit(out)
        return 0

    if not bool(ctx.get("yes")):
        raise SafetyError("Refused: deleting an invoice accrual requires --apply --yes")
    if not bool(ctx.get("ack_irreversible")):
        raise SafetyError("Refused: deleting an invoice accrual requires --ack-irreversible")
    plan = _load_plan_from_ctx(ctx)
    ...
    verification = _verify_removed(ctx=ctx, invoice_number=invoice_number)
    receipt = {
        "tool": ctx.get("tool") or "fortnox-api-tool",
        "version": ctx.get("tool_version") or None,
        "applied_at_utc": _utc_now(),
    ...
    receipt_path = _write_receipt_if_requested(ctx, receipt)
    out = {"ok": bool(verification.get("ok")), "dry_run": False, "receipt": receipt, "receipt_out": receipt_path}
    ctx["audit"].write("invoice_accruals.remove.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1
