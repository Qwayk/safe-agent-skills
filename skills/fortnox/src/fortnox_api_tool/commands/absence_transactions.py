...
def _load_payload_file(path_str: str) -> tuple[Path, dict[str, Any], dict[str, Any]]:
    ...
    transaction = obj.get("AbsenceTransaction")
    if not isinstance(transaction, dict):
        raise ValidationError("JSON file must contain a top-level AbsenceTransaction object")
...
def _extract_id_from_response(body: dict[str, Any] | None) -> str | None:
    if not isinstance(body, dict):
        return None
    transaction = body.get("AbsenceTransaction")
    if not isinstance(transaction, dict):
        return None
...
def _verify_absent(*, ctx: dict[str, Any], transaction_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "http_status": payload["status"],
        "data": payload["body"],
        "error": "Expected absence transaction to be absent after delete verification",
    }


def cmd_absence_transactions_create(args: Any, ctx: dict[str, Any]) -> int:
    payload_file, payload_obj, _transaction = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    selector = {"kind": "absence_transaction", "action": "create", "path": "/absencetransactions"}
    plan = _build_plan(
        action="create",
        selector=selector,
        ...
        risk_level="medium",
        risk_reasons=["fortnox-write", "absence-transaction-create"],
        verification_plan={"type": "read-after-write", "path_template": "/absencetransactions/{id}"},
        rollback_notes="No generic rollback. Recreate the absence transaction explicitly if needed.",
        ctx=ctx,
    )
    plan_path = _write_plan_if_requested(ctx, plan)
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("absence_transactions.create.plan", {"plan_out": plan_path})
        ctx["out"].emit(out)
        return 0
    ...
    payload = request_json(ctx=ctx, method="POST", path="/absencetransactions", json_body=payload_obj, expect_json=True)
    transaction_id = _extract_id_from_response(payload.get("body"))
    if not transaction_id:
        raise ValidationError("Could not determine absence-transaction id for create verification")
    ...
    ctx["audit"].write("absence_transactions.create.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1


def cmd_absence_transactions_update(args: Any, ctx: dict[str, Any]) -> int:
    transaction_id = str(getattr(args, "id", "") or "").strip()
    payload_file, payload_obj, _transaction = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    selector = {
        "kind": "absence_transaction",
        "action": "update",
        "path": f"/absencetransactions/{transaction_id}",
        "id": transaction_id,
    }
    plan = _build_plan(
        ...
        payload_obj=payload_obj,
        risk_level="medium",
        risk_reasons=["fortnox-write", "absence-transaction-update"],
        verification_plan={"type": "read-after-write", "path": f"/absencetransactions/{transaction_id}"},
    ...
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("absence_transactions.update.plan", {"plan_out": plan_path, "id": transaction_id})
        ctx["out"].emit(out)
        return 0
    ...
    receipt_path = _write_receipt_if_requested(ctx, receipt)
    out = {"ok": bool(verification.get("ok")), "dry_run": False, "receipt": receipt, "receipt_out": receipt_path}
    ctx["audit"].write("absence_transactions.update.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1


def cmd_absence_transactions_delete(args: Any, ctx: dict[str, Any]) -> int:
    transaction_id = str(getattr(args, "id", "") or "").strip()
    selector = {
        "kind": "absence_transaction",
        "action": "delete",
    ...
    plan = _build_plan(
        ...
        risk_reasons=["fortnox-write", "absence-transaction-delete", "irreversible"],
        verification_plan={"type": "absence-check", "path": f"/absencetransactions/{transaction_id}", "expect_http_status": 404},
        rollback_notes="No generic rollback. Recreate the absence transaction explicitly if needed.",
        ctx=ctx,
    )
    plan_path = _write_plan_if_requested(ctx, plan)
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("absence_transactions.delete.plan", {"plan_out": plan_path, "id": transaction_id})
        ctx["out"].emit(out)
        return 0

    if not bool(ctx.get("yes")):
        raise SafetyError("Refused: deleting an absence transaction requires --apply --yes")
    if not bool(ctx.get("ack_irreversible")):
        raise SafetyError("Refused: deleting an absence transaction requires --ack-irreversible")

    plan = _load_plan_from_ctx(ctx)
    _validate_plan_for_apply(plan, action="delete", selector=selector, payload_file=None, ctx=ctx)
    payload = request_json(ctx=ctx, method="DELETE", path=f"/absencetransactions/{transaction_id}", expect_json=False)
    ...
    receipt_path = _write_receipt_if_requested(ctx, receipt)
    out = {"ok": bool(verification.get("ok")), "dry_run": False, "receipt": receipt, "receipt_out": receipt_path}
    ctx["audit"].write("absence_transactions.delete.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1
