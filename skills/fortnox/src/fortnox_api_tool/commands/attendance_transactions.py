...
def _load_payload_file(path_str: str) -> tuple[Path, dict[str, Any], dict[str, Any]]:
    ...
    transaction = obj.get("AttendanceTransaction")
    if not isinstance(transaction, dict):
        raise ValidationError("JSON file must contain a top-level AttendanceTransaction object")
...
def _extract_id_from_response(body: dict[str, Any] | None) -> str | None:
    if not isinstance(body, dict):
        return None
    transaction = body.get("AttendanceTransaction")
    if not isinstance(transaction, dict):
        return None
    raw = transaction.get("id")
    if raw is None:
        return None
    value = str(raw).strip()
...
def _verify_present(*, ctx: dict[str, Any], transaction_id: str) -> dict[str, Any]:
...
def _verify_absent(*, ctx: dict[str, Any], transaction_id: str) -> dict[str, Any]:
    path = f"/attendancetransactions/{transaction_id}"
    try:
        payload = request_json(ctx=ctx, method="GET", path=path, expect_json=True)
    except Exception as e:  # noqa: BLE001
    ...
    return {
        ...
        "http_status": payload["status"],
        "data": payload["body"],
        "error": "Expected attendance transaction to be absent after delete verification",
    }


def cmd_attendance_transactions_create(args: Any, ctx: dict[str, Any]) -> int:
    payload_file, payload_obj, _transaction = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    selector = {"kind": "attendance_transaction", "action": "create", "path": "/attendancetransactions"}
    plan = _build_plan(
        action="create",
        selector=selector,
        ...
        risk_level="medium",
        risk_reasons=["fortnox-write", "attendance-transaction-create"],
        verification_plan={"type": "read-after-write", "path_template": "/attendancetransactions/{id}"},
        rollback_notes="No generic rollback. Recreate the attendance transaction explicitly if needed.",
        ctx=ctx,
    )
    plan_path = _write_plan_if_requested(ctx, plan)
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("attendance_transactions.create.plan", {"plan_out": plan_path})
        ctx["out"].emit(out)
        return 0
    ...
    payload = request_json(ctx=ctx, method="POST", path="/attendancetransactions", json_body=payload_obj, expect_json=True)
    transaction_id = _extract_id_from_response(payload.get("body"))
    if not transaction_id:
        raise ValidationError("Could not determine attendance-transaction id for create verification")
    verification = _verify_present(ctx=ctx, transaction_id=transaction_id)
    receipt = {
        "tool": ctx.get("tool") or "fortnox-api-tool",
        "version": ctx.get("tool_version") or None,
    ...
    ctx["audit"].write("attendance_transactions.create.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1


def cmd_attendance_transactions_update(args: Any, ctx: dict[str, Any]) -> int:
    transaction_id = str(getattr(args, "id", "") or "").strip()
    payload_file, payload_obj, _transaction = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    selector = {
        "kind": "attendance_transaction",
        "action": "update",
        "path": f"/attendancetransactions/{transaction_id}",
        "id": transaction_id,
    }
    plan = _build_plan(
        ...
        payload_obj=payload_obj,
        risk_level="medium",
        risk_reasons=["fortnox-write", "attendance-transaction-update"],
        verification_plan={"type": "read-after-write", "path": f"/attendancetransactions/{transaction_id}"},
    ...
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("attendance_transactions.update.plan", {"plan_out": plan_path, "id": transaction_id})
        ctx["out"].emit(out)
        return 0
    ...
    receipt_path = _write_receipt_if_requested(ctx, receipt)
    out = {"ok": bool(verification.get("ok")), "dry_run": False, "receipt": receipt, "receipt_out": receipt_path}
    ctx["audit"].write("attendance_transactions.update.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1


def cmd_attendance_transactions_delete(args: Any, ctx: dict[str, Any]) -> int:
    transaction_id = str(getattr(args, "id", "") or "").strip()
    selector = {
        "kind": "attendance_transaction",
        "action": "delete",
    ...
    plan = _build_plan(
        ...
        risk_reasons=["fortnox-write", "attendance-transaction-delete", "irreversible"],
        verification_plan={"type": "absence-check", "path": f"/attendancetransactions/{transaction_id}", "expect_http_status": 404},
        rollback_notes="No generic rollback. Recreate the attendance transaction explicitly if needed.",
        ctx=ctx,
    )
    plan_path = _write_plan_if_requested(ctx, plan)
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("attendance_transactions.delete.plan", {"plan_out": plan_path, "id": transaction_id})
        ctx["out"].emit(out)
        return 0

    if not bool(ctx.get("yes")):
        raise SafetyError("Refused: deleting an attendance transaction requires --apply --yes")
    if not bool(ctx.get("ack_irreversible")):
        raise SafetyError("Refused: deleting an attendance transaction requires --ack-irreversible")

    plan = _load_plan_from_ctx(ctx)
    _validate_plan_for_apply(plan, action="delete", selector=selector, payload_file=None, ctx=ctx)
    payload = request_json(ctx=ctx, method="DELETE", path=f"/attendancetransactions/{transaction_id}", expect_json=False)
    verification = _verify_absent(ctx=ctx, transaction_id=transaction_id)
    receipt = {
        "tool": ctx.get("tool") or "fortnox-api-tool",
        "version": ctx.get("tool_version") or None,
        "applied_at_utc": _utc_now(),
    ...
    receipt_path = _write_receipt_if_requested(ctx, receipt)
    out = {"ok": bool(verification.get("ok")), "dry_run": False, "receipt": receipt, "receipt_out": receipt_path}
    ctx["audit"].write("attendance_transactions.delete.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1
