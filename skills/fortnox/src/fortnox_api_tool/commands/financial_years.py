...
def _load_payload_file(path_str: str) -> tuple[Path, dict[str, Any], dict[str, Any]]:
    ...
    financial_year = obj.get("FinancialYear")
    if not isinstance(financial_year, dict):
        raise ValidationError("JSON file must contain a top-level FinancialYear object")
    return path, obj, financial_year


def _extract_id_from_payload(financial_year: dict[str, Any]) -> str | None:
    raw = financial_year.get("Id")
    if raw is None:
        return None
    value = str(raw).strip()
...
def _extract_id_from_response(body: dict[str, Any] | None) -> str | None:
    if not isinstance(body, dict):
        return None
    financial_year = body.get("FinancialYear")
    if not isinstance(financial_year, dict):
        return None
    return _extract_id_from_payload(financial_year)
...
def _verify_present(*, ctx: dict[str, Any], year_id: str) -> dict[str, Any]:
...
def cmd_financial_years_create(args: Any, ctx: dict[str, Any]) -> int:
    payload_file, payload_obj, financial_year = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    selector = {
        "kind": "financial-year",
        "action": "create",
        "path": "/financialyears",
    ...
    plan = _build_plan(
        ...
        risk_level="medium",
        risk_reasons=["fortnox-write", "financial-year-create"],
        verification_plan={"type": "read-after-write", "path_template": "/financialyears/{Id}"},
        rollback_notes="No generic rollback. Recreate the financial year explicitly if needed.",
        ctx=ctx,
    )
    plan_path = _write_plan_if_requested(ctx, plan)
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("financial-years.create.plan", {"plan_out": plan_path})
        ctx["out"].emit(out)
        return 0
    ...
    payload = request_json(ctx=ctx, method="POST", path="/financialyears", json_body=payload_obj, expect_json=True)
    year_id = _extract_id_from_response(payload.get("body")) or _extract_id_from_payload(financial_year)
    if not year_id:
        raise ValidationError("Could not determine Id for create verification")
    verification = _verify_present(ctx=ctx, year_id=year_id)
    receipt = {
        "tool": ctx.get("tool") or "fortnox-api-tool",
        "version": ctx.get("tool_version") or None,
    ...
    ctx["audit"].write("financial-years.create.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
