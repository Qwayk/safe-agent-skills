...
def _load_payload_file(path_str: str) -> tuple[Path, dict[str, Any], dict[str, Any]]:
    ...
    voucher_series = obj.get("VoucherSeries")
    if not isinstance(voucher_series, dict):
        raise ValidationError("JSON file must contain a top-level VoucherSeries object")
    return path, obj, voucher_series


def _extract_code_from_payload(voucher_series: dict[str, Any]) -> str | None:
    raw = voucher_series.get("Code")
    if raw is None:
        return None
    value = str(raw).strip()
...
def _extract_code_from_response(body: dict[str, Any] | None) -> str | None:
    if not isinstance(body, dict):
        return None
    voucher_series = body.get("VoucherSeries")
    if not isinstance(voucher_series, dict):
        return None
    return _extract_code_from_payload(voucher_series)
...
def cmd_voucher_series_create(args: Any, ctx: dict[str, Any]) -> int:
    payload_file, payload_obj, voucher_series = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    selector = {
        "kind": "voucher-series",
        "action": "create",
        "path": "/voucherseries",
    ...
    plan = _build_plan(
        ...
        risk_level="medium",
        risk_reasons=["fortnox-write", "voucher-series-create"],
        verification_plan={"type": "read-after-write", "path_template": "/voucherseries/{Code}"},
        rollback_notes="No generic rollback. Recreate the voucher-series explicitly if needed.",
        ctx=ctx,
    )
    plan_path = _write_plan_if_requested(ctx, plan)
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("voucher-series.create.plan", {"plan_out": plan_path})
        ctx["out"].emit(out)
        return 0
    ...
    code = _extract_code_from_response(payload.get("body")) or _extract_code_from_payload(voucher_series)
    if not code:
    ...
    ctx["audit"].write("voucher-series.create.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1


def cmd_voucher_series_update(args: Any, ctx: dict[str, Any]) -> int:
    code = str(getattr(args, "code", "") or "").strip()
    payload_file, payload_obj, voucher_series = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    payload_code = _extract_code_from_payload(voucher_series)
    if payload_code and payload_code != code:
        raise ValidationError("VoucherSeries.Code in the JSON file must match --code")
    selector = {
        "kind": "voucher-series",
        "action": "update",
        "path": f"/voucherseries/{code}",
    ...
    plan = _build_plan(
        ...
        payload_obj=payload_obj,
        risk_level="medium",
        risk_reasons=["fortnox-write", "voucher-series-update"],
        verification_plan={"type": "read-after-write", "path": f"/voucherseries/{code}"},
    ...
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("voucher-series.update.plan", {"plan_out": plan_path, "code": code})
        ctx["out"].emit(out)
        return 0
    ...
    receipt_path = _write_receipt_if_requested(ctx, receipt)
    out = {"ok": bool(verification.get("ok")), "dry_run": False, "receipt": receipt, "receipt_out": receipt_path}
    ctx["audit"].write("voucher-series.update.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1
