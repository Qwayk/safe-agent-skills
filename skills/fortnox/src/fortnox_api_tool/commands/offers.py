...
def _load_payload_file(path_str: str) -> tuple[Path, dict[str, Any], dict[str, Any]]:
    ...
    if not isinstance(offer, dict):
        raise ValidationError("JSON file must contain a top-level Offer object")
    return path, obj, offer
...
def _extract_document_number_from_payload(offer: dict[str, Any]) -> str | None:
    raw = offer.get("DocumentNumber")
    if raw is None:
        return None
    value = str(raw).strip()
...
def _extract_document_number_from_response(body: dict[str, Any] | None) -> str | None:
    ...
    if not isinstance(offer, dict):
        return None
    return _extract_document_number_from_payload(offer)
...
def _extract_offer(verification: dict[str, Any]) -> dict[str, Any] | None:
...
def _verification_has_cancelled_true(verification: dict[str, Any]) -> bool:
    offer = _extract_offer(verification)
    if not isinstance(offer, dict):
        return False
    return bool(offer.get("Cancelled") is True)


def _verification_has_invoice_reference_present(verification: dict[str, Any]) -> bool:
    offer = _extract_offer(verification)
    if not isinstance(offer, dict):
        return False
    invoice_reference = offer.get("InvoiceReference")
    if isinstance(invoice_reference, int):
        return True
    if isinstance(invoice_reference, str):
        return bool(invoice_reference.strip())
    return False


def _verification_has_order_reference_present(verification: dict[str, Any]) -> bool:
    offer = _extract_offer(verification)
    if not isinstance(offer, dict):
        return False
    order_reference = offer.get("OrderReference")
    if isinstance(order_reference, int):
        return True
    if isinstance(order_reference, str):
        return bool(order_reference.strip())
    return False
...
def _verification_has_sent_true(verification: dict[str, Any]) -> bool:
    ...
    if not isinstance(offer, dict):
        return False
    return bool(offer.get("Sent") is True)


def _run_offers_state_action(
    *,
    args: Any,
    ctx: dict[str, Any],
    action: str,
    ...
    payload_file, payload_obj, offer = _load_payload_file_optional(str(getattr(args, "json_file", "") or "").strip())
    payload_document_number = _extract_document_number_from_payload(offer) if offer is not None else None
    if payload_document_number and payload_document_number != document_number:
        raise ValidationError("Offer.DocumentNumber in the JSON file must match --document-number")
    selector = {
        "kind": "offer",
        "action": action,
        "path": f"/offers/{document_number}/{endpoint}",
    ...
    plan = _build_plan(
        ...
        risk_level="high",
        risk_reasons=risk_reasons,
        verification_plan={"type": "read-after-write", "path": f"/offers/{document_number}"},
    ...
    if not bool(ctx.get("yes")):
        raise SafetyError(f"Refused: {action} on an offer requires --apply --yes")
...
def cmd_offers_create(args: Any, ctx: dict[str, Any]) -> int:
    payload_file, payload_obj, offer = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    selector = {"kind": "offer", "action": "create", "path": "/offers"}
    plan = _build_plan(
        action="create",
        selector=selector,
        payload_file=payload_file,
        payload_obj=payload_obj,
        risk_level="medium",
        risk_reasons=["fortnox-write", "offer-create"],
        verification_plan={"type": "read-after-write", "path_template": "/offers/{DocumentNumber}"},
        rollback_notes="No generic rollback. Recreate or update the offer explicitly if needed.",
        ctx=ctx,
    )
    plan_path = _write_plan_if_requested(ctx, plan)
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("offers.create.plan", {"plan_out": plan_path})
        ctx["out"].emit(out)
        return 0

    plan = _load_plan_from_ctx(ctx)
    _validate_plan_for_apply(plan, action="create", selector=selector, payload_file=payload_file, ctx=ctx)
    payload = request_json(ctx=ctx, method="POST", path="/offers", json_body=payload_obj, expect_json=True)
    document_number = (
        _extract_document_number_from_response(payload["body"])
        or _extract_document_number_from_payload(offer)
    )
    ...
    ctx["audit"].write("offers.create.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1


def cmd_offers_update(args: Any, ctx: dict[str, Any]) -> int:
    document_number = str(getattr(args, "document_number", "") or "").strip()
    payload_file, payload_obj, offer = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    payload_document_number = _extract_document_number_from_payload(offer)
    if payload_document_number and payload_document_number != document_number:
        raise ValidationError("Offer.DocumentNumber in the JSON file must match --document-number")
    selector = {
        "kind": "offer",
    ...
    plan = _build_plan(
        ...
        payload_obj=payload_obj,
        risk_level="medium",
        risk_reasons=["fortnox-write", "offer-update"],
        verification_plan={"type": "read-after-write", "path": f"/offers/{document_number}"},
    ...
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("offers.update.plan", {"plan_out": plan_path, "document_number": document_number})
        ctx["out"].emit(out)
        return 0
    ...
    receipt_path = _write_receipt_if_requested(ctx, receipt)
    out = {"ok": bool(verification.get("ok")), "dry_run": False, "receipt": receipt, "receipt_out": receipt_path}
    ctx["audit"].write("offers.update.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1


def cmd_offers_cancel(args: Any, ctx: dict[str, Any]) -> int:
    return _run_offers_state_action(
        args=args,
        ctx=ctx,
        action="cancel",
        endpoint="cancel",
        verification_check=_verification_has_cancelled_true,
        audit_plan_key="offers.cancel.plan",
        audit_apply_key="offers.cancel.apply",
        risk_reasons=["fortnox-write", "offer-cancel", "status-change"],
        expected_check="cancelled_true",
    )


def cmd_offers_createinvoice(args: Any, ctx: dict[str, Any]) -> int:
    return _run_offers_state_action(
        args=args,
        ctx=ctx,
        action="create-invoice",
        endpoint="createinvoice",
        verification_check=_verification_has_invoice_reference_present,
        audit_plan_key="offers.createinvoice.plan",
        audit_apply_key="offers.createinvoice.apply",
        risk_reasons=["fortnox-write", "offer-create-invoice", "status-change"],
        expected_check="invoice_reference_present",
    )


def cmd_offers_createorder(args: Any, ctx: dict[str, Any]) -> int:
    return _run_offers_state_action(
        args=args,
        ctx=ctx,
        action="create-order",
        endpoint="createorder",
        verification_check=_verification_has_order_reference_present,
        audit_plan_key="offers.createorder.plan",
        audit_apply_key="offers.createorder.apply",
        risk_reasons=["fortnox-write", "offer-create-order", "status-change"],
        expected_check="order_reference_present",
    )


def cmd_offers_externalprint(args: Any, ctx: dict[str, Any]) -> int:
    return _run_offers_state_action(
        args=args,
        ctx=ctx,
        ...
        audit_plan_key="offers.externalprint.plan",
        audit_apply_key="offers.externalprint.apply",
        risk_reasons=["fortnox-write", "offer-externalprint", "status-change"],
        expected_check="sent_true",
    )


def cmd_offers_send_as_email(args: Any, ctx: dict[str, Any]) -> int:
    return _run_offers_state_action(
        args=args,
        ctx=ctx,
        ...
        endpoint="email",
        verification_check=_verification_has_sent_true,
        audit_plan_key="offers.send_as_email.plan",
        audit_apply_key="offers.send_as_email.apply",
        risk_reasons=["fortnox-write", "offer-send-email", "delivery-trigger"],
        expected_check="sent_true",
        http_method="GET",
    )
