...
def _load_payload_file(path_str: str) -> tuple[Path, dict[str, Any], dict[str, Any]]:
    ...
    invoice_payment = obj.get("InvoicePayment")
    if not isinstance(invoice_payment, dict):
        raise ValidationError("JSON file must contain a top-level InvoicePayment object")
...
def _extract_number_from_response(body: dict[str, Any] | None) -> str | None:
    if not isinstance(body, dict):
        return None
    invoice_payment = body.get("InvoicePayment")
    if not isinstance(invoice_payment, dict):
        return None
...
def _verify_removed(*, ctx: dict[str, Any], number: str) -> dict[str, Any]:
    ...
    return {
        ...
        "http_status": payload["status"],
        "data": payload["body"],
        "error": "Expected invoice payment to be absent after delete verification",
    }
...
def _verification_has_booked_true(verification: dict[str, Any]) -> bool:
    ...
    if not isinstance(data, dict):
        return False
    invoice_payment = data.get("InvoicePayment")
    if not isinstance(invoice_payment, dict):
        return False
    return bool(invoice_payment.get("Booked") is True)


def cmd_invoice_payments_create(args: Any, ctx: dict[str, Any]) -> int:
    payload_file, payload_obj, invoice_payment = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    selector = {"kind": "invoice-payment", "action": "create", "path": "/invoicepayments"}
    plan = _build_plan(
        action="create",
        selector=selector,
        ...
        risk_level="medium",
        risk_reasons=["fortnox-write", "invoice-payment-create"],
        verification_plan={"type": "read-after-write", "path_template": "/invoicepayments/{Number}"},
        rollback_notes="No generic rollback. Recreate or update the invoice payment explicitly if needed.",
        ctx=ctx,
    )
    plan_path = _write_plan_if_requested(ctx, plan)
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("invoice_payments.create.plan", {"plan_out": plan_path})
        ctx["out"].emit(out)
        return 0
    ...
    payload = request_json(ctx=ctx, method="POST", path="/invoicepayments", json_body=payload_obj, expect_json=True)
    ...
    ctx["audit"].write("invoice_payments.create.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1


def cmd_invoice_payments_update(args: Any, ctx: dict[str, Any]) -> int:
    ...
    if payload_number and payload_number != number:
        raise ValidationError("InvoicePayment.Number in the JSON file must match --number")
    selector = {
    ...
    plan = _build_plan(
        ...
        payload_obj=payload_obj,
        risk_level="medium",
        risk_reasons=["fortnox-write", "invoice-payment-update"],
        verification_plan={"type": "read-after-write", "path": f"/invoicepayments/{number}"},
    ...
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("invoice_payments.update.plan", {"plan_out": plan_path, "number": number})
        ctx["out"].emit(out)
        return 0
    ...
    receipt_path = _write_receipt_if_requested(ctx, receipt)
    out = {"ok": bool(verification.get("ok")), "dry_run": False, "receipt": receipt, "receipt_out": receipt_path}
    ctx["audit"].write("invoice_payments.update.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1


def cmd_invoice_payments_remove(args: Any, ctx: dict[str, Any]) -> int:
    number = str(getattr(args, "number", "") or "").strip()
    selector = {
        "kind": "invoice-payment",
        "action": "remove",
    ...
    plan = _build_plan(
        ...
        risk_reasons=["fortnox-write", "invoice-payment-delete", "irreversible"],
        verification_plan={"type": "absence-check", "path": f"/invoicepayments/{number}", "expect_http_status": 404},
        rollback_notes="No generic rollback. Recreate the invoice payment explicitly if you need to restore it.",
        ctx=ctx,
    )
    plan_path = _write_plan_if_requested(ctx, plan)
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("invoice_payments.remove.plan", {"plan_out": plan_path, "number": number})
        ctx["out"].emit(out)
        return 0

    if not bool(ctx.get("yes")):
        raise SafetyError("Refused: deleting an invoice payment requires --apply --yes")
    if not bool(ctx.get("ack_irreversible")):
        raise SafetyError("Refused: deleting an invoice payment requires --ack-irreversible")
    plan = _load_plan_from_ctx(ctx)
    _validate_plan_for_apply(plan, action="remove", selector=selector, payload_file=None, ctx=ctx)
    payload = request_json(ctx=ctx, method="DELETE", path=f"/invoicepayments/{number}", expect_json=False)
    ...
    receipt_path = _write_receipt_if_requested(ctx, receipt)
    out = {"ok": bool(verification.get("ok")), "dry_run": False, "receipt": receipt, "receipt_out": receipt_path}
    ctx["audit"].write("invoice_payments.remove.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1


def cmd_invoice_payments_bookkeep(args: Any, ctx: dict[str, Any]) -> int:
    number = str(getattr(args, "number", "") or "").strip()
    payload_file, payload_obj, invoice_payment = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    payload_number = _extract_number_from_payload(invoice_payment)
    if payload_number and payload_number != number:
    ...
    selector = {
        "kind": "invoice-payment",
        "action": "bookkeep",
        "path": f"/invoicepayments/{number}/bookkeep",
        "number": number,
    }
    plan = _build_plan(
        action="bookkeep",
        ...
        payload_obj=payload_obj,
        risk_level="high",
        risk_reasons=["fortnox-write", "invoice-payment-bookkeep", "status-change"],
        verification_plan={"type": "read-after-write", "path": f"/invoicepayments/{number}", "expect_booked": True},
    ...
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("invoice_payments.bookkeep.plan", {"plan_out": plan_path, "number": number})
        ctx["out"].emit(out)
        return 0

    if not bool(ctx.get("yes")):
        raise SafetyError("Refused: bookkeep on an invoice payment requires --apply --yes")
    plan = _load_plan_from_ctx(ctx)
    ...
    ctx["audit"].write("invoice_payments.bookkeep.apply", {"receipt_out": receipt_path, "verified": verification_ok})
    ctx["out"].emit(out)
