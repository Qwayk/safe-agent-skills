...
def _load_payload_file(path_str: str) -> tuple[Path, dict[str, Any], dict[str, Any]]:
    ...
    tax_reduction = obj.get("TaxReduction")
    if not isinstance(tax_reduction, dict):
        raise ValidationError("JSON file must contain a top-level TaxReduction object")
    return path, obj, tax_reduction


def _extract_id_from_payload(tax_reduction: dict[str, Any] | None) -> str | None:
    if not isinstance(tax_reduction, dict):
        return None
    return _string_value(tax_reduction.get("Id"))


def _extract_id_from_response(body: dict[str, Any] | None) -> str | None:
    if not isinstance(body, dict):
        return None
    tax_reduction = body.get("TaxReduction")
    if not isinstance(tax_reduction, dict):
        return None
    return _extract_id_from_payload(tax_reduction)
...
def _verify_present(*, ctx: dict[str, Any], tax_reduction_id: str) -> dict[str, Any]:
    path = f"/taxreductions/{tax_reduction_id}"
    try:
...
def _verify_absent(*, ctx: dict[str, Any], tax_reduction_id: str) -> dict[str, Any]:
    path = f"/taxreductions/{tax_reduction_id}"
    try:
        payload = request_json(ctx=ctx, method="GET", path=path, expect_json=True)
    except Exception as e:  # noqa: BLE001
    ...
    return {
        ...
        "http_status": payload["status"],
        "data": payload["body"],
        "error": "Expected tax reduction to be absent after remove verification",
    }


def cmd_tax_reductions_list(args: Any, ctx: dict[str, Any]) -> int:
    _ = args
    path = "/taxreductions"
    payload = get_json(ctx=ctx, path=path)
    return _emit_read(ctx, audit_key="tax_reductions.list", path=path, payload=payload)


def cmd_tax_reductions_get(args: Any, ctx: dict[str, Any]) -> int:
    tax_reduction_id = str(getattr(args, "id", "") or "").strip()
    path = f"/taxreductions/{tax_reduction_id}"
    payload = get_json(ctx=ctx, path=path)
    return _emit_read(ctx, audit_key="tax_reductions.get", path=path, payload=payload)


def cmd_tax_reductions_create(args: Any, ctx: dict[str, Any]) -> int:
    payload_file, payload_obj, tax_reduction = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    selector = {
        "kind": "tax-reduction",
        "action": "create",
        "path": "/taxreductions",
    ...
    plan = _build_plan(
        ...
        risk_level="medium",
        risk_reasons=["fortnox-write", "tax-reduction-create"],
        verification_plan={"type": "read-after-write", "path_template": "/taxreductions/{Id}"},
        rollback_notes="No generic rollback. Remove the created tax reduction explicitly if needed.",
        ctx=ctx,
    )
    plan_path = _write_plan_if_requested(ctx, plan)
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("tax-reductions.create.plan", {"plan_out": plan_path})
        ctx["out"].emit(out)
        return 0
    ...
    tax_reduction_id = _extract_id_from_response(payload.get("body")) or _extract_id_from_payload(tax_reduction)
    if not tax_reduction_id:
        raise ValidationError("Could not determine Id for create verification")
    verification = _verify_present(ctx=ctx, tax_reduction_id=tax_reduction_id)
    receipt = {
        "tool": ctx.get("tool") or "fortnox-api-tool",
        "version": ctx.get("tool_version") or None,
        ...
        "changed": True,
        "http_status": payload["status"],
        "target_tax_reduction_id": tax_reduction_id,
    ...
    ctx["audit"].write("tax-reductions.create.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1


def cmd_tax_reductions_update(args: Any, ctx: dict[str, Any]) -> int:
    tax_reduction_id = str(getattr(args, "id", "") or "").strip()
    payload_file, payload_obj, tax_reduction = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    payload_id = _extract_id_from_payload(tax_reduction)
    if payload_id and payload_id != tax_reduction_id:
        raise ValidationError("TaxReduction.Id in the JSON file must match --id")
    selector = {
        "kind": "tax-reduction",
        "action": "update",
        "path": f"/taxreductions/{tax_reduction_id}",
    ...
    plan = _build_plan(
        ...
        payload_obj=payload_obj,
        risk_level="medium",
        risk_reasons=["fortnox-write", "tax-reduction-update"],
        verification_plan={"type": "read-after-write", "path": f"/taxreductions/{tax_reduction_id}"},
    ...
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("tax-reductions.update.plan", {"plan_out": plan_path, "id": tax_reduction_id})
        ctx["out"].emit(out)
        return 0
    ...
    out = {"ok": bool(verification.get("ok")), "dry_run": False, "receipt": receipt, "receipt_out": receipt_path}
    ctx["audit"].write(
        "tax-reductions.update.apply",
        {"receipt_out": receipt_path, "id": tax_reduction_id, "verified": verification.get("ok")},
    )
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1


def cmd_tax_reductions_remove(args: Any, ctx: dict[str, Any]) -> int:
    tax_reduction_id = str(getattr(args, "id", "") or "").strip()
    selector = {
        "kind": "tax-reduction",
        "action": "remove",
    ...
    plan = _build_plan(
        ...
        payload_obj=None,
        risk_level="high",
        risk_reasons=["fortnox-write", "tax-reduction-remove", "irreversible"],
        verification_plan={"type": "absence-check", "path": f"/taxreductions/{tax_reduction_id}"},
        rollback_notes="No generic rollback. Recreate the tax reduction manually if needed.",
        ctx=ctx,
    )
    plan_path = _write_plan_if_requested(ctx, plan)
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("tax-reductions.remove.plan", {"plan_out": plan_path, "id": tax_reduction_id})
        ctx["out"].emit(out)
        return 0
        ...
        ctx["out"].emit(
            {
                ...
                "refused": True,
                "reasons": [
                    "Refused: remove requires --yes",
                    "Refused: remove requires --ack-irreversible",
                ],
            }
        )
        return 0
    ...
    verification = _verify_absent(ctx=ctx, tax_reduction_id=tax_reduction_id)
    receipt = {
        "tool": ctx.get("tool") or "fortnox-api-tool",
        "version": ctx.get("tool_version") or None,
        "applied_at_utc": _utc_now(),
