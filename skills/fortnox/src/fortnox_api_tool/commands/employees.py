...
def _load_payload_file(path_str: str) -> tuple[Path, dict[str, Any], dict[str, Any]]:
    ...
    if not isinstance(employee, dict):
        raise ValidationError("JSON file must contain a top-level Employee object")
    return path, obj, employee


def _extract_employee_id_from_payload(employee: dict[str, Any]) -> str | None:
    raw = employee.get("EmployeeId")
    if raw is None:
        return None
    value = str(raw).strip()
...
def _extract_employee_id_from_response(body: dict[str, Any] | None) -> str | None:
    if not isinstance(body, dict):
        return None
    employee = body.get("Employee")
    if not isinstance(employee, dict):
        return None
    return _extract_employee_id_from_payload(employee)
...
def _verify_present(*, ctx: dict[str, Any], employee_id: str) -> dict[str, Any]:
...
def cmd_employees_create(args: Any, ctx: dict[str, Any]) -> int:
    payload_file, payload_obj, employee = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    selector = {"kind": "employee", "action": "create", "path": "/employees"}
    plan = _build_plan(
        action="create",
        selector=selector,
        ...
        risk_level="medium",
        risk_reasons=["fortnox-write", "employee-create"],
        verification_plan={"type": "read-after-write", "path_template": "/employees/{EmployeeId}"},
        rollback_notes="No generic rollback. Recreate the employee explicitly if needed.",
        ctx=ctx,
    )
    plan_path = _write_plan_if_requested(ctx, plan)
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("employees.create.plan", {"plan_out": plan_path})
        ctx["out"].emit(out)
        return 0
    ...
    payload = request_json(ctx=ctx, method="POST", path="/employees", json_body=payload_obj, expect_json=True)
    employee_id = _extract_employee_id_from_response(payload.get("body")) or _extract_employee_id_from_payload(employee)
    if not employee_id:
        raise ValidationError("Could not determine EmployeeId for create verification")
    verification = _verify_present(ctx=ctx, employee_id=employee_id)
    receipt = {
        "tool": ctx.get("tool") or "fortnox-api-tool",
        "version": ctx.get("tool_version") or None,
    ...
    ctx["audit"].write("employees.create.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1


def cmd_employees_update(args: Any, ctx: dict[str, Any]) -> int:
    employee_id = str(getattr(args, "employee_id", "") or "").strip()
    payload_file, payload_obj, employee = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    payload_employee_id = _extract_employee_id_from_payload(employee)
    if payload_employee_id and payload_employee_id != employee_id:
        raise ValidationError("Employee.EmployeeId in the JSON file must match --employee-id")
    selector = {
        "kind": "employee",
        "action": "update",
        "path": f"/employees/{employee_id}",
    ...
    plan = _build_plan(
        ...
        payload_obj=payload_obj,
        risk_level="medium",
        risk_reasons=["fortnox-write", "employee-update"],
        verification_plan={"type": "read-after-write", "path": f"/employees/{employee_id}"},
    ...
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("employees.update.plan", {"plan_out": plan_path, "employee_id": employee_id})
        ctx["out"].emit(out)
        return 0
    ...
    receipt_path = _write_receipt_if_requested(ctx, receipt)
    out = {"ok": bool(verification.get("ok")), "dry_run": False, "receipt": receipt, "receipt_out": receipt_path}
    ctx["audit"].write("employees.update.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1
