...
def _load_payload_file(path_str: str) -> tuple[Path, dict[str, Any], dict[str, Any]]:
    ...
    if not isinstance(article, dict):
        raise ValidationError("JSON file must contain a top-level Article object")
    return path, obj, article


def _extract_article_number_from_payload(article: dict[str, Any]) -> str | None:
    raw = article.get("ArticleNumber")
    if raw is None:
        return None
    value = str(raw).strip()
...
def _extract_article_number_from_response(body: dict[str, Any] | None) -> str | None:
    if not isinstance(body, dict):
        return None
    article = body.get("Article")
    if not isinstance(article, dict):
        return None
    return _extract_article_number_from_payload(article)
...
def _verify_present(*, ctx: dict[str, Any], article_number: str) -> dict[str, Any]:
...
def _verify_absent(*, ctx: dict[str, Any], article_number: str) -> dict[str, Any]:
    path = f"/articles/{article_number}"
    try:
        payload = request_json(ctx=ctx, method="GET", path=path, expect_json=True)
    except Exception as e:  # noqa: BLE001
    ...
    return {
        ...
        "http_status": payload["status"],
        "data": payload["body"],
        "error": "Expected article to be absent after delete verification",
    }


def cmd_articles_create(args: Any, ctx: dict[str, Any]) -> int:
    payload_file, payload_obj, article = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    selector = {
        "kind": "article",
        "action": "create",
        "path": "/articles",
    ...
    plan = _build_plan(
        ...
        risk_level="medium",
        risk_reasons=["fortnox-write", "article-create"],
        verification_plan={"type": "read-after-write", "path_template": "/articles/{ArticleNumber}"},
        rollback_notes="No generic rollback. Recreate the article explicitly if needed.",
        ctx=ctx,
    )
    plan_path = _write_plan_if_requested(ctx, plan)
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("articles.create.plan", {"plan_out": plan_path})
        ctx["out"].emit(out)
        return 0
    ...
    article_number = _extract_article_number_from_response(payload.get("body")) or _extract_article_number_from_payload(article)
    if not article_number:
        raise ValidationError("Could not determine ArticleNumber for create verification")
    verification = _verify_present(ctx=ctx, article_number=article_number)
    receipt = {
        "tool": ctx.get("tool") or "fortnox-api-tool",
        "version": ctx.get("tool_version") or None,
    ...
    ctx["audit"].write("articles.create.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1


def cmd_articles_update(args: Any, ctx: dict[str, Any]) -> int:
    article_number = str(getattr(args, "article_number", "") or "").strip()
    payload_file, payload_obj, article = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    payload_article_number = _extract_article_number_from_payload(article)
    if payload_article_number and payload_article_number != article_number:
        raise ValidationError("Article.ArticleNumber in the JSON file must match --article-number")
    selector = {
        "kind": "article",
        "action": "update",
        "path": f"/articles/{article_number}",
    ...
    plan = _build_plan(
        ...
        payload_obj=payload_obj,
        risk_level="medium",
        risk_reasons=["fortnox-write", "article-update"],
        verification_plan={"type": "read-after-write", "path": f"/articles/{article_number}"},
    ...
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("articles.update.plan", {"plan_out": plan_path, "article_number": article_number})
        ctx["out"].emit(out)
        return 0
    ...
    receipt_path = _write_receipt_if_requested(ctx, receipt)
    out = {"ok": bool(verification.get("ok")), "dry_run": False, "receipt": receipt, "receipt_out": receipt_path}
    ctx["audit"].write("articles.update.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1


def cmd_articles_delete(args: Any, ctx: dict[str, Any]) -> int:
    article_number = str(getattr(args, "article_number", "") or "").strip()
    selector = {
        "kind": "article",
        "action": "delete",
    ...
    plan = _build_plan(
        ...
        risk_reasons=["fortnox-write", "article-delete", "irreversible"],
        verification_plan={"type": "absence-check", "path": f"/articles/{article_number}", "expect_http_status": 404},
        rollback_notes="No generic rollback. Recreate the article explicitly if needed.",
        ctx=ctx,
    )
    plan_path = _write_plan_if_requested(ctx, plan)
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("articles.delete.plan", {"plan_out": plan_path, "article_number": article_number})
        ctx["out"].emit(out)
        return 0

    if not bool(ctx.get("yes")):
        raise SafetyError("Refused: deleting an article requires --apply --yes")
    if not bool(ctx.get("ack_irreversible")):
        raise SafetyError("Refused: deleting an article requires --ack-irreversible")

    plan = _load_plan_from_ctx(ctx)
    _validate_plan_for_apply(plan, action="delete", selector=selector, payload_file=None, ctx=ctx)
    payload = request_json(ctx=ctx, method="DELETE", path=f"/articles/{article_number}", expect_json=False)
    verification = _verify_absent(ctx=ctx, article_number=article_number)
    receipt = {
        "tool": ctx.get("tool") or "fortnox-api-tool",
        "version": ctx.get("tool_version") or None,
        "applied_at_utc": _utc_now(),
    ...
    receipt_path = _write_receipt_if_requested(ctx, receipt)
    out = {"ok": bool(verification.get("ok")), "dry_run": False, "receipt": receipt, "receipt_out": receipt_path}
    ctx["audit"].write("articles.delete.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1
