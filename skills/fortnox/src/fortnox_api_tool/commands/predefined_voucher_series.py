...
def _load_payload_file(path_str: str) -> tuple[Path, dict[str, Any], dict[str, Any]]:
    ...
    predefined_voucher_series = obj.get("PreDefinedVoucherSeries")
    if not isinstance(predefined_voucher_series, dict):
        raise ValidationError("JSON file must contain a top-level PreDefinedVoucherSeries object")
    return path, obj, predefined_voucher_series


def _extract_name_from_payload(predefined_voucher_series: dict[str, Any]) -> str | None:
    raw = predefined_voucher_series.get("Name")
    if raw is None:
        return None
    value = str(raw).strip()
...
def _extract_name_from_response(body: dict[str, Any] | None) -> str | None:
    if not isinstance(body, dict):
        return None
    predefined_voucher_series = body.get("PreDefinedVoucherSeries")
    if not isinstance(predefined_voucher_series, dict):
        return None
    return _extract_name_from_payload(predefined_voucher_series)
...
def cmd_predefined_voucher_series_update(args: Any, ctx: dict[str, Any]) -> int:
    name = str(getattr(args, "name", "") or "").strip()
    payload_file, payload_obj, predefined_voucher_series = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    payload_name = _extract_name_from_payload(predefined_voucher_series)
    if payload_name and payload_name != name:
        raise ValidationError("PreDefinedVoucherSeries.Name in the JSON file must match --name")
    selector = {
        "kind": "predefined-voucher-series",
        "action": "update",
        "path": f"/predefinedvoucherseries/{name}",
        "name": name,
    }
    plan = _build_plan(
        ...
        risk_level="medium",
        risk_reasons=["fortnox-write", "predefined-voucher-series-update"],
        verification_plan={"type": "read-after-write", "path": f"/predefinedvoucherseries/{name}"},
    ...
    if not bool(ctx.get("apply")):
        ...
        ctx["audit"].write("predefined-voucher-series.update.plan", {"plan_out": plan_path, "name": name})
        ctx["out"].emit(out)
        return 0
    ...
    ctx["audit"].write(
        "predefined-voucher-series.update.apply",
