...
def _load_payload_file(path_str: str) -> tuple[Path, dict[str, Any], dict[str, Any]]:
    ...
    if not isinstance(order, dict):
        raise ValidationError("JSON file must contain a top-level Order object")
    return path, obj, order
...
def _extract_document_number_from_payload(order: dict[str, Any]) -> str | None:
    raw = order.get("DocumentNumber")
    if raw is None:
        return None
    value = str(raw).strip()
...
def _extract_document_number_from_response(body: dict[str, Any] | None) -> str | None:
    ...
    if not isinstance(order, dict):
        return None
    return _extract_document_number_from_payload(order)
...
def _extract_order(verification: dict[str, Any]) -> dict[str, Any] | None:
...
def _verification_has_cancelled_true(verification: dict[str, Any]) -> bool:
    order = _extract_order(verification)
    if not isinstance(order, dict):
        return False
    return bool(order.get("Cancelled") is True)
...
def _verification_has_invoice_reference_present(verification: dict[str, Any]) -> bool:
    ...
    if not isinstance(order, dict):
        return False
    invoice_reference = order.get("InvoiceReference")
...
def _verification_has_sent_true(verification: dict[str, Any]) -> bool:
    ...
    if not isinstance(order, dict):
        return False
    return bool(order.get("Sent") is True)


def _run_orders_state_action(
    *,
    args: Any,
    ctx: dict[str, Any],
    action: str,
    ...
    payload_file, payload_obj, order = _load_payload_file_optional(str(getattr(args, "json_file", "") or "").strip())
    payload_document_number = _extract_document_number_from_payload(order) if order is not None else None
    if payload_document_number and payload_document_number != document_number:
        raise ValidationError("Order.DocumentNumber in the JSON file must match --document-number")
    selector = {
        "kind": "order",
        "action": action,
        "path": f"/orders/{document_number}/{endpoint}",
    ...
    plan = _build_plan(
        ...
        risk_level="high",
        risk_reasons=risk_reasons,
        verification_plan={"type": "read-after-write", "path": f"/orders/{document_number}"},
    ...
    if not bool(ctx.get("yes")):
        raise SafetyError(f"Refused: {action} on an order requires --apply --yes")
...
def cmd_orders_create(args: Any, ctx: dict[str, Any]) -> int:
    payload_file, payload_obj, order = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    selector = {
        "kind": "order",
        "action": "create",
        "path": "/orders",
    ...
    plan = _build_plan(
        ...
        payload_obj=payload_obj,
        risk_level="medium",
        risk_reasons=["fortnox-write", "order-create"],
        verification_plan={"type": "read-after-write", "path_template": "/orders/{DocumentNumber}"},
        rollback_notes="No generic rollback. Recreate or update the order explicitly if needed.",
        ctx=ctx,
    )
    plan_path = _write_plan_if_requested(ctx, plan)
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("orders.create.plan", {"plan_out": plan_path})
        ctx["out"].emit(out)
        return 0

    plan = _load_plan_from_ctx(ctx)
    _validate_plan_for_apply(plan, action="create", selector=selector, payload_file=payload_file, ctx=ctx)
    payload = request_json(ctx=ctx, method="POST", path="/orders", json_body=payload_obj, expect_json=True)
    document_number = (
        _extract_document_number_from_response(payload["body"])
        or _extract_document_number_from_payload(order)
    )
    ...
    ctx["audit"].write("orders.create.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1


def cmd_orders_update(args: Any, ctx: dict[str, Any]) -> int:
    document_number = str(getattr(args, "document_number", "") or "").strip()
    payload_file, payload_obj, order = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    payload_document_number = _extract_document_number_from_payload(order)
    if payload_document_number and payload_document_number != document_number:
        raise ValidationError("Order.DocumentNumber in the JSON file must match --document-number")
    selector = {
        "kind": "order",
    ...
    plan = _build_plan(
        ...
        payload_obj=payload_obj,
        risk_level="medium",
        risk_reasons=["fortnox-write", "order-update"],
        verification_plan={"type": "read-after-write", "path": f"/orders/{document_number}"},
    ...
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("orders.update.plan", {"plan_out": plan_path, "document_number": document_number})
        ctx["out"].emit(out)
        return 0
    ...
    receipt_path = _write_receipt_if_requested(ctx, receipt)
    out = {"ok": bool(verification.get("ok")), "dry_run": False, "receipt": receipt, "receipt_out": receipt_path}
    ctx["audit"].write("orders.update.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1


def cmd_orders_cancel(args: Any, ctx: dict[str, Any]) -> int:
    return _run_orders_state_action(
        args=args,
        ctx=ctx,
        action="cancel",
        endpoint="cancel",
        verification_check=_verification_has_cancelled_true,
        audit_plan_key="orders.cancel.plan",
        audit_apply_key="orders.cancel.apply",
        risk_reasons=["fortnox-write", "order-cancel", "status-change"],
        expected_check="cancelled_true",
    )


def cmd_orders_createinvoice(args: Any, ctx: dict[str, Any]) -> int:
    return _run_orders_state_action(
        args=args,
        ctx=ctx,
        ...
        audit_plan_key="orders.createinvoice.plan",
        audit_apply_key="orders.createinvoice.apply",
        risk_reasons=["fortnox-write", "order-create-invoice", "status-change"],
        expected_check="invoice_reference_present",
    )


def cmd_orders_externalprint(args: Any, ctx: dict[str, Any]) -> int:
    return _run_orders_state_action(
        args=args,
        ctx=ctx,
        ...
        audit_plan_key="orders.externalprint.plan",
        audit_apply_key="orders.externalprint.apply",
        risk_reasons=["fortnox-write", "order-externalprint", "status-change"],
        expected_check="sent_true",
    )


def cmd_orders_send_as_email(args: Any, ctx: dict[str, Any]) -> int:
    return _run_orders_state_action(
        args=args,
        ctx=ctx,
        ...
        endpoint="email",
        verification_check=_verification_has_sent_true,
        audit_plan_key="orders.send_as_email.plan",
        audit_apply_key="orders.send_as_email.apply",
        risk_reasons=["fortnox-write", "order-send-email", "delivery-trigger"],
        expected_check="sent_true",
        http_method="GET",
    )
