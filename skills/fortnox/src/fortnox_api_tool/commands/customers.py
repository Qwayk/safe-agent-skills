...
def _load_payload_file(path_str: str) -> tuple[Path, dict[str, Any], dict[str, Any]]:
    ...
    if not isinstance(customer, dict):
        raise ValidationError("JSON file must contain a top-level Customer object")
    return path, obj, customer


def _extract_customer_number_from_payload(customer: dict[str, Any]) -> str | None:
    raw = customer.get("CustomerNumber")
    if raw is None:
        return None
    value = str(raw).strip()
...
def _extract_customer_number_from_response(body: dict[str, Any] | None) -> str | None:
    if not isinstance(body, dict):
        return None
    customer = body.get("Customer")
    if not isinstance(customer, dict):
        return None
    return _extract_customer_number_from_payload(customer)
...
def _verify_present(*, ctx: dict[str, Any], customer_number: str) -> dict[str, Any]:
...
def _verify_absent(*, ctx: dict[str, Any], customer_number: str) -> dict[str, Any]:
    path = f"/customers/{customer_number}"
    try:
        payload = request_json(ctx=ctx, method="GET", path=path, expect_json=True)
    except Exception as e:  # noqa: BLE001
    ...
    return {
        ...
        "http_status": payload["status"],
        "data": payload["body"],
        "error": "Expected customer to be absent after delete verification",
    }


def cmd_customers_create(args: Any, ctx: dict[str, Any]) -> int:
    payload_file, payload_obj, customer = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    selector = {
        "kind": "customer",
        "action": "create",
        "path": "/customers",
    ...
    plan = _build_plan(
        ...
        risk_level="medium",
        risk_reasons=["fortnox-write", "customer-create"],
        verification_plan={"type": "read-after-write", "path_template": "/customers/{CustomerNumber}"},
        rollback_notes="No generic rollback. Recreate the customer explicitly if needed.",
        ctx=ctx,
    )
    plan_path = _write_plan_if_requested(ctx, plan)
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("customers.create.plan", {"plan_out": plan_path})
        ctx["out"].emit(out)
        return 0
    ...
    customer_number = _extract_customer_number_from_response(payload["body"]) or _extract_customer_number_from_payload(customer)
    if not customer_number:
        raise ValidationError("Could not determine CustomerNumber for create verification")
    verification = _verify_present(ctx=ctx, customer_number=customer_number)
    receipt = {
        "tool": ctx.get("tool") or "fortnox-api-tool",
        "version": ctx.get("tool_version") or None,
        ...
        "changed": True,
        "http_status": payload["status"],
        "target_customer_number": customer_number,
    ...
    ctx["audit"].write("customers.create.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1


def cmd_customers_update(args: Any, ctx: dict[str, Any]) -> int:
    customer_number = str(getattr(args, "customer_number", "") or "").strip()
    payload_file, payload_obj, customer = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    payload_customer_number = _extract_customer_number_from_payload(customer)
    if payload_customer_number and payload_customer_number != customer_number:
        raise ValidationError("Customer.CustomerNumber in the JSON file must match --customer-number")
    selector = {
        "kind": "customer",
        "action": "update",
        "path": f"/customers/{customer_number}",
    ...
    plan = _build_plan(
        ...
        payload_obj=payload_obj,
        risk_level="medium",
        risk_reasons=["fortnox-write", "customer-update"],
        verification_plan={"type": "read-after-write", "path": f"/customers/{customer_number}"},
    ...
    if not bool(ctx.get("apply")):
        ...
        ctx["audit"].write(
            ...
            {"plan_out": plan_path, "customer_number": customer_number},
        )
        ctx["out"].emit(out)
        return 0
    ...
    receipt_path = _write_receipt_if_requested(ctx, receipt)
    out = {"ok": bool(verification.get("ok")), "dry_run": False, "receipt": receipt, "receipt_out": receipt_path}
    ctx["audit"].write("customers.update.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1


def cmd_customers_delete(args: Any, ctx: dict[str, Any]) -> int:
    customer_number = str(getattr(args, "customer_number", "") or "").strip()
    selector = {
        "kind": "customer",
        "action": "delete",
    ...
    plan = _build_plan(
        ...
        risk_reasons=["fortnox-write", "customer-delete", "irreversible"],
        verification_plan={"type": "absence-check", "path": f"/customers/{customer_number}", "expect_http_status": 404},
        rollback_notes="No generic rollback. Recreate the customer explicitly if needed.",
        ctx=ctx,
    )
    plan_path = _write_plan_if_requested(ctx, plan)
    ...
    if not bool(ctx.get("yes")):
        raise SafetyError("Refused: deleting a customer requires --apply --yes")
    if not bool(ctx.get("ack_irreversible")):
        raise SafetyError("Refused: deleting a customer requires --ack-irreversible")
    ...
    verification = _verify_absent(ctx=ctx, customer_number=customer_number)
    receipt = {
        "tool": ctx.get("tool") or "fortnox-api-tool",
        "version": ctx.get("tool_version") or None,
        "applied_at_utc": _utc_now(),
