...
def _load_payload_file(path_str: str) -> tuple[Path, dict[str, Any], dict[str, Any]]:
    ...
    predefined_account = obj.get("PreDefinedAccount")
    if not isinstance(predefined_account, dict):
        raise ValidationError("JSON file must contain a top-level PreDefinedAccount object")
    return path, obj, predefined_account


def _extract_name_from_payload(predefined_account: dict[str, Any]) -> str | None:
    raw = predefined_account.get("Name")
    if raw is None:
        return None
    value = str(raw).strip()
...
def _extract_name_from_response(body: dict[str, Any] | None) -> str | None:
    if not isinstance(body, dict):
        return None
    predefined_account = body.get("PreDefinedAccount")
    if not isinstance(predefined_account, dict):
        return None
    return _extract_name_from_payload(predefined_account)
...
def _verify_present(*, ctx: dict[str, Any], name: str) -> dict[str, Any]:
...
def cmd_predefined_accounts_update(args: Any, ctx: dict[str, Any]) -> int:
    name = str(getattr(args, "name", "") or "").strip()
    payload_file, payload_obj, predefined_account = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    payload_name = _extract_name_from_payload(predefined_account)
    if payload_name and payload_name != name:
        raise ValidationError("PreDefinedAccount.Name in the JSON file must match --name")
    selector = {
        "kind": "predefined-account",
        "action": "update",
        "path": f"/predefinedaccounts/{name}",
        "name": name,
    }
    plan = _build_plan(
        ...
        risk_level="medium",
        risk_reasons=["fortnox-write", "predefined-account-update"],
        verification_plan={"type": "read-after-write", "path": f"/predefinedaccounts/{name}"},
    ...
    if not bool(ctx.get("apply")):
        ...
        ctx["audit"].write("predefined-accounts.update.plan", {"plan_out": plan_path, "name": name})
        ctx["out"].emit(out)
        return 0
    ...
    payload = request_json(ctx=ctx, method="PUT", path=f"/predefinedaccounts/{name}", json_body=payload_obj, expect_json=True)
    verification = _verify_present(ctx=ctx, name=name)
    receipt = {
        "tool": ctx.get("tool") or "fortnox-api-tool",
        "version": ctx.get("tool_version") or None,
    ...
    ctx["audit"].write("predefined-accounts.update.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
