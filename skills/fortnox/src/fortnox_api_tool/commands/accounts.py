...
def _load_payload_file(path_str: str) -> tuple[Path, dict[str, Any], dict[str, Any]]:
    ...
    if not isinstance(account, dict):
        raise ValidationError("JSON file must contain a top-level Account object")
    return path, obj, account


def _extract_number_from_payload(account: dict[str, Any]) -> str | None:
    raw = account.get("Number")
    if raw is None:
        return None
    value = str(raw).strip()
...
def _extract_number_from_response(body: dict[str, Any] | None) -> str | None:
    ...
    if not isinstance(account, dict):
        return None
    return _extract_number_from_payload(account)
...
def _verify_absent(*, ctx: dict[str, Any], number: str) -> dict[str, Any]:
    path = f"/accounts/{number}"
    try:
        payload = request_json(ctx=ctx, method="GET", path=path, expect_json=True)
    except Exception as e:  # noqa: BLE001
    ...
    return {
        ...
        "http_status": payload["status"],
        "data": payload["body"],
        "error": "Expected account to be absent after delete verification",
    }
...
def cmd_accounts_create(args: Any, ctx: dict[str, Any]) -> int:
    payload_file, payload_obj, account = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    ...
    plan = _build_plan(
        ...
        risk_level="medium",
        risk_reasons=["fortnox-write", "account-create"],
        verification_plan={"type": "read-after-write", "path_template": "/accounts/{Number}"},
        rollback_notes="No generic rollback. Recreate the account explicitly if needed.",
        ctx=ctx,
    )
    plan_path = _write_plan_if_requested(ctx, plan)
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("accounts.create.plan", {"plan_out": plan_path, "financial_year": financial_year})
        ctx["out"].emit(out)
        return 0
    ...
    number = _extract_number_from_response(payload["body"]) or _extract_number_from_payload(account)
    if not number:
    ...
    ctx["audit"].write("accounts.create.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1


def cmd_accounts_update(args: Any, ctx: dict[str, Any]) -> int:
    number = str(getattr(args, "number", "") or "").strip()
    payload_file, payload_obj, account = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    payload_number = _extract_number_from_payload(account)
    if payload_number and payload_number != number:
        raise ValidationError("Account.Number in the JSON file must match --number")
    financial_year = getattr(args, "financial_year", None)
    selector = {
        "kind": "account",
        "action": "update",
    ...
    plan = _build_plan(
        ...
        payload_obj=payload_obj,
        risk_level="medium",
        risk_reasons=["fortnox-write", "account-update"],
        verification_plan={"type": "read-after-write", "path": f"/accounts/{number}"},
    ...
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("accounts.update.plan", {"plan_out": plan_path, "number": number, "financial_year": financial_year})
        ctx["out"].emit(out)
        return 0
    ...
    receipt_path = _write_receipt_if_requested(ctx, receipt)
    out = {"ok": bool(verification.get("ok")), "dry_run": False, "receipt": receipt, "receipt_out": receipt_path}
    ctx["audit"].write("accounts.update.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1


def cmd_accounts_delete(args: Any, ctx: dict[str, Any]) -> int:
    number = str(getattr(args, "number", "") or "").strip()
    selector = {
        "kind": "account",
        "action": "delete",
    ...
    plan = _build_plan(
        ...
        risk_reasons=["fortnox-write", "account-delete", "irreversible"],
        verification_plan={"type": "absence-check", "path": f"/accounts/{number}", "expect_http_status": 404},
        rollback_notes="No generic rollback. Recreate the account explicitly if needed.",
        ctx=ctx,
    )
    plan_path = _write_plan_if_requested(ctx, plan)
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("accounts.delete.plan", {"plan_out": plan_path, "number": number})
        ctx["out"].emit(out)
        return 0

    if not bool(ctx.get("yes")):
        raise SafetyError("Refused: deleting an account requires --apply --yes")
    if not bool(ctx.get("ack_irreversible")):
        raise SafetyError("Refused: deleting an account requires --ack-irreversible")
    ...
    payload = request_json(ctx=ctx, method="DELETE", path=f"/accounts/{number}", expect_json=False)
    verification = _verify_absent(ctx=ctx, number=number)
    receipt = {
        "tool": ctx.get("tool") or "fortnox-api-tool",
        "version": ctx.get("tool_version") or None,
        "applied_at_utc": _utc_now(),
    ...
    receipt_path = _write_receipt_if_requested(ctx, receipt)
    out = {"ok": bool(verification.get("ok")), "dry_run": False, "receipt": receipt, "receipt_out": receipt_path}
    ctx["audit"].write("accounts.delete.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1
