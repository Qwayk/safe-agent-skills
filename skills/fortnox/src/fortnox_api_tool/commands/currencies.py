...
def _load_payload_file(path_str: str) -> tuple[Path, dict[str, Any], dict[str, Any]]:
    ...
    if not isinstance(currency, dict):
        raise ValidationError("JSON file must contain a top-level Currency object")
    return path, obj, currency


def _extract_code_from_payload(currency: dict[str, Any]) -> str | None:
    raw = currency.get("Code")
    if raw is None:
        return None
    value = str(raw).strip()
...
def _extract_code_from_response(body: dict[str, Any] | None) -> str | None:
    ...
    if not isinstance(currency, dict):
        return None
    return _extract_code_from_payload(currency)
...
def _verify_absent(*, ctx: dict[str, Any], code: str) -> dict[str, Any]:
    ...
    return {
        ...
        "http_status": payload["status"],
        "data": payload["body"],
        "error": "Expected currency to be absent after remove verification",
    }


def cmd_currencies_create(args: Any, ctx: dict[str, Any]) -> int:
    payload_file, payload_obj, currency = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    selector = {
        "kind": "currency",
        "action": "create",
        "path": "/currencies",
    ...
    plan = _build_plan(
        ...
        risk_level="medium",
        risk_reasons=["fortnox-write", "currency-create"],
        verification_plan={"type": "read-after-write", "path_template": "/currencies/{Code}"},
        rollback_notes="No generic rollback. Recreate the currency explicitly if needed.",
        ctx=ctx,
    )
    plan_path = _write_plan_if_requested(ctx, plan)
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("currencies.create.plan", {"plan_out": plan_path})
        ctx["out"].emit(out)
        return 0
    ...
    payload = request_json(ctx=ctx, method="POST", path="/currencies", json_body=payload_obj, expect_json=True)
    code = _extract_code_from_response(payload.get("body")) or _extract_code_from_payload(currency)
    if not code:
    ...
    ctx["audit"].write("currencies.create.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1


def cmd_currencies_update(args: Any, ctx: dict[str, Any]) -> int:
    code = str(getattr(args, "code", "") or "").strip()
    payload_file, payload_obj, currency = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    payload_code = _extract_code_from_payload(currency)
    if payload_code and payload_code != code:
        raise ValidationError("Currency.Code in the JSON file must match --code")
    selector = {
        "kind": "currency",
        "action": "update",
        "path": f"/currencies/{code}",
    ...
    plan = _build_plan(
        ...
        payload_obj=payload_obj,
        risk_level="medium",
        risk_reasons=["fortnox-write", "currency-update"],
        verification_plan={"type": "read-after-write", "path": f"/currencies/{code}"},
    ...
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("currencies.update.plan", {"plan_out": plan_path, "code": code})
        ctx["out"].emit(out)
        return 0
    ...
    receipt_path = _write_receipt_if_requested(ctx, receipt)
    out = {"ok": bool(verification.get("ok")), "dry_run": False, "receipt": receipt, "receipt_out": receipt_path}
    ctx["audit"].write("currencies.update.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1


def cmd_currencies_remove(args: Any, ctx: dict[str, Any]) -> int:
    code = str(getattr(args, "code", "") or "").strip()
    selector = {
        "kind": "currency",
        "action": "remove",
    ...
    plan = _build_plan(
        ...
        risk_reasons=["fortnox-write", "currency-remove", "irreversible"],
        verification_plan={"type": "absence-check", "path": f"/currencies/{code}", "expect_http_status": 404},
        rollback_notes="No generic rollback. Recreate the currency explicitly if needed.",
        ctx=ctx,
    )
    plan_path = _write_plan_if_requested(ctx, plan)
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("currencies.remove.plan", {"plan_out": plan_path, "code": code})
        ctx["out"].emit(out)
        return 0

    if not bool(ctx.get("yes")):
        raise SafetyError("Refused: removing a currency requires --apply --yes")
    if not bool(ctx.get("ack_irreversible")):
        raise SafetyError("Refused: removing a currency requires --ack-irreversible")

    plan = _load_plan_from_ctx(ctx)
    _validate_plan_for_apply(plan, action="remove", selector=selector, payload_file=None, ctx=ctx)
    payload = request_json(ctx=ctx, method="DELETE", path=f"/currencies/{code}", expect_json=False)
    ...
    receipt_path = _write_receipt_if_requested(ctx, receipt)
    out = {"ok": bool(verification.get("ok")), "dry_run": False, "receipt": receipt, "receipt_out": receipt_path}
    ctx["audit"].write("currencies.remove.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1
