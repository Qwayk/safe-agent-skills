...
def _load_payload_file(path_str: str) -> tuple[Path, dict[str, Any], dict[str, Any]]:
    ...
    if not isinstance(price_list, dict):
        raise ValidationError("JSON file must contain a top-level PriceList object")
    return path, obj, price_list


def _extract_code_from_payload(price_list: dict[str, Any]) -> str | None:
    raw = price_list.get("Code")
    if raw is None:
        return None
    value = str(raw).strip()
...
def _extract_code_from_response(body: dict[str, Any] | None) -> str | None:
    if not isinstance(body, dict):
        return None
    price_list = body.get("PriceList")
    if not isinstance(price_list, dict):
        return None
    return _extract_code_from_payload(price_list)
...
def cmd_price_lists_create(args: Any, ctx: dict[str, Any]) -> int:
    payload_file, payload_obj, price_list = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    selector = {
        "kind": "price-list",
        "action": "create",
        "path": "/pricelists",
    ...
    plan = _build_plan(
        ...
        risk_level="medium",
        risk_reasons=["fortnox-write", "price-list-create"],
        verification_plan={"type": "read-after-write", "path_template": "/pricelists/{Code}"},
        rollback_notes="No generic rollback. Recreate the price list explicitly if needed.",
        ctx=ctx,
    )
    plan_path = _write_plan_if_requested(ctx, plan)
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("price-lists.create.plan", {"plan_out": plan_path})
        ctx["out"].emit(out)
        return 0
    ...
    payload = request_json(ctx=ctx, method="POST", path="/pricelists", json_body=payload_obj, expect_json=True)
    code = _extract_code_from_response(payload.get("body")) or _extract_code_from_payload(price_list)
    if not code:
    ...
    ctx["audit"].write("price-lists.create.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1


def cmd_price_lists_update(args: Any, ctx: dict[str, Any]) -> int:
    code = str(getattr(args, "code", "") or "").strip()
    payload_file, payload_obj, price_list = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    payload_code = _extract_code_from_payload(price_list)
    if payload_code and payload_code != code:
        raise ValidationError("PriceList.Code in the JSON file must match --code")
    selector = {
        "kind": "price-list",
        "action": "update",
        "path": f"/pricelists/{code}",
    ...
    plan = _build_plan(
        ...
        payload_obj=payload_obj,
        risk_level="medium",
        risk_reasons=["fortnox-write", "price-list-update"],
        verification_plan={"type": "read-after-write", "path": f"/pricelists/{code}"},
    ...
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("price-lists.update.plan", {"plan_out": plan_path, "code": code})
        ctx["out"].emit(out)
        return 0
    ...
    receipt_path = _write_receipt_if_requested(ctx, receipt)
    out = {"ok": bool(verification.get("ok")), "dry_run": False, "receipt": receipt, "receipt_out": receipt_path}
    ctx["audit"].write("price-lists.update.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1
