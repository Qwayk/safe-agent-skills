...
def _load_payload_file(path_str: str) -> tuple[Path, dict[str, Any], dict[str, Any]]:
    ...
    cost_center = obj.get("CostCenter")
    if not isinstance(cost_center, dict):
        raise ValidationError("JSON file must contain a top-level CostCenter object")
    return path, obj, cost_center


def _extract_code_from_payload(cost_center: dict[str, Any]) -> str | None:
    raw = cost_center.get("Code")
    if raw is None:
        return None
    value = str(raw).strip()
...
def _extract_code_from_response(body: dict[str, Any] | None) -> str | None:
    if not isinstance(body, dict):
        return None
    cost_center = body.get("CostCenter")
    if not isinstance(cost_center, dict):
        return None
    return _extract_code_from_payload(cost_center)
...
def _verify_absent(*, ctx: dict[str, Any], code: str) -> dict[str, Any]:
    ...
    return {
        ...
        "http_status": payload["status"],
        "data": payload["body"],
        "error": "Expected cost center to be absent after remove verification",
    }


def cmd_cost_centers_create(args: Any, ctx: dict[str, Any]) -> int:
    payload_file, payload_obj, cost_center = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    selector = {
        "kind": "cost-center",
        "action": "create",
        "path": "/costcenters",
    ...
    plan = _build_plan(
        ...
        risk_level="medium",
        risk_reasons=["fortnox-write", "cost-center-create"],
        verification_plan={"type": "read-after-write", "path_template": "/costcenters/{Code}"},
        rollback_notes="No generic rollback. Recreate the cost center explicitly if needed.",
        ctx=ctx,
    )
    plan_path = _write_plan_if_requested(ctx, plan)
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("cost-centers.create.plan", {"plan_out": plan_path})
        ctx["out"].emit(out)
        return 0
    ...
    code = _extract_code_from_response(payload.get("body")) or _extract_code_from_payload(cost_center)
    if not code:
    ...
    ctx["audit"].write("cost-centers.create.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1


def cmd_cost_centers_update(args: Any, ctx: dict[str, Any]) -> int:
    code = str(getattr(args, "code", "") or "").strip()
    payload_file, payload_obj, cost_center = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    payload_code = _extract_code_from_payload(cost_center)
    if payload_code and payload_code != code:
        raise ValidationError("CostCenter.Code in the JSON file must match --code")
    selector = {
        "kind": "cost-center",
        "action": "update",
        "path": f"/costcenters/{code}",
    ...
    plan = _build_plan(
        ...
        payload_obj=payload_obj,
        risk_level="medium",
        risk_reasons=["fortnox-write", "cost-center-update"],
        verification_plan={"type": "read-after-write", "path": f"/costcenters/{code}"},
    ...
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("cost-centers.update.plan", {"plan_out": plan_path, "code": code})
        ctx["out"].emit(out)
        return 0
    ...
    receipt_path = _write_receipt_if_requested(ctx, receipt)
    out = {"ok": bool(verification.get("ok")), "dry_run": False, "receipt": receipt, "receipt_out": receipt_path}
    ctx["audit"].write("cost-centers.update.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1


def cmd_cost_centers_remove(args: Any, ctx: dict[str, Any]) -> int:
    code = str(getattr(args, "code", "") or "").strip()
    selector = {
        "kind": "cost-center",
        "action": "remove",
    ...
    plan = _build_plan(
        ...
        risk_reasons=["fortnox-write", "cost-center-delete", "irreversible"],
        verification_plan={"type": "absence-check", "path": f"/costcenters/{code}", "expect_http_status": 404},
        rollback_notes="No generic rollback. Recreate the cost center explicitly if needed.",
        ctx=ctx,
    )
    plan_path = _write_plan_if_requested(ctx, plan)
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("cost-centers.remove.plan", {"plan_out": plan_path, "code": code})
        ctx["out"].emit(out)
        return 0

    if not bool(ctx.get("yes")):
        raise SafetyError("Refused: removing a cost center requires --apply --yes")
    if not bool(ctx.get("ack_irreversible")):
        raise SafetyError("Refused: removing a cost center requires --ack-irreversible")

    plan = _load_plan_from_ctx(ctx)
    _validate_plan_for_apply(plan, action="remove", selector=selector, payload_file=None, ctx=ctx)
    payload = request_json(ctx=ctx, method="DELETE", path=f"/costcenters/{code}", expect_json=False)
    ...
    receipt_path = _write_receipt_if_requested(ctx, receipt)
    out = {"ok": bool(verification.get("ok")), "dry_run": False, "receipt": receipt, "receipt_out": receipt_path}
    ctx["audit"].write("cost-centers.remove.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1
