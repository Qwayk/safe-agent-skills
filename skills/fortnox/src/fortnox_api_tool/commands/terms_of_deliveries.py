...
def _load_payload_file(path_str: str) -> tuple[Path, dict[str, Any], dict[str, Any]]:
    ...
    terms_of_delivery = obj.get("TermsOfDelivery")
    if not isinstance(terms_of_delivery, dict):
        raise ValidationError("JSON file must contain a top-level TermsOfDelivery object")
    return path, obj, terms_of_delivery


def _extract_code_from_payload(terms_of_delivery: dict[str, Any]) -> str | None:
    raw = terms_of_delivery.get("Code")
    if raw is None:
        return None
    value = str(raw).strip()
...
def _extract_code_from_response(body: dict[str, Any] | None) -> str | None:
    if not isinstance(body, dict):
        return None
    terms_of_delivery = body.get("TermsOfDelivery")
    if not isinstance(terms_of_delivery, dict):
        return None
    return _extract_code_from_payload(terms_of_delivery)
...
def cmd_terms_of_deliveries_create(args: Any, ctx: dict[str, Any]) -> int:
    payload_file, payload_obj, terms_of_delivery = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    selector = {
        "kind": "terms-of-delivery",
        "action": "create",
    ...
    plan = _build_plan(
        ...
        risk_level="medium",
        risk_reasons=["fortnox-write", "terms-of-delivery-create"],
        verification_plan={"type": "read-after-write", "path_template": "/termsofdeliveries/{Code}"},
        rollback_notes="No generic rollback. Recreate the terms-of-delivery record explicitly if needed.",
        ctx=ctx,
    )
    plan_path = _write_plan_if_requested(ctx, plan)
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("terms-of-deliveries.create.plan", {"plan_out": plan_path})
        ctx["out"].emit(out)
        return 0
    ...
    code = _extract_code_from_response(payload.get("body")) or _extract_code_from_payload(terms_of_delivery)
    if not code:
    ...
    ctx["audit"].write("terms-of-deliveries.create.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1


def cmd_terms_of_deliveries_update(args: Any, ctx: dict[str, Any]) -> int:
    code = str(getattr(args, "code", "") or "").strip()
    payload_file, payload_obj, terms_of_delivery = _load_payload_file(str(getattr(args, "json_file", "") or "").strip())
    payload_code = _extract_code_from_payload(terms_of_delivery)
    if payload_code and payload_code != code:
        raise ValidationError("TermsOfDelivery.Code in the JSON file must match --code")
    selector = {
        "kind": "terms-of-delivery",
        "action": "update",
        "path": f"/termsofdeliveries/{code}",
    ...
    plan = _build_plan(
        ...
        payload_obj=payload_obj,
        risk_level="medium",
        risk_reasons=["fortnox-write", "terms-of-delivery-update"],
        verification_plan={"type": "read-after-write", "path": f"/termsofdeliveries/{code}"},
    ...
    if not bool(ctx.get("apply")):
        out = {"ok": True, "dry_run": True, "plan": plan, "plan_out": plan_path}
        ctx["audit"].write("terms-of-deliveries.update.plan", {"plan_out": plan_path, "code": code})
        ctx["out"].emit(out)
        return 0
    ...
    receipt_path = _write_receipt_if_requested(ctx, receipt)
    out = {"ok": bool(verification.get("ok")), "dry_run": False, "receipt": receipt, "receipt_out": receipt_path}
    ctx["audit"].write("terms-of-deliveries.update.apply", {"receipt_out": receipt_path, "verified": verification.get("ok")})
    ctx["out"].emit(out)
    return 0 if bool(verification.get("ok")) else 1
