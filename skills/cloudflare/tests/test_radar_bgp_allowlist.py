...
class TestRadarBGPAllowlist(unittest.TestCase):
    def test_allowlist_includes_all_radar_bgp_operations(self) -> None:
        rows = [r for r in load_live_inventory_rows() if str(r.get("path_template") or "").strip().startswith("/radar/bgp/")]
        ...
        for r in rows:
            ...
            self.assertTrue(path.startswith("/radar/bgp/"))
    ...
    def test_operations_list_hides_radar_bgp_by_default(self) -> None:
    ...
    def test_operations_call_radar_bgp_refuses_missing_out_on_apply(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as d, patch("requests.Session.request", new=fake_request):
            ...
            buf = io.StringIO()
            cmd = allowlisted_operation_command_by_operation_id("radar-get-bgp-timeseries")
