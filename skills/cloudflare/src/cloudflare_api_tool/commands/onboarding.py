...
def cmd_onboarding(args: object, ctx: dict) -> int:
    ...
    missing: list[str] = []
    if not _has_nonempty_env_value(lines, "CLOUDFLARE_API_BASE_URL"):
        missing.append("CLOUDFLARE_API_BASE_URL")
    if not _has_nonempty_env_value(lines, "CLOUDFLARE_API_TOKEN"):
    ...
    steps = [
        ...
        "Run: cloudflare-api-tool --output json auth check",
    ]
    ...
    payload = {
        ...
        "onboarding": {
            ...
            "missing": missing,
            "next_command": "cloudflare-api-tool --output json auth check",
            "steps": steps,
        },
    }
    ...
    if str(getattr(args, "output", "json")) == "json":
        ...
        print("")
        print("Next: cloudflare-api-tool --output json auth check")
    return 0
