...
from .plan import env_fingerprint as env_fingerprint_for_plan
from .runs import (
    RunContext,
...
def build_parser() -> argparse.ArgumentParser:
    p = _ToolArgumentParser(prog="ga4-api-tool")
    ...
    p.add_argument("--fields", default=None, help="Google system parameter: partial response selector")
    p.add_argument("--quota-user", default=None, help="Google system parameter: quota user (string)")
    ...
    p.add_argument(
        "--receipt-out",
        default=None,
        help="Write a read-only job receipt; reserved for future safe write apply receipts",
    )
    p.add_argument(
        "--ack-irreversible",
        action="store_true",
    ...
    demo_write = demo_sub.add_parser("write", help="Write plan with current apply refusal (demo)")
    ...
    discovery_methods_cmd.register_discovery_commands(sub)
...
def main(argv: list[str]) -> int:
    ...
    try:
        if bool(args.version):
            payload = {"ok": True, "tool": "ga4-api-tool", "version": __version__}
            if args.output == "json":
                out.emit(payload)
            else:
                print(f"ga4-api-tool {__version__}")
            return 0

        if not getattr(args, "cmd", None):
            parser.error("Missing command. Use --help to see available commands.")

        command_str = "ga4-api-tool " + " ".join(argv)
        audit.bind_context(
            {
                "tool": "ga4-api-tool",
                "version": __version__,
        ...
        env_fp = env_fingerprint_for_plan(cfg)
        ...
        _finalize_run_artifacts(
            ...
            tool="ga4-api-tool",
            version=__version__,
            command="ga4-api-tool " + " ".join(argv),
            env_fingerprint=None,
            output_obj=out.last if isinstance(out.last, dict) else None,
            audit_log_path=run_audit_log_path or global_audit_log_path,
            audit_log_global_path=global_audit_log_path,
