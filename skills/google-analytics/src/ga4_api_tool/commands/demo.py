...
from ..plan import build_before_state_contract, build_before_state_refusal_plan, build_no_recovery_contract
from ..json_files import read_json_file, write_json_file


_BEFORE_STATE_REFUSAL_REASON = (
    "Refused: this GA4 demo command is a stub and cannot execute a real provider write."
)
...
def cmd_demo_read(args: Any, ctx: dict[str, Any]) -> int:
    _ = args
    out = {"ok": True, "message": "demo read (safe)", "env": ctx["cfg"].admin_base_url}
...
def _build_demo_plan(*, selector: str, ctx: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "env_fingerprint": ctx["cfg"].admin_base_url,
...
def _validate_plan_for_apply(plan: dict[str, Any], *, selector: str, ctx: dict[str, Any]) -> None:
    ...
    if str(baseline.get("env_fingerprint") or "") != str(ctx["cfg"].admin_base_url):
