...
from ..plan import (
    build_before_state_contract,
    build_before_state_refusal_plan,
    build_no_recovery_contract,
    env_fingerprint as env_fingerprint_for_plan,
)


_BEFORE_STATE_REFUSAL_REASON = (
    "Refused: this GA4 jobs command only has stub write actions and cannot execute real provider write rows."
)
...
def _demo_write(row: dict[str, str], ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"ok": True, "refused": True, "reasons": [_BEFORE_STATE_REFUSAL_REASON]}
...
def _env_fingerprint(cfg: Any) -> str:
    """
    Stable environment identifier for jobs plans, refusals, and read-only receipts.

    - In real CLI runs, `cfg` is a `ga4_api_tool.config.Config` and we use the same hash fingerprint
      as discovery-method plans (`ga4_api_tool.plan.env_fingerprint`).
    - In unit tests, we also accept a simple object with `base_url` to keep tests lightweight.
    """
    if cfg is None:
        return ""
    ...
    try:
        return str(env_fingerprint_for_plan(cfg))
    except Exception:
        admin = getattr(cfg, "admin_base_url", None)
        data = getattr(cfg, "data_base_url", None)
        parts: list[str] = []
        if isinstance(admin, str) and admin.strip():
            parts.append(f"admin_base_url={admin.strip()}")
        if isinstance(data, str) and data.strip():
            parts.append(f"data_base_url={data.strip()}")
        return "|".join(parts)


def _build_jobs_plan(*, job_file: str | None, rows: list[dict[str, Any]], ctx: dict[str, Any]) -> dict[str, Any]:
    has_write = _has_write_actions(rows)
    risk_level = "high" if has_write else "low"
    ...
    env_fp = _env_fingerprint(ctx.get("cfg"))
    return {
        ...
        "before_state": build_before_state_contract(required=has_write, provider="GA4"),
        "verification_plan": (
            build_before_state_refusal_plan(provider="GA4")
            if has_write
            else {"type": "per-row", "notes": "Read-only job actions return per-row results."}
        ),
...
def _validate_plan_for_apply(plan: dict[str, Any], *, ctx: dict[str, Any]) -> None:
    ...
    if str(baseline.get("env_fingerprint") or "") != str(_env_fingerprint(ctx.get("cfg"))):
