...
```bash
ga4-api-tool jobs run --file jobs.csv
```
...
```bash
ga4-api-tool jobs run --file jobs.csv --plan-out plan.json
```

2) Review the plan (human/Codex), then request apply from the saved plan:

```bash
ga4-api-tool --apply --yes --plan-in plan.json jobs run --file jobs.csv
```
...
Current jobs write apply requires explicit no-snapshot approval before any GA4 write action or stub receipt output until before-state capture exists.
...
If you want to include a write action, run with `--apply --yes` to confirm the missing-approval refusal:

```bash
ga4-api-tool --apply --yes jobs run --file examples/jobs_with_write.csv
```
