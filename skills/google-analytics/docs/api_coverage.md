...
Google Analytics coverage shows exactly what the shipped commands can do with GA4 properties, reports, audiences, links, and admin settings. Start here when an ask sounds possible but you need to know whether it is already shipped, read-only, plan-first, gated, excluded, or outside the tool.
...
A good first coverage check is: "Check whether the shipped commands can run this GA4 report, inspect this property, and show which admin changes are covered."
