...
class TestReadmePublicContract(unittest.TestCase):
    def test_public_readme_opening_stays_user_facing(self) -> None:
        ...
        self.assertTrue(text.startswith("# Google Analytics\n"))
        self.assertNotIn("## Simplicity lock", text)
        self.assertNotIn("# ga4-api-tool", text)
        self.assertIn("Google Analytics is where traffic", text)
        for stale_phrase in [
            "Use this skill when",
    ...
    def test_start_here_section_stays_human_facing(self) -> None:
        ...
        self.assertIn(
            "[What you can do with Google Analytics](docs/use_cases.md)",
            text,
        )
        self.assertIn(
            "[Connect your Google Analytics access](docs/onboarding.md)",
            text,
        )
    ...
    def test_quickstart_opens_with_human_links(self) -> None:
        ...
        self.assertIn(
            "[Connect your Google Analytics access](onboarding.md)",
            text,
        )
    ...
    def test_use_cases_stay_specific_and_human(self) -> None:
        ...
        self.assertIn("Google Analytics work usually starts with two questions", text)
